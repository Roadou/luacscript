function PaGlobal_ServerSelect_All:initialize()
  ...
  self._ui._stc_channelDesc[self._CHANNEL_TYPE.ROSEWAR] = UI.getChildControl(self._ui._stc_leftBg, "ROSEWAR_INFO")
  ...
  self._ui._stc_channelDesc[self._CHANNEL_TYPE.OCEANSIEGE_CHANNEL] = UI.getChildControl(self._ui._stc_leftBg, "OCEANSIEGECHANNEL_INFO")
  local roseWarTitle = UI.getChildControl(self._ui._stc_channelDesc[self._CHANNEL_TYPE.ROSEWAR], "StaticText_Title")
  local roseWarDesc = UI.getChildControl(self._ui._stc_channelDesc[self._CHANNEL_TYPE.ROSEWAR], "StaticText_Desc")
  roseWarTitle:SetTextMode(__eTextMode_AutoWrap)
  roseWarTitle:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_ROSEWARCHANNEL_SUB_TITLE"))
  roseWarDesc:SetTextMode(__eTextMode_AutoWrap)
  roseWarDesc:SetText([[


]] .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_ROSEWARCHANNEL_SUB_DESC") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_ROSEWARCHANNEL_DATE_SUB_TITLE") .. "\n" .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_ROSEWARCHANNEL_DATE_SUB_DESC"))
  ...
  self._ui._btn_eventBanner = UI.getChildControl(self._ui._stc_rightBg, "Button_EventBanner")
  self._ui._btn_eventBanner:SetShow(false)
  ...
  if isGameServiceTypeConsole() == true then
    self._movieURL = {
      "coui://UI_Movie/CS/Loading/Join/Edania_01.webm",
      "coui://UI_Movie/CS/Loading/Join/Edania_02.webm",
      "coui://UI_Movie/CS/Loading/Join/Edania_03.webm"
    }
  ...
  if _ContentsGroup_MorningLand2 == true then
    self._movieURL = {
      "coui://UI_Movie/CS/Loading/Login/login.webm"
    }
    if true == ToClient_isConsole() then
      self._movieLength = {9000, 9000}
    else
  ...
  self._moviePath = {
    "UI_Movie/PC/Loading/Login/Edania_01.bk2",
    "UI_Movie/PC/Loading/Login/Edania_02.bk2",
    "UI_Movie/PC/Loading/Login/Edania_03.bk2"
  }
  self._movieOrder = {
    1,
    2,
...
function PaGlobal_ServerSelect_All:initDesriptionData()
  ...
  self._channelDescriptionData = {
    ...
    [self._CHANNEL_TYPE.OCEANSIEGE_CHANNEL] = {
...
function PaGlobal_ServerSelect_All:initTreeData()
  ...
  local isAccessibleMultiNationSiege = ToClient_isMultiNationSiegeUser()
  local isAccessibleMultiNationChampionship = ToClient_isMultiNationChampionshipUser()
  ...
  for ii = 1, worldServerCount do
    ...
    local isMultiNationSiegeServer = channelServerData:isMultiNationSiegeServer()
    local isMultiNationChampionshipServer = channelServerData:isMultiNationChampionshipServer()
    if isMultiNationSiegeServer == true and isAccessibleMultiNationSiege == false then
      needSkip = true
    end
    if isMultiNationChampionshipServer == true and isAccessibleMultiNationChampionship == false then
      needSkip = true
    end
    if needSkip == false then
      ...
      for jj = 1, channelCount do
        ...
        if true == chData._isInstanceChannel or chData:isMultiNationChampionshipInstanceServer() == true then
          ...
          local isMultiNationServer = chData:isMultiNationServer()
          if isUseTotalAccountVer2() == true then
            ...
            if getAccessNationServerString == "" or getEnteredNationString == getAccessNationServerString then
              if isMultiNationServer == true then
                if isAccessibleMultiNationSiege == true or isAccessibleMultiNationChampionship == true then
                  tempTable[#tempTable + 1] = {
                    isWorldServer = false,
                    worldIndex = ii,
                    channelIndex = jj,
              ...
              elseif chData._isHardCoreChannel == true then
                hardCoreChannelTable[#hardCoreChannelTable + 1] = {
                  isWorldServer = false,
                  worldIndex = ii,
                  channelIndex = jj,
                  isSpeedChannel = chData._isSpeedChannel,
          ...
          elseif isMultiNationServer == true then
            if isAccessibleMultiNationSiege == true or isAccessibleMultiNationChampionship == true then
              tempTable[#tempTable + 1] = {
                isWorldServer = false,
                worldIndex = ii,
      ...
      if _ContentsGroup_HardCoreChannel == true then
        for idx = 1, #hardCoreChannelTable do
          worldElement:createChild(toInt64(0, key))
          self._treeData[key] = hardCoreChannelTable[idx]
          key = key + 1
        end
      end
      local isCrossplayMode = ToClient_getGameOptionControllerWrapper():getSamePlatformOnly()
...
function PaGlobal_ServerSelect_All:createChannelBranch(content, key)
  ...
  if self._isConsole == true and #self._listContents == 2 then
  ...
  local stc_RoseWar = UI.getChildControl(channelButtonBG, "StaticText_RoseWarServer")
  ...
  if channelServerData._isSiegeChannel == true and isBeingWar == true then
    if channelServerData:getSiegeTerritoryType() == __eSiegeTerritoryType_Ocean then
      self:showChannelIcon(channelButtonBG, self._CHANNEL_TYPE.OCEANSIEGE_CHANNEL)
    else
  ...
  elseif channelServerData:isPVPServer() and channelServerData._isHardCoreChannel == false then
  ...
  if _ContentsGroup_RoseWar == true and channelServerData._roseWarGameNo == true and ToClient_GetRoseWarState() == __eRoseWar_Start then
    self:showChannelIcon(channelButtonBG, self._CHANNEL_TYPE.ROSEWAR)
  end
  local isMultiNationServer = channelServerData:isMultiNationServer()
  if isMultiNationServer == true then
    txt_State:SetText("")
...
function PaGlobal_ServerSelect_All:enterMemorizedChannel(channelIdx)
  ...
  if channelServerData:isMultiNationSiegeServer() == true and PaGlobal_AsiaChampionshop_Agreement_Open ~= nil then
    PaGlobal_AsiaChampionshop_Agreement_Open(self._selectedWorldIndex - 1, channelIdx - 1)
    return
  end
  if channelServerData:isMultiNationChampionshipServer() == true then
    ToClient_PrepareJoinMultiNationField(__eMultiNationServerTypeChampionShip)
    local function joinChampionshipServer()
      selectServerGroup(self._selectedWorldIndex - 1, channelIdx - 1)
    end
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY"),
      content = "\236\134\148\235\157\188\235\160\136 \236\177\148\237\148\188\236\150\184\236\137\189 \236\132\156\235\178\132\236\151\144 \236\158\133\236\158\165\237\149\152\236\139\156\234\178\160\236\138\181\235\139\136\234\185\140?",
      functionApply = joinChampionshipServer,
...
function PaGlobal_ServerSelect_All:setMainServer(worldIndex, serverIndex)
  ...
  if _ContentsGroup_HardCoreChannel == true and serverData._isHardCoreChannel == true then
    ...
    local messageBoxData = {
      ...
      content = PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantSelectMainChannel"),
      functionApply = MessageBox_Empty_function,
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    }
    MessageBox.showMessageBox(messageBoxData)
    ...
    if value.isOpen then
      ...
      if value.imageCount > 0 then
        ...
        for index = 1, value.imageCount do
          ...
          if v == 10 then
            targetControl:ChangeTextureInfoNameAsync("Combine/Loading/" .. value.iconPath .. index - 1 .. ".dds")
          elseif true == _ContentsGroup_ChinaFontColor then
            if "bgCalpeon_1" ~= tostring(value.iconPath .. index) and "bgCalpeon_5" ~= tostring(value.iconPath .. index) then
              targetControl:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Lobby/" .. value.iconPath .. index .. ".dds")
            end
          else
            targetControl:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Lobby/" .. value.iconPath .. index .. ".dds")
          end
...
function PaGlobal_ServerSelect_All:getChannelIcon(buttonBG, iconType)
  ...
  elseif self._CHANNEL_TYPE.ROSEWAR == iconType then
    return UI.getChildControl(buttonBG, "StaticText_RoseWarServer")
  elseif self._CHANNEL_TYPE.HARDCORE_CHANNEL == iconType then
    return UI.getChildControl(buttonBG, "StaticText_HardCoreServer")
  elseif self._CHANNEL_TYPE.OCEANSIEGE_CHANNEL == iconType then
    return UI.getChildControl(buttonBG, "StaticText_OceanSiegeServer")
  end
end
function PaGlobal_ServerSelect_All:updateChannelIconPos(buttonBG, iconType)
  ...
  if self._CHANNEL_TYPE.ROSEWAR ~= iconType then
    local roseWarIcon = self:getChannelIcon(buttonBG, self._CHANNEL_TYPE.ROSEWAR)
    if true == roseWarIcon:GetShow() then
      addIconPos = addIconPos + roseWarIcon:GetSizeX()
    end
  end
  ...
  if self._CHANNEL_TYPE.OCEANSIEGE_CHANNEL ~= iconType then
    local oceanSiegeChannelIcon = self:getChannelIcon(buttonBG, self._CHANNEL_TYPE.OCEANSIEGE_CHANNEL)
    if oceanSiegeChannelIcon:GetShow() == true then
      addIconPos = addIconPos + oceanSiegeChannelIcon:GetSizeX()
    end
  end
  local iconControl = self:getChannelIcon(buttonBG, iconType)
...
function PaGlobal_ServerSelect_All:initBanners_PC()
  ...
  if true == ToClient_IsContentsGroupOpen("1436") or true == _ContentsGroup_SeasonContents then
    ...
    self._ui._btn_eventBanner:SetShow(false)
    self._ui._btn_eventBanner:addInputEvent("Mouse_LUp", "PaGlobalFunc_ServerSelectEventNotice()")
    self._ui._btn_eventBanner:ComputePos()
  end
end
function PaGlobalFunc_ServerSelectEventNotice()
  local serviceType = getGameServiceType()
  local resourceType = ToClient_getResourceType()
  local kr = "https://www.kr.playblackdesert.com/ko-KR/News/Detail?groupContentNo=13244\t"
  local jp = "https://www.jp.playblackdesert.com/ja-JP/News/Detail?groupContentNo=9712"
  local ru = "https://www.ru.playblackdesert.com/News/Notice/Detail?boardNo=10241&countryType=ru-RU"
  local naeu = "https://www.naeu.playblackdesert.com/News/Detail?groupContentNo=7921"
  local tw = "https://www.tw.playblackdesert.com/zh-TW/News/Detail?groupContentNo=17088&countryType=zh-TW"
  local sa = "https://www.sa.playblackdesert.com/News/Detail?groupContentNo=5581&countryType=pt-BR"
  local tr = "https://www.tr.playblackdesert.com/News/Detail?groupContentNo=11337"
  local asia = "https://blackdesert.pearlabyss.com/Asia/News/Notice/Detail?_boardNo=7454"
  local urlList = {
    [1] = kr,
    [2] = kr,
    [3] = kr,
...
function PaGlobal_ServerSelect_All:initBanners()
  ...
  elseif _ContentsGroup_UsingPMAPI == true then
  ...
  for ii = 0, #self._ui._stc_bannerBgList do
    ...
    self._ui._stc_bannerBgList[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_A, "HandleEventPad_ServerSelect_All_ToWebBanner(\"A\", " .. ii .. ")")
  end
...
function PaGlobal_ServerSelect_All:closeChannelDesc(exception)
  for i, v in pairs(self._ui._stc_channelDesc) do
    if i ~= exception then
      v:SetShow(false)
    end
  end
