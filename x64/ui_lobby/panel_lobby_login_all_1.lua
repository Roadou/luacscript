...
function PaGlobal_Login_All:initLoginImage()
  ...
  if true == _ContentsOption_CH_LOGO then
    ...
    for index = 1, 5 do
      ...
      targetControl:ChangeTextureInfoNameAsync(CHTable[index])
    ...
    for index = 1, imageCount do
      local targetControl = UI.createControl(__ePAUIControl_Static, Panel_Lobby_Login_All, "Static_Bg_" .. imageIndex)
      CopyBaseProperty(tempBg, targetControl)
      targetControl:ChangeTextureInfoNameAsync(baseLink .. "bgCalpeon_" .. index .. ".dds")
      targetControl:SetSize(screenX, screenY)
      targetControl:SetPosX(0)
      targetControl:SetPosY(0)
      targetControl:SetAlpha(0)
...
function PaGlobal_Login_All:onScreenResize()
  ...
  if isGameTypeKorea() or isGameTypeTaiwan() or isGameTypeGT() or isGameTypeTR() or isGameTypeTH() or isGameTypeID() or true == self._isConsole or isGameTypeRussia() or isGameTypeJapan() or isGameTypeNA() or isGameTypeSA() or isGameTypeASIA() then
  ...
  elseif _ContentsOption_CH_LOGO then
    ...
    self._ui._stc_daumCI:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/lobby/login_CI_Daum.dds")
