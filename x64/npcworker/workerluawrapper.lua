...
function workerWrapper:getWorkString()
  ...
  if self:isWorking() then
    ...
    elseif self._workingWrapper:isType(CppEnums.NpcWorkingType.eNpcWorkingType_PalaceManagement) then
      workString = Util.Time.timeFormatting(math.ceil(working_LeftTime / 1000))
      local esSSW = ToClient_getItemExchangeSourceStaticStatusWrapperByWorker(self._workerNoRaw)
      if nil ~= esSSW then
        local _leftWorkCount = self:getWorkingCountXXX()
        local resultItemWrapper = esSSW:getResultItemStaticStatusWrapper()
        if resultItemWrapper ~= nil then
          if _leftWorkCount > 0 then
            workString = Util.Time.timeFormatting(math.ceil(working_LeftTime / 1000)) .. " [" .. esSSW:getDescription() .. ":" .. resultItemWrapper:getName() .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERLUAWRAPPER_LEFTWORKCOUNT", "leftWorkCount", _leftWorkCount) .. "]"
          else
            workString = Util.Time.timeFormatting(math.ceil(working_LeftTime / 1000)) .. " [" .. esSSW:getDescription() .. ":" .. resultItemWrapper:getName() .. "]"
          end
        elseif _leftWorkCount > 0 then
          workString = Util.Time.timeFormatting(math.ceil(working_LeftTime / 1000)) .. " [" .. esSSW:getDescription() .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERLUAWRAPPER_LEFTWORKCOUNT", "leftWorkCount", _leftWorkCount) .. "]"
        else
          workString = Util.Time.timeFormatting(math.ceil(working_LeftTime / 1000)) .. " [" .. esSSW:getDescription() .. "]"
        end
...
function workerWrapper:getWorkStringPalace()
  self:reflesh()
  local workString = ""
  if self:isWorking() then
    local working_LeftTime = 0
    ...
    if self._workingWrapper:isType(CppEnums.NpcWorkingType.eNpcWorkingType_PalaceManagement) then
      workString = ""
      local esSSW = ToClient_getItemExchangeSourceStaticStatusWrapperByWorker(self._workerNoRaw)
      if nil ~= esSSW then
        local _leftWorkCount = self:getWorkingCountXXX()
        local resultItemWrapper = esSSW:getResultItemStaticStatusWrapper()
        if resultItemWrapper ~= nil then
          workString = PAGlobalFunc_ReturnAppliedItemColorTextForNewUI("[" .. resultItemWrapper:getName() .. "]", resultItemWrapper)
        else
          workString = esSSW:getDescription()
        end
      end
    end
  end
...
function workerWrapper:getWorkLeftTimePalace()
  self:reflesh()
  local workString = ""
  if self:isWorking() then
    local working_LeftTime = 0
    ...
    if self._workingWrapper:isType(CppEnums.NpcWorkingType.eNpcWorkingType_PalaceManagement) then
      ...
      if nil ~= esSSW then
        ...
        if resultItemWrapper ~= nil then
          if _leftWorkCount > 0 then
            workString = Util.Time.timeFormatting(math.ceil(working_LeftTime / 1000)) .. " " .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERLUAWRAPPER_LEFTWORKCOUNT", "leftWorkCount", _leftWorkCount)
          else
            workString = Util.Time.timeFormatting(math.ceil(working_LeftTime / 1000))
          end
        elseif _leftWorkCount > 0 then
...
function workerWrapper:getWorkTimePercents(workingType, houseUseType, waypointKey, itemExchangeKeyRaw)
  self:reflesh()
  local checkData = self._workerWrapper:get():getStaticSkillCheckData()
  checkData:set(workingType, houseUseType, waypointKey)
  checkData:setItemExchangeStaticStatus(ItemExchangeKey(itemExchangeKeyRaw))
  checkData._diceCheckForceSuccess = true
  return self._workerWrapper:get():getWorkTimePercents(checkData)
end
function workerWrapper:getWorkTimeStatic(workingType, houseUseType, waypointKey, itemExchangeKeyRaw)
  self:reflesh()
  local checkData = self._workerWrapper:get():getStaticSkillCheckData()
  checkData:set(workingType, houseUseType, waypointKey)
  checkData:setItemExchangeStaticStatus(ItemExchangeKey(itemExchangeKeyRaw))
  checkData._diceCheckForceSuccess = true
  return self._workerWrapper:get():getWorkTimeStatic(checkData)
end
...
function workerWrapper:getWorkingNodeDescXXX()
  ...
  if self:isWorking() then
    ...
    elseif self._workingWrapper:isType(CppEnums.NpcWorkingType.eNpcWorkingType_PalaceManagement) then
      local name = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_PALACEMANAGEMENT_TITLE")
      workString = name .. ":" .. self._workingWrapper:getWorkingNodeName()
    end
  else
    workString = ""
  end
