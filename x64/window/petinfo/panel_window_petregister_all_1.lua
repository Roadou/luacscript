function PaGlobal_PetRegister_All:initialize()
  ...
  PaGlobal_PetRegister_All:setDescText(true)
...
function PaGlobal_PetRegister_All:inputPetName(fromWhereType, fromSlotNo)
  ...
  PaGlobal_PetRegister_All:setDescText(self._isRegister)
...
function PaGlobal_PetRegister_All:changePetName()
  if Panel_Window_PetRegister_All == nil then
    return
  end
  if Panel_Window_PetRegister_All:GetShow() == true then
  ...
  PaGlobal_PetRegister_All:setDescText(self._isRegister)
  self._ui.stc_TitleText:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_CHANGENAME"))
  self._ui.stc_PetIcon:ChangeTextureInfoNameAsync(self._selectPetIconPath)
...
function PaGlobal_PetRegister_All:changePetNameConfirm()
  if Panel_Window_PetRegister_All == nil then
    return
  end
  ClearFocusEdit()
  local petName = self._ui.edit_Name:GetEditText()
  local function confirm_ChangeName()
    local petNo_s64 = tonumber64(PaGlobal_PetRegister_All._selectPetNo)
    ToClient_requestChangePetName(petNo_s64, petName, __ePetType_Normal)
    PaGlobal_PetRegister_All:prepareClose()
  end
  local _title = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_CHANGENAME")
  local _contenet = PAGetStringParam1(Defines.StringSheet_RESOURCE, "PANEL_CHANGEPETNAME_CONTINUE", "name", petName)
  local messageBoxData = {
    title = _title,
    content = _contenet,
    functionYes = confirm_ChangeName,
    functionNo = MessageBox_Empty_function,
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  }
  MessageBox.showMessageBox(messageBoxData)
end
function PaGlobal_PetRegister_All:setDescText(isRegister)
  local descPath
  if isRegister == true then
    descPath = "LUA_PETREGISTER_DESC_PARAM"
  else
    descPath = "LUA_PETCHANGENAME_DESC_PARAM"
  end
