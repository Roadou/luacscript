...
function PaGlobal_Itemwarp:itemUseNotify(whereType, slotNo, regionKey)
  ...
  PaGlobal_Itemwarp._selectTownKey = 0
  PaGlobal_Itemwarp:registEvent(0)
  if 0 == regionKey then
...
function PaGlobal_Itemwarp:showOceanSiegeWarpTownList()
  PaGlobal_Itemwarp._selectWhereType = nil
  PaGlobal_Itemwarp._selectSlotNo = nil
  PaGlobal_Itemwarp._selectTownKey = 0
  PaGlobal_Itemwarp:registEvent(1)
  PaGlobal_Itemwarp._ui.list2_Town:getElementManager():clearKey()
  local nearTownListCount = ToClient_GetSiege2024RevivePositionCount()
  for key = 0, nearTownListCount - 1 do
    PaGlobal_Itemwarp._ui.list2_Town:getElementManager():pushKey(toInt64(0, key))
  end
  PaGlobal_Itemwarp:prepareOpen()
end
function PaGlobal_Itemwarp:gotoSelectTownOceanSiege()
  local gotoTown = function()
    ToClient_RequestOceanSiegeTeleportNearSeaport(PaGlobal_Itemwarp._selectTownKey)
    PaGlobal_Itemwarp:prepareClose()
  end
  local areaName = ToClient_GetRegionNameByReviveIndex(PaGlobal_Itemwarp._selectTownKey)
  local messageBoxMemo = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_ITEMWARP_MSGCONTENT", "areaName", areaName)
  local messageBoxData = {
    title = PAGetString(Defines.StringSheet_GAME, "LUA_ITEMWARP_MSGTITLE"),
    content = messageBoxMemo,
...
function PaGlobal_Itemwarp:registEvent(index)
  if index == 0 then
    PaGlobal_Itemwarp._ui.btn_Apply:addInputEvent("Mouse_LUp", "HandleEventLUp_Itemwarp_GotoSelectTown()")
    PaGlobal_Itemwarp._ui.list2_Town:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_Itemwarp_TownListCreate")
  elseif index == 1 then
    PaGlobal_Itemwarp._ui.btn_Apply:addInputEvent("Mouse_LUp", "PaGlobal_Itemwarp:gotoSelectTownOceanSiege()")
    PaGlobal_Itemwarp._ui.list2_Town:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_Itemwarp_OceanSiegeCreateTownList")
  end
end
