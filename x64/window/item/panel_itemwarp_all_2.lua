...
function PaGlobal_Itemwarp_TownListCreate(content, key)
  local townListBlueBg = UI.getChildControl(content, "RadioButton_TownList_BlueBG")
  ...
  for index = 0, nearTownListCount - 1 do
    if _key == index then
      ...
      townListBlueBg:SetText(areaName)
    end
  end
...
function PaGlobal_Itemwarp_OceanSiegeCreateTownList(content, key)
  local townListBlueBg = UI.getChildControl(content, "RadioButton_TownList_BlueBG")
  local reviceIndex = Int64toInt32(key)
  if PaGlobal_Itemwarp._selectTownKey == reviceIndex then
    townListBlueBg:SetCheck(true)
  else
    townListBlueBg:SetCheck(false)
  end
  local areaName = ToClient_GetRegionNameByReviveIndex(reviceIndex)
  townListBlueBg:SetText(areaName)
  townListBlueBg:addInputEvent("Mouse_LUp", "ItemWarp_SetectTown(" .. reviceIndex .. ")")
end
