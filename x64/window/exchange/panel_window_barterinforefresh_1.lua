...
function PaGlobal_BarterInfoRefresh:close()
  if ToClient_WorldMapIsShow() == true then
    if WorldMapPopupManager:popWantPanel(Panel_Window_Barter_Refresh) == false then
...
function PaGlobal_BarterInfoRefresh:requestRefreshBarterInfo()
  if 2 == self._selectIndex then
    if _ContentsGroup_BarterConvenienceRenewal == true then
      if ToClient_isCanChangeBarterList(self._selectIndex) == false then
      ...
      if needCountOnce <= 0 then
        return
      end
      local maxCount = remainCount / needCountOnce
      if remainCount % needCountOnce ~= 0 then
        maxCount = maxCount + 1
      end
      local maxCount_s64 = toInt64(0, maxCount)
      local remainCoolTimeSec = ToClient_changeBarterInfoRemainSec()
      local needCountForOneMinute = ToClient_getBarterConvertNeedExchangeCount()
      if needCountForOneMinute > 0 then
        local minutePerOnce = needCountOnce / needCountForOneMinute
        local secondPerOnce = minutePerOnce * 60
        local secondPerOnce_s64 = toInt64(0, secondPerOnce)
        local timeMaxCount = remainCoolTimeSec / secondPerOnce_s64
        if remainCoolTimeSec % secondPerOnce_s64 ~= Defines.s64_const.s64_0 then
          timeMaxCount = timeMaxCount + Defines.s64_const.s64_1
        end
        if maxCount_s64 > timeMaxCount then
          maxCount_s64 = timeMaxCount
        end
      end
      if maxCount_s64 == Defines.s64_const.s64_0 then
        maxCount_s64 = Defines.s64_const.s64_1
      end
      Panel_NumberPad_Show_MaxCount(true, maxCount_s64, nil, PaGlobal_BarterInfoSearch_RequestCoolDownBarterListConfirm)
    else
      PaGlobal_BarterInfoSearch_RequestChangeBarterList()
      return
    end
