    ...
    if true == dataArray.isTitle then
      ...
      for Idx = 0, WorkList_Control.maxIconRow - 1 do
        ...
        if itemExchangeSSW:isSet() then
          ...
          slotPool.icon:ChangeTextureInfoNameAsync(workIcon)
    ...
    for recipeIdx = 0, recipeCount - 1 do
      ...
      if CppEnums.eHouseUseType.Depot ~= groupType then
        ...
        for gradeIdx = 0, maxLevel - 1 do
          ...
          upgradeBtn.Mask:SetShow(false)
          upgradeBtn.BG:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/houseinfo/housecontrol_00.dds")
          local x1, y1, x2, y2
...
function HouseUseTypeState_Control:Update()
  ...
  self._Icon_Working:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Default/Default_Buttons.dds")
...
function FGlobal_HouseUseTypeState_Update(guildNoRaw)
  if Worldmap_Grand_GuildHouseControl:GetShow() and guildNoRaw == ownerGuildNoRaw then
    ...
    HouseUseTypeState_Control._Icon_Working:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Default/Default_Buttons.dds")
...
function HouseImage_Control:Update()
  ...
  if nil ~= guildHouse_Img[getKeyRaw] then
    self._Image:ChangeTextureInfoNameAsync(guildHouse_Img[getKeyRaw])
  else
    self._Image:ChangeTextureInfoNameAsync("icon/new_icon/03_etc/06_housing/1201.dds")
  end
end
