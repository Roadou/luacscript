...
function HandleEventLUp_WorldMap_StableList_ShowBeforeLook(slotIdx)
  ...
  if true == slot._isLookChange then
    ...
    slot._stc_HorseImage:ChangeTextureInfoNameAsync(servantInfo:getBaseIconPath1())
  end
...
function PaGlobal_WorldMap_StableList_TransferOpen(slotNo)
  if nil == slotNo or slotNo < 0 or slotNo >= PaGlobal_WorldMap_StableList._MAXSLOTCOUNT then
    return
  end
