...
function Work_Info:_setDataByIndex(index)
  ...
  if nil ~= characterStaticStatusWrapper then
    ...
    if nil ~= itemSSW then
      defalut_Control._work_Info._Result_Icon:ChangeTextureInfoNameAsync("icon/" .. itemSSW:getIconPath())
  ...
  for index = 1, #workerArray do
    ...
    if nil ~= workerWrapper then
      if CppEnums.NpcWorkingType.eNpcWorkingType_Count == workerWrapper:getWorkingType() and not workerWrapper:getIsAuctionInsert() then
        ...
        local regionNameRaw = regionInfo:getAreaName()
        local distance = 0
        ...
        if isCalculated == false then
          distance = ToClient_getCalculateMoveDistance(WorkerNo(workerNo), Work_Info._position) / 100
