...
local function Worker_SortByRegionInfo()
  ...
  for index = 1, waitingWorkerCount do
    ...
    if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
      ...
      local distance = workerData._distance
  ...
  for Index = 1, waitingWorkerCount do
    ...
    if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
      ...
      if isCalculated == false then
        distance = ToClient_getCalculateMoveDistance(workerNo, Work_Info._position) / 100
...
function Plant_Work_Info_Update(isWorkerOver)
  ...
  if nil ~= workerData then
    ...
    defalut_Control._work_Info._WorkDestination:SetSelectItemContentsText(workerData._regionNameRaw)
  else
...
function FGlobal_Plant_WorkManager_Open(node, isNormalNode)
  ...
  if Worker_List._selected_Index ~= nil then
    local workerData = Worker_List._data_Table[Worker_List._selected_Index]
    defalut_Control._work_Info._WorkDestination:SetSelectItemContentsText(workerData._regionNameRaw)
  end
  defalut_Control._currentNode = node
  defalut_Control._currentIsNormalNode = isNormalNode
end
function FGlobal_Plant_WorkManager_ReOpen()
  if defalut_Control._currentNode ~= nil then
    if defalut_Control._currentIsNormalNode == nil then
      FGlobal_Plant_WorkManager_Open(defalut_Control._currentNode)
    else
      FGlobal_Plant_WorkManager_Open(defalut_Control._currentNode, defalut_Control._currentIsNormalNode)
    end
  end
end
