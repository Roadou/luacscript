...
function PaGlobal_Worldmap_Grand_WaypointPreset:prepareOpen()
  ...
  if ToClient_isInInstanceField(__eInstanceContentsType_RedBattleField) == true then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantUseFunction"))
    return
  end
  self:update()
  self:open()
