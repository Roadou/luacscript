...
local isOpendedSearchMenu = false
local isOpendedContentsMenu = false
...
local savedBtn = {}
local bookMarkBtn, useMagnusPathBtn, useDefaultPathBtn
function initializeNaviBtn()
  ...
  useMagnusPathBtn = UI.getChildControl(Panel_NaviButton, "Button_UseMagnusPath")
  useDefaultPathBtn = UI.getChildControl(Panel_NaviButton, "Button_UseDefaultPath")
  useMagnusPathBtn:SetShow(false)
  useDefaultPathBtn:SetShow(false)
  ...
  if naviBtnSizeX < naviBtn:GetTextSizeX() + 10 then
    ...
    useMagnusPathBtn:SetSize(naviBtn:GetSizeX(), naviBtn:GetSizeY())
    useDefaultPathBtn:SetSize(naviBtn:GetSizeX(), naviBtn:GetSizeY())
  end
  ...
  if _ContentsGroup_FindWayMagnus == true then
    useMagnusPathBtn:addInputEvent("Mouse_LUp", "HandleClicked_UseMagnusPath()")
    useMagnusPathBtn:addInputEvent("Mouse_On", "SimpleTooltip_MagnusPathBtn(true)")
    useMagnusPathBtn:addInputEvent("Mouse_Out", "SimpleTooltip_MagnusPathBtn(false)")
    useDefaultPathBtn:addInputEvent("Mouse_LUp", "HandleClicked_RecaculateNavigationPath()")
  else
    Panel_NaviButton:SetSize(Panel_NaviButton:GetSizeX(), naviBtn:GetSizeY() + 10)
  end
end
...
function WorldMap_ShortcutButton_RePos()
  ...
  local isLoopPath = ToClient_WorldMapNaviIsLoopPath()
  if isLoopPath == true then
  ...
  if _ContentsGroup_FindWayMagnus == true then
    if savedBtn[1]:GetShow() == true or isLoopPath == true then
      useMagnusPathBtn:SetShow(false)
      useDefaultPathBtn:SetShow(false)
    elseif ToClient_IsWorldMapNaviMagnusMode() == true then
      useMagnusPathBtn:SetShow(false)
      useDefaultPathBtn:SetShow(true)
      naviBtn:SetIgnore(true)
    else
      useMagnusPathBtn:SetShow(true)
      useDefaultPathBtn:SetShow(false)
      naviBtn:SetIgnore(false)
    end
    if useMagnusPathBtn:GetShow() == true or useDefaultPathBtn:GetShow() == true then
      Panel_NaviButton:SetSize(naviBtn:GetSizeX() + 10, 155)
    else
      Panel_NaviButton:SetSize(naviBtn:GetSizeX() + 10, 105)
    end
  else
    Panel_NaviButton:SetSize(naviBtn:GetSizeX() + 10, 105)
  end
...
function HandleClicked_CompleteNode()
  ...
  local isDesert = selfPlayer:getRegionInfoWrapper():isDesert()
  if nil ~= isDesert and true == isDesert and (false == _ContentsGroup_DesertOceanNaviAvailable or false == selfPlayer:isResistDesert()) then
    return
  end
  ...
  for idx = 1, 3 do
    ...
    savedBtn[idx]:SetPosY(naviBtn:GetSizeY() + 10)
  end
  if _ContentsGroup_FindWayMagnus == true then
    useMagnusPathBtn:SetShow(false)
    useDefaultPathBtn:SetShow(false)
...
function HandleClicked_UseMagnusPath()
  ...
  if selfPlayer:getRegionInfoWrapper() == nil then
    return
  end
  if Panel_WorldMap_BookMark ~= nil and Panel_WorldMap_BookMark:GetShow() == true then
    PaGlobalFunc_WorldMap_BookMark_Close()
  end
  local isDesert = selfPlayer:getRegionInfoWrapper():isDesert()
  if nil ~= isDesert and true == isDesert then
  ...
  local rv = ToClient_UseMagnusPath()
  if rv ~= 0 then
    Proc_ShowMessage_Ack(PAGetStringSymNo(rv))
    return
  end
  ToClient_HighlightMagnusNaviPath(false)
  TooltipSimple_Hide()
  useMagnusPathBtn:SetShow(false)
  useDefaultPathBtn:SetShow(true)
end
function HandleClicked_RecaculateNavigationPath()
  TooltipSimple_Hide()
  useDefaultPathBtn:SetShow(false)
  useMagnusPathBtn:SetShow(true)
  naviBtn:SetIgnore(false)
  ToClient_RecalculateNavigationPath(NavigationGuideParam())
end
function SimpleTooltip_MagnusPathBtn(isShow)
  if isShow == false then
    ToClient_HighlightMagnusNaviPath(isShow)
    TooltipSimple_Hide()
    return
  end
  ToClient_HighlightMagnusNaviPath(isShow)
...
function FromClient_RClickWorldmapPanel(pos3D, immediately, isTopPicking)
  ...
  local showNaviPanelOnDesert = false
  if _ContentsGroup_DesertOceanNaviAvailable == true and isResistDesert == true then
    showNaviPanelOnDesert = true
  end
  if ToClient_WorldMapNaviIsLoopPath() == false and isLoopAvailable == true and (showNaviPanelOnDesert == true or isToDesert == false and isFromDesert == false) and ToClient_WorldMapNaviEmpty() == false and HideAutoCompletedNaviBtn == false then
...
function FGlobal_PopCloseWorldMapActual()
  ...
  if _ContentsGroup_UseCharacterWp == true then
    if PaGlobalFunc_UseCharacterWp_All_IsShow ~= nil and PaGlobalFunc_UseCharacterWp_All_IsShow() == true then
      PaGlobalFunc_UseCharacterWp_All_Close()
    end
  elseif PaGlobalFunc_WorldMap_Grand_NodeLevel_GetShow ~= nil and PaGlobalFunc_WorldMap_Grand_NodeLevel_GetShow() == true then
...
function FromClient_WorldMapOpen()
  ...
  if _ContentsGroup_PalaceManagement == true and Panel_Window_PalaceManagement_Map:GetShow() == true then
    PaGlobal_PalaceManagement_Map_Close()
  end
  ...
  if _ContentsGroup_WorldMapMenuFilter == true then
    ToClient_CheckConditionForAbyssOneEnterShow()
  end
  ...
  if PaGlobalFunc_WorldMapHardCoreModeSet() == false and 0 ~= worldmapTabNo then
    WorldMapStateChange(worldmapTabNo, true)
...
function PaGlobal_NaviButton_Open()
  if ToClient_WorldMapNaviEmpty() == true and ToClient_WorldMapNaviIsLoopPath() == true then
    return
  end
  local naviEndPos = ToClient_getNaviEndPathPostion()
  if naviEndPos.x == 0 and naviEndPos.y == 0 and naviEndPos.z == 0 then
...
function FGlobal_WorldMapCloseBeforeCheck()
  ...
  if ToClient_isConsole() == true and nil ~= Panel_Window_WorkerAuction_All then
  ...
  if true == _ContentsGroup_PalaceManagement then
    if nil ~= Panel_Window_PalaceManagement_WorkManager and true == Panel_Window_PalaceManagement_WorkManager:GetShow() then
      PaGlobal_PalaceManagement_Worker_Close()
      return false
    end
    if nil ~= Panel_Window_PalaceManagement_Node and true == Panel_Window_PalaceManagement_Node:GetShow() then
      PaGlobal_PalaceManagement_Node_Close()
      return false
    end
    if nil ~= Panel_Window_PalaceManagement_Map and true == Panel_Window_PalaceManagement_Map:GetShow() then
      PaGlobal_PalaceManagement_Map_Close()
      return false
    end
  end
...
function FGlobal_WorldMapWindowEscape()
  ...
  local _worldmapUI = {
    ...
    [19] = _ContentsGroup_WorldMapNodeNavigation == false or _ContentsGroup_WorldMapNodeNavigation == true and PaGlobal_Worldmap_Grand_NodeNavigation_GetShow() == false,
    [20] = _ContentsGroup_PalaceManagement == false or _ContentsGroup_PalaceManagement == true and Panel_Window_PalaceManagement_Map:GetShow() == false,
    [21] = _ContentsGroup_PalaceManagement == false or _ContentsGroup_PalaceManagement == true and Panel_Window_PalaceManagement_Node:GetShow() == false,
    [22] = _ContentsGroup_PalaceManagement == false or _ContentsGroup_PalaceManagement == true and Panel_Window_PalaceManagement_WorkManager:GetShow() == false
  }
  if true == ToClient_WorldMapIsShow() then
...
function FGlobal_WorldMapClose()
  ...
  isFirstCall = false
  if ToClient_isConsole() == false and nil ~= Panel_Window_WorkerManager_All and true == Panel_Window_WorkerManager_All:GetShow() then
    PaGlobalFunc_WorkerManager_All_Close()
  end
...
function FGlobal_WorldMapCloseSubPanel()
  ...
  PaGlobal_Edania_WorldMapTooltip_Close()
...
function FromClient_NewWorldMap_RenderStateChange(state)
  ...
  elseif eWorldmapState.eWMS_FAMILY_CHARACTER == state then
    ...
    ToClient_WorldmapMonsterInfoSetShow(true)
    ToClient_worldmapBuildingManagerSetShow(false)
    ToClient_worldmapQuestManagerSetShow(false)
    ToClient_worldmapGuideLineSetShow(false)
    ToClient_worldmapDeliverySetShow(false)
...
function FromClient_SetTownMode(waypointKey)
  ...
  if true == _ContentsGroup_ForXBoxFinalCert then
    ...
    if _ContentsGroup_WorldMapMenuFilter == true then
      FGlobal_NodeMenu_SetExplorePointBG(false)
      isOpendedSearchMenu = FGlobal_GetShow_WorldmapSearchMenu()
      isOpendedContentsMenu = FGlobal_GetShow_WorldmapContentsMenu()
      FGlobal_SetShow_WorldmapSearchMenu(false)
      FGlobal_SetCheck_WorldmapContentsMenu(false)
...
function FromClient_resetTownMode()
  ...
  if true == _ContentsGroup_ForXBoxFinalCert then
    ...
    if _ContentsGroup_WorldMapMenuFilter == true then
      FGlobal_SetCheck_WorldmapSearchMenu(isOpendedSearchMenu)
      FGlobal_SetShow_WorldmapSearchMenu(isOpendedSearchMenu)
      FGlobal_SetCheck_WorldmapContentsMenu(isOpendedContentsMenu)
      WorldMapContentsListCheck()
    end
