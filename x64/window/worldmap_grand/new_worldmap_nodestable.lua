...
function nodeStableList:init()
  for slotidx = 0, self._config.slotCount - 1 do
    ...
    slot.raceHorseIcon = UI.createAndCopyBasePropertyControl(Panel_NodeStable, "List_iconRaceHorse", slot.button, "nodeStableList_Slot_RaceHorse_" .. slotidx)
    ...
    slot.raceHorseIcon:SetPosX(iconConfig.startGradeX + slot.grade:GetSizeX() - slot.raceHorseIcon:GetSizeX())
    slot.raceHorseIcon:SetPosY(iconConfig.startGradeY + slot.grade:GetSizeY())
...
function nodeStableList:update()
  ...
  for servantIdx = self._startSlotIndex, self._nodeServantCount - 1 do
    ...
    if nil ~= servantInfo then
      local isLinkedHorse = servantInfo:isLink() and (CppEnums.VehicleType.Type_Horse == servantInfo:getVehicleType() or servantInfo:getVehicleType() == CppEnums.VehicleType.Type_RaceHorse)
      if uiIdx <= self._config.slotCount - 1 then
        local slot = self._slots[uiIdx]
        slot.name:SetText(servantInfo:getName())
        slot.icon:ChangeTextureInfoNameAsync(servantInfo:getIconPath1())
        ...
        if (servantInfo:getVehicleType() == CppEnums.VehicleType.Type_Horse or servantInfo:getVehicleType() == CppEnums.VehicleType.Type_RaceHorse) and true == servantInfo:isDisplayGender() then
        ...
        if CppEnums.VehicleType.Type_Horse == servantInfo:getVehicleType() or servantInfo:getVehicleType() == CppEnums.VehicleType.Type_RaceHorse then
          slot.grade:SetShow(true)
          slot.grade:SetText("")
          local tierName = servantInfo:getDisplayTierName()
          if nil ~= tierName then
            slot.grade:SetText(tierName)
          end
          if 9 <= servantInfo:getTier() and isContentsStallionEnable then
            slot.stallionIcon:SetShow(false)
            slot.raceHorseIcon:SetShow(false)
          elseif isContentsStallionEnable then
            slot.stallionIcon:SetShow(true)
            slot.raceHorseIcon:SetShow(true)
            if true == servantInfo:isStallion() then
              slot.stallionIcon:SetMonoTone(false)
              slot.raceHorseIcon:SetMonoTone(false)
            else
              slot.stallionIcon:SetMonoTone(true)
              slot.raceHorseIcon:SetMonoTone(true)
            end
          end
