...
local _stateData = {
  ...
  [STATE.eWMS_FAMILY_CHARACTER] = {
    tooltip = PAGetString(Defines.StringSheet_GAME, "LUA_FAMILYCHARACTER_WORLDMAPTOOLTIP_TITLE")
  }
}
...
function WorldMapSideBar:open(sustainStates)
  ...
  self._viewToggles[STATE.eWMS_FAMILY_CHARACTER][VIEW_TOGGLE.eCheck_Node] = true
...
function Input_WorldMapSideBar_SetStateTo(state)
  ...
  elseif STATE.eWMS_FAMILY_CHARACTER == state then
    renderState = STATE.eWMS_EXPLORE_PLANT
    self._isBlackFog = true
    self._isGuildWarMode = false
  else
