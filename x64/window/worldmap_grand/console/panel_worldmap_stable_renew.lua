  ...
  for servantIdx = 0, nodeServantCount - 1 do
    ...
    if nil ~= servantInfo then
      ...
      if CppEnums.VehicleType.Type_Horse == servantInfo:getVehicleType() then
        ...
        self._horseInfoList[servantIdx]._tier = ""
        local tierName = servantInfo:getDisplayTierName()
        if nil ~= tierName then
          self._horseInfoList[servantIdx]._tier = tierName
        end
        if servantInfo:getTier() < 9 and ToClient_IsContentsGroupOpen("243") then
...
function PaGlobalFunc_WorldMap_Stable_List2EventControlCreate(list_content, key)
  ...
  static_Image:ChangeTextureInfoNameAsync(info._icon)
  ...
  static_Sex:ChangeTextureInfoNameAsync("renewal/ui_icon/console_icon_01.dds")
