local Window_WorldMap_RingMenuInfo = {
  ...
  _strConfig = {
    ...
    [8] = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_PALACEMANAGEMENT_TITLE")
  },
  _uiTypeConfig = {
    _node = 0,
    _quest = 1,
...
function Window_WorldMap_RingMenuInfo:RingMenuClear()
  ...
  for index = 0, self._config._count - 1 do
    if nil ~= self._ui._ringMenu[index] then
      ...
      if index < self._config._menuCount then
        self._currentConfig[index] = index
        self._ui._ringMenu[index]._button:SetShow(true)
        self._ui._ringMenu[index]._icon:SetShow(true)
      else
        self._ui._ringMenu[index]._button:SetShow(false)
        self._ui._ringMenu[index]._icon:SetShow(false)
      end
    end
  end
end
function Window_WorldMap_RingMenuInfo:FocusOn(uiType)
  ...
  self._ui._static_FocusKeyGuide:ChangeTextureInfoNameAsync("renewal/ui_icon/console_xboxkey_00.dds")
...
function PaGlobalFunc_WorldMap_RingMenu_RingMenuSelect(index)
  ...
  if state._enter == index then
    if PaGlobalFunc_WorldMap_FamilyCharacterList_GetShow() == true then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_FAMILYCHARACTER_ENTER_NODE_ERROR"))
      return
    end
    local nodeInfo = self._currentWorldNode:FromClient_getExplorationNodeInClient()
  ...
  elseif state._makeAndServent == index then
    self:WorkerManage()
  elseif state._palaceManagement == index then
    local nodeInfo = self._currentWorldNode:FromClient_getExplorationNodeInClient()
    local plantKey = nodeInfo:getPlantKey()
    PaGlobalFunc_WorldMap_TopMenu_SetCurrentNodeInfo(nodeInfo)
    PaGlobalFunc_WorldMap_RightMenu_SetCurrentNodeInfo(nodeInfo)
    ...
    PaGlobal_PalaceManagement_Map_Open()
  end
end
...
function PaGlobalFunc_FromClient_WorldMap_RingMenu_LClickedWorldMapNode(explorationNode)
  ...
  self:CheckAndSetForPalaceManagement()
end
function Window_WorldMap_RingMenuInfo:CheckAndSetForPalaceManagement()
  if Window_WorldMap_RingMenuInfo == nil then
    return
  end
  local palaceManageWayPointKey = Toclient_getPalaceManageRegionWaypointKey()
  if self._currentWayPointKey == palaceManageWayPointKey then
    self._currentConfig[self._config._craftManage] = self._config._palaceManagement
    self._ui._ringMenu[self._config._craftManage]._button:SetShow(false)
    self._ui._ringMenu[self._config._craftManage]._icon:SetShow(false)
    self._ui._ringMenu[self._config._palaceManagement]._button:SetShow(true)
    self._ui._ringMenu[self._config._palaceManagement]._icon:SetShow(true)
  end
end
...
function Window_WorldMap_RingMenuInfo:ConditionCheck(position)
  ...
  if self._currentConfig[self._config._craftManage] == self._config._craftManage and 0 == self._houseCount and self._config._craftManage == position then
    return false
  end
  return true
end
function Window_WorldMap_RingMenuInfo:UpdateRingMenu(position)
  ...
  if position == __eRingMenuPosition_Default then
    if __eRingMenuPosition_Default ~= self._currentRingMenuIndex then
      ...
      PaGlobalFunc_WorldMap_RingMenu_RingMenuSelect(self._currentConfig[self._currentRingMenuIndex])
    end
  else
    for index = 0, self._config._count - 1 do
      local ringMenuIndex = self._currentConfig[index]
      if nil ~= self._ui._ringMenu[ringMenuIndex] then
        self._ui._ringMenu[ringMenuIndex]._button:SetCheck(index == position)
      end
    end
    local textIndex = self._currentConfig[position]
    self._ui._staticText_Tooltip:SetShow(true)
    self._ui._staticText_Tooltip:SetText(self._strConfig[textIndex])
  end
