...
local Window_WorldMap_TopMenuInfo = {
  ...
  _strConfig = {
    ...
    [8] = PAGetString(Defines.StringSheet_GAME, "LUA_FAMILYCHARACTER_CONSOLE_TOP_MENU")
  },
  _stateFilterConfig = {
    [0] = {
      true,
...
function Window_WorldMap_TopMenuInfo:UpdateInfo(index)
  ...
  elseif eState.eWMS_PRODUCT_NODE == renderState then
    ...
    self._isGuildWarMode = false
    renderState = eState.eWMS_EXPLORE_PLANT
  elseif eState.eWMS_FAMILY_CHARACTER == renderState then
    self._isBlackFog = true
    self._isGuildWarMode = false
    renderState = eState.eWMS_EXPLORE_PLANT
  else
  ...
  if index == eState.eWMS_FAMILY_CHARACTER then
...
function PaGlobalFunc_FromClient_WorldMap_TopMenu_RenderStateChange(state)
  ...
  elseif eState.eWMS_LOCATION_INFO_HUMIDITY == state then
    ...
    ToClient_worldmapExceptionLifeKnowledgeSetShow(knowledgeShow, CppEnums.WaypointKeyUndefined)
    ToClient_WorldmapBookMarkInfoSetShow(true)
  elseif eState.eWMS_FAMILY_CHARACTER == state then
    ToClient_worldmapNodeMangerSetShow(true)
    ToClient_worldmapActorManagerFamilyCharacterIconSetShow(true)
    ToClient_worldmapBuildingManagerSetShow(false)
    ToClient_worldmapQuestManagerSetShow(false)
...
function Window_WorldMap_TopMenuInfo:InitControl()
  ...
  if _ContentsGroup_WorldMapFamilyCharacterManagement == true then
    for index = 0, self._config._menuCount - 2 do
      self._ui._menuButtonList[index]:SetSpanSize(self._ui._menuButtonList[index]:GetSpanSize().x - 30, self._ui._menuButtonList[index]:GetSpanSize().y)
    end
    self._ui._menuButtonList[8]:SetShow(true)
  else
    self._config._menuCount = self._config._menuCount - 1
    self._ui._menuButtonList[8]:SetShow(false)
  end
...
function PaGlobalFunc_WorldMap_TopMenu_HardCoreModeSet()
  if ToClient_HardCoreChannelWithContensOption() == false then
    return
  end
  for checkIndex, value in pairs(CppEnums.WorldMapCheckState) do
    local isShow = false
    if checkIndex == CppEnums.WorldMapCheckState.eCheck_Monster then
      isShow = true
    end
  ...
  PaGlobalFunc_UpdateHardCoreBossListUIForConsole()
end
function PaGlobalFunc_WorldMap_TopMenu_Open()
  ...
  if ToClient_HardCoreChannelWithContensOption() == true then
    PaGlobalFunc_WorldMap_TopMenu_HardCoreModeSet()
  end
  ...
  if worldmapTabNo == CppEnums.WorldMapState.eWMS_FAMILY_CHARACTER then
    ToClient_worldmapActorManagerFamilyCharacterIconSetShow(true)
    PaGlobalFunc_WorldMap_FamilyCharacterList_Open()
  end
end
...
function Window_WorldMap_TopMenuInfo:UpdateContributeText()
  ...
  local installedDescText = PAGetString(Defines.StringSheet_GAME, "LUA_USINGEXPLOREPOINT_TOTALPOINT_TOOLTIP") .. "\n"
  self._ui.txt_contributeDesc:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_contributeDesc:SetText(titleText .. descText .. installedDescText .. _contributeBubbleText)
