...
local nodeStableInfo = {
  ...
  _Info_RacseHorseIcon = UI.getChildControl(Panel_NodeStableInfo, "Info_raceHorse"),
...
function nodeStableInfo:update()
  ...
  self._Info_RacseHorseIcon:SetShow(false)
  ...
  if CppEnums.VehicleType.Type_Horse == servantInfo:getVehicleType() or CppEnums.VehicleType.Type_RaceHorse == servantInfo:getVehicleType() or CppEnums.VehicleType.Type_RidableBabyElephant == servantInfo:getVehicleType() then
    ...
    self._Info_Grade_Value:SetText("")
    local tierName = servantInfo:getDisplayTierName()
    if nil ~= tierName then
      self._Info_Grade_Value:SetText(tierName)
    end
    ...
    elseif isContentsStallionEnable then
      ...
      self._Info_RacseHorseIcon:SetShow(true)
      if true == servantInfo:isStallion() then
        self._Info_StallionIcon:SetMonoTone(false)
        self._Info_RacseHorseIcon:SetMonoTone(false)
      else
        self._Info_StallionIcon:SetMonoTone(true)
        self._Info_RacseHorseIcon:SetMonoTone(true)
      end
    end
    if CppEnums.VehicleType.Type_RaceHorse == servantInfo:getVehicleType() then
      self._Info_RacseHorseIcon:SetShow(true)
      self._Info_StallionIcon:SetShow(false)
    else
      self._Info_RacseHorseIcon:SetShow(false)
  ...
  for skill_idx, skillReal_Idx in pairs(skillTempWrapper) do
    ...
    if skill_idx > self._startSlotIndex then
      ...
      self._skillslot[control_Idx].icon:ChangeTextureInfoNameAsync("Icon/" .. skillWrapper:getIconPath())
  ...
  if not servantInfo:doHaveVehicleSkill() and servantInfo:getVehicleType() == CppEnums.VehicleType.Type_Carriage then
    ...
    for linkedidx = 0, maxlinkcount - 1 do
      ...
      if nil ~= sInfo then
        ...
        linkedHorse.icon:ChangeTextureInfoNameAsync(sInfo:getIconPath1())
