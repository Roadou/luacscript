...
function LargCraft_List:_updateSlot()
  ...
  defalut_Control._largCraft_List._Icon:ChangeTextureInfoNameAsync(icon)
    ...
    for Index = 1, waitingWorkerCount do
      ...
      if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
        ...
        if isCalculated == false then
          distance = ToClient_getCalculateMoveDistance(workerNo, LargCraft_List._position) / 100
...
function SubWork_Info_Update(isWorkOver)
  ...
  if gradeType > 0 and gradeType <= #UI.itemSlotConfig.borderTexture then
    defalut_Control._subWork_Info._Resource_Icon_Border:ChangeTextureInfoNameAsync(UI.itemSlotConfig.borderTexture[gradeType].texture)
  ...
  defalut_Control._subWork_Info._Resource_Icon:ChangeTextureInfoNameAsync(icon)
