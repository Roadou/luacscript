...
function Work_List:_updateSlot(_isFirst)
  ...
  for index = 1, self._slotCount do
    ...
    if nil ~= self._data_Table[rowIndex][collumIndex] then
      ...
      defalut_Control._work_List._Icon[index]:ChangeTextureInfoNameAsync(self._data_Table[rowIndex][collumIndex]._workIcon)
      ...
      if gradeType > 0 and gradeType <= #UI.itemSlotConfig.borderTexture then
        defalut_Control._work_List._Icon_Border[index]:ChangeTextureInfoNameAsync(UI.itemSlotConfig.borderTexture[gradeType].texture)
...
local function Worker_SortByRegionInfo(workIndex)
  ...
  for index = 1, waitingWorkerCount do
    ...
    if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
      ...
      local distance = Worker_List._data_Table[possibleWorkerIndex]._distance
  ...
  if esSSW:isSet() then
    ...
    for Index = 1, waitingWorkerCount do
      ...
      if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
        ...
        for i = 1, #distanceCache do
          if distanceCache[i]._regionName == regionNameRaw then
            distance = distanceCache[i]._distance
            isCalculated = true
            break
          end
        end
        if isCalculated == false then
          distance = ToClient_getCalculateMoveDistance(workerNo, Work_List._position) / 100
          distanceCache[#distanceCache + 1] = {_distance = distance, _regionName = regionNameRaw}
        end
        self._data_Table[_idx] = {
          _workerNo = workerNo,
...
function Work_Info_Update(isWorkOver)
  ...
  defalut_Control._work_Info._Result_Icon:ChangeTextureInfoNameAsync(icon)
  ...
  for idx = 1, resourceCount do
    ...
    if gradeType > 0 and gradeType <= #UI.itemSlotConfig.borderTexture then
      defalut_Control._work_Info._Resource_Icon_Boder[idx]:ChangeTextureInfoNameAsync(UI.itemSlotConfig.borderTexture[gradeType].texture)
    ...
    defalut_Control._work_Info._Resource_Icon[idx]:ChangeTextureInfoNameAsync(itemIcon)
    ...
    if enchantLevel > 0 then
      ...
      if enchantLevel > 0 and enchantLevel < 26 then
        printenchantLevel = HighRomaEnchantLevel_ReplaceString(enchantLevel)
      end
...
function Work_Estimated_Update(isWorkOver)
  ...
  if nil ~= Worker_List._data_Table[workerIndex] then
    distance = Worker_List._data_Table[workerIndex]._distance
...
function Inherit_List:_updateSlot()
  ...
  for idx = 1, self._Item_MaxCount do
    ...
    if nil ~= _data[index] then
      ...
      defalut_Control._select_Inherit._Icon[idx]:ChangeTextureInfoNameAsync(icon)
      if enchantLevel > 0 then
        local printenchantLevel = ""
        if enchantLevel > 0 and enchantLevel < 26 then
          printenchantLevel = HighRomaEnchantLevel_ReplaceString(enchantLevel)
      ...
      if gradeType > 0 and gradeType <= #UI.itemSlotConfig.borderTexture then
        defalut_Control._select_Inherit._Icon_Border[idx]:ChangeTextureInfoNameAsync(UI.itemSlotConfig.borderTexture[gradeType].texture)
