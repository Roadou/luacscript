...
function DyeingRegister:initialize()
  ...
  local stc_slotBg = UI.getChildControl(self._ui.stc_paletteGroup, "Static_SlotBGTemplate")
  self._ui.stc_slotBG = {}
  self._ui.slot_dyes = {}
  local slotConfig = {
    createIcon = true,
  ...
  for ii = 1, slotCount do
    ...
    slot.count = UI.createAndCopyBasePropertyControl(stc_slotBg, "StaticText_Count", self._ui.stc_slotBG[ii], "Static_Count_" .. ii)
  ...
  for ii = 1, #self._ui.slot_dyes do
    if nil ~= self._dyeSlotNo[ii + self._scrollAmount] then
      ...
      if nil ~= itemWrapper then
        ...
        self._ui.slot_dyes[ii].count:SetText(tostring(itemWrapper:getCount()))
      end
...
function PaGlobalFunc_DyeingRegister_Regist(index)
  ...
  local function confirmMessageBox(count_s64)
    if nil == count_s64 then
      return
    end
    ...
    local messageBoxMemo = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_PALETTE_SURE_THIS_AMPLUE_COUNT", "itemName", ampuleName, "itemCount", tostring(count_s64))
  ...
  if count_s64 > toInt64(0, 1) then
    Panel_NumberPad_Show(true, count_s64, 0, confirmMessageBox)
  else
    confirmMessageBox(count_s64)
  end
end
