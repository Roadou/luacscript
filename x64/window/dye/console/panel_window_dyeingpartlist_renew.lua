...
function DyeingPartList:update()
  ...
  for ii = 1, equipSlotCount do
    ...
    if nil ~= itemWrapper then
      ...
      if nil ~= targetData.pictogramUV then
        ...
        pictogram:ChangeTextureInfoNameAsync("renewal/ui_icon/console_icon_equip.dds")
        local x1, y1, x2, y2
        if classType == __eClassType_ShyWaman and 2 == ii then
          pictogram:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Dye/Dye_New_00.dds")
  ...
  if self._currentTargetIndex == TARGET_INDEX.CHARACTER then
    self._ui.stc_listBG:SetSize(self._ui.stc_listBG:GetSizeX(), _LISTBG_ORIGINAL_SIZE + 50)
    self._ui.txt_keyGuideA:SetPosY(lastSlot:GetParentPosY() + 134)
    self._ui._stc_checkButtonBG:SetShow(true)
    self._ui._stc_checkButtonBG:ComputePos()
    for idx = 0, self._maxToggleCount - 1 do
...
function PaGlobalFunc_DyeingMain_ToggleOption(clickedType)
  ...
  if clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.Underwear then
    ...
    if selfPlayer:get():isWearingBack() == true then
      ...
      DyeingPartList._ui._chk_buttons[clickedType]:SetCheck(false)
      return
    end
