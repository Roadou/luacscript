...
function HandleEventLUp_Extraction_Costume_All_ListControlCreate(control, key)
  ...
  local maidIcon = UI.getChildControl(itemSlotBg, "Static_MaidOnlyIcon")
  ...
  itemIcon:ChangeTextureInfoNameAsync("icon/" .. itemIconPath)
  if itemWrapper ~= nil then
    ...
    if isCash == true then
      local isTaxFree = itemWrapper:get():isVested() == false and itemWrapper:isSealed() == true and requestIsRegisterItemForItemMarket(itemWrapper:get():getKey()) == true
      if isTaxFree == true then
        cashIcon:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/tax.dds")
      else
        cashIcon:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/inventory/cashicon.dds")
      end
      local x1, y1, x2, y2 = setTextureUV_Func(cashIcon, 1, 1, 28, 28)
      cashIcon:getBaseTexture():setUV(x1, y1, x2, y2)
      cashIcon:setRenderTexture(cashIcon:getBaseTexture())
    end
    if itemWrapper:isMaidEquipableItem() == true then
      maidIcon:SetShow(true)
      cashIcon:SetShow(false)
    else
      maidIcon:SetShow(false)
    end
  end
  if itemText:IsLimitText() == true or extractCount:IsLimitText() == true then
    ...
    btn_item:removeInputEvent("Mouse_On")
    btn_item:removeInputEvent("Mouse_Out")
  end
...
function HandleEventOn_Extraction_Costume_All_ShowToolTipForLimitText(key_32, isShow)
  ...
  if PaGlobal_Extraction_Costume_All._isValks == nil then
    TooltipSimple_Hide()
    return
  end
