...
function PaGlobal_Extraction_System_All:updateExtractionList()
  ...
  for slotNo = 0, invenMaxSize - 1 do
    ...
    if false == PaGlobal_Extraction_System_All:fillter_ForSacrificeTarget(sortSlotNo, itemWrapper, whereType) then
      ...
      PaGlobal_Extraction_System_All._itemInfo.name[PaGlobal_Extraction_System_All._systemCnt] = name
      PaGlobal_Extraction_System_All._itemInfo.iconPath[PaGlobal_Extraction_System_All._systemCnt] = itemSSW:getIconPath()
...
function PaGlobal_Extraction_System_All:getEnchantText(isEquipable, isCash, isAccessory, balksCnt, cronsCnt, enchantLev)
  local enchantText = ""
  if isEquipable == true and enchantLev > 0 and enchantLev < 26 then
    enchantText = HighRomaEnchantLevel_ReplaceString(enchantLev)
  else
    enchantText = ""
  end
  if true == isAccessory then
    enchantText = HighRomaEnchantLevel_ReplaceString(enchantLev + 15)
  end
  if true == isCash then
    enchantText = ""
  end
