function InputMLUp_FairyRegister_RegisterName_All(str)
  ...
  if PaGlobal_FairyRegister_All._isRegister == true then
    PaGlobal_FairyRegister_All:registerName(str)
  else
    PaGlobal_FairyRegister_All:changeFairyName(str)
  end
end
...
function PaGlobal_FairyRegister_ChangeNameYes_All()
  if Panel_Window_FairyRegister_All == nil then
    return
  end
  PaGlobal_FairyRegister_All:changeFairyNameConfirm()
end
function FromClient_InputFairyName_All(fromWhereType, fromSlotNo)
  if nil == Panel_Window_FairyRegister_All then
    return
  end
  PaGlobal_FairyRegister_All:inputFairyName(fromWhereType, fromSlotNo)
end
function FromClient_FairyChangeName_Finish()
  PaGlobal_FairyInfo_All:Clear()
  PaGlobal_FairyInfo_All:update()
  Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_FAIRY_CHANGENAME_ACK"))
end
function FromClient_CheckFairyNameChangeItemExist(isExist)
  if isExist == true then
    PaGlobal_FairyRegister_All._ui.stc_PetIcon:ChangeTextureInfoNameAsync(PaGlobal_FairyInfo_All._iconPath)
    PaGlobal_FairyRegister_All._isRegister = false
    PaGlobal_FairyRegister_All:setDescText(PaGlobal_FairyRegister_All._isRegister)
    PaGlobal_FairyRegister_All._ui.stc_TitleText:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_CHANGENAME"))
    if Panel_Window_FairyRegister_All:GetShow() == true then
      PaGlobal_FairyRegister_All:prepareClose()
    else
      PaGlobal_FairyRegister_All:prepareOpen()
    end
  else
    PaGlobalFunc_EasyBuy_Open(131, nil, false)
  end
end
...
function HandleEventOnOut_FairyInfo_ChangeNameButton(isOn)
  if _ContentsGroup_UsePadSnapping == true then
    if isOn == true then
      Panel_FairyInfo_All:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
      PaGlobal_FairyInfo_All._ui_console.stc_KeyGuide_A:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_SELECT"))
  ...
  if isOn == true then
    PaGlobal_FairyInfo_SimpleTooltip_All(true, 16)
  else
    PaGlobal_FairyInfo_SimpleTooltip_All(false)
  end
end
...
function PaGlobal_FairyChangeName_Open_All()
  if Panel_Window_FairyRegister_All == nil then
    return
  end
  if Panel_Window_FairyRegister_All == nil then
  ...
  ToClient_checkNameChangeItemExist(__ePetType_Fairy)
end
...
function PaGlobal_FairyPanel_ALL_Close()
  ...
  if Panel_Window_FairyRegister_All:GetShow() == true then
    PaGlobal_FairyRegister_Close_All()
  end
end
