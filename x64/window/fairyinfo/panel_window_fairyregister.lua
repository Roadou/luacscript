...
PaGlobal_FairyRegister = {
  _ui = {
    ...
    keyGuideBG = UI.getChildControl(Panel_Window_FairyRegister, "Static_KeyGuide_Console")
  },
...
function PaGlobal_FairyRegister:init()
  ...
  if isGameTypeNA() or isGameTypeTaiwan() or isGameTypeTR() or isGameTypeTH() or isGameTypeASIA() or isGameTypeID() then
    self._ui.fairyNamingPolicyDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  if isGameTypeNA() or isGameTypeTaiwan() or isGameTypeASIA() then
  ...
  self._ui.keyGuideBG:SetShow(_ContentsGroup_UsePadSnapping)
end
...
function FromClient_InputFairyName(fromWhereType, fromSlotNo)
  ...
  self._ui.fairyIcon:ChangeTextureInfoNameAsync(petIconPath)
