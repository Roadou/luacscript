...
function PaGlobal_FairyRegister_All:initialize()
  ...
  self._ui.stc_TitleText = UI.getChildControl(self._ui.stc_TitleBG, "Static_Text_Title")
  ...
  self._ui_console.stc_BottomKeyGuide = UI.getChildControl(Panel_Window_FairyRegister_All, "Static_KeyGuide_Console")
  self._ui_console.stc_KeyGuide_A = UI.getChildControl(self._ui_console.stc_BottomKeyGuide, "StaticText_Select_Console")
  self._ui_console.stc_KeyGuide_B = UI.getChildControl(self._ui_console.stc_BottomKeyGuide, "StaticText_Exit_Console")
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuides, self._ui_console.stc_BottomKeyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT)
...
function PaGlobal_FairyRegister_All:SetUiSetting()
  ...
  self._ui_console.stc_BottomKeyGuide:SetPosY(self._ui.stc_BottomButtonBG:GetPosY() + self._ui.stc_BottomButtonBG:GetSizeY())
  if true == _ContentsGroup_RenewUI then
    local bgSizeY = self._ui.txt_NamingPolicyDesc:GetTextSizeY() + self._ui.txt_NamingPolicyDesc:GetSpanSize().y + 10
    self._ui.txt_NamingPolicyDesc:SetSize(self._ui.txt_NamingPolicyDesc:GetSizeX(), self._ui.txt_NamingPolicyDesc:GetTextSizeY())
    self._ui.stc_NamingPolicyBG:SetSize(self._ui.stc_NamingPolicyBG:GetSizeX(), bgSizeY)
  end
end
function PaGlobal_FairyRegister_All:registEventHandler()
  ...
  if _ContentsGroup_UsePadSnapping == false then
    self._ui_pc.btn_Close_pc:addInputEvent("Mouse_LUp", "PaGlobal_FairyRegister_Close_All()")
...
function PaGlobal_FairyRegister_All:inputFairyName(fromWhereType, fromSlotNo)
  if nil == Panel_Window_FairyRegister_All then
    return
  end
  ...
  PaGlobal_FairyRegister_All:setDescText(PaGlobal_FairyRegister_All._isRegister)
  self._ui.stc_TitleText:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_PETREGISTER_TITLE"))
...
function PaGlobal_FairyRegister_All:prepareOpen()
  if Panel_Window_FairyRegister_All == nil then
    return
  end
  if _ContentsGroup_UsePadSnapping == false then
    PaGlobal_FairyRegister_All:clearEdit()
  end
...
function PaGlobal_FairyRegister_All:validate()
  ...
  self._ui.stc_TitleText:isValidate()
  ...
  self._ui_console.stc_BottomKeyGuide:isValidate()
...
function PaGlobal_FairyRegister_All:changeFairyName(str)
  if Panel_Window_FairyRegister_All == nil then
    return
  end
  if str ~= nil then
  ...
  ClearFocusEdit()
  local fairyName = PaGlobal_FairyRegister_All._ui.edit_Name:GetEditText()
  local _title = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_CHANGENAME")
  local _contenet = PAGetStringParam1(Defines.StringSheet_RESOURCE, "PANEL_CHANGEFAIRYNAME_CONTINUE", "name", fairyName)
  local messageBoxData = {
    title = _title,
    content = _contenet,
    functionYes = PaGlobal_FairyRegister_ChangeNameYes_All,
    functionNo = MessageBox_Empty_function,
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  }
  MessageBox.showMessageBox(messageBoxData)
end
function PaGlobal_FairyRegister_All:changeFairyNameConfirm()
  if Panel_Window_FairyRegister_All == nil then
    return
  end
  local fairyName = PaGlobal_FairyRegister_All._ui.edit_Name:GetEditText()
  self._fairyNo = PaGlobal_FairyInfo_GetFairyNo_All()
  ToClient_requestChangePetName(self._fairyNo, fairyName, __ePetType_Fairy)
  self:prepareClose()
end
function PaGlobal_FairyRegister_All:clearEdit()
  if Panel_Window_FairyRegister_All == nil then
    return
  end
...
function PaGlobal_FairyRegister_All:setDescText(isRegister)
  local descPath
  if isRegister == true then
    descPath = "LUA_FAIRYREGISTER_DESC_PARAM"
  else
    descPath = "LUA_FAIRYCHANGENAME_DESC_PARAM"
  end
  local defaultDesc = PAGetStringParam2(Defines.StringSheet_GAME, descPath, "min", tostring(getGameServiceTypeNameMinLength()), "max", tostring(getGameServiceTypePetNameLength()))
  PaGlobal_FairyRegister_All._ui.txt_Desc:SetText(defaultDesc)
end
