...
function FGlobal_LevelupGuide_Open(isLevelUp)
  ...
  if nil ~= PaGlobalFunc_DailyStamp_GetShowPanel and true == PaGlobalFunc_DailyStamp_GetShowPanel() or nil ~= Panel_Window_DailyStamp_All and true == Panel_Window_DailyStamp_All:GetShow() and not isGameTypeNA() then
  ...
  local isNeverShow = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eLevelUpGuideNeverShowFlag)
  if isNeverShow == true then
    return
  end
...
function LevelupGuide_CheckForDay()
  ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eLevelUpGuideNeverShowFlag, true, CppEnums.VariableStorageType.eVariableStorageType_Character)
  Panel_LevelupGuide:SetShow(false, false)
end
