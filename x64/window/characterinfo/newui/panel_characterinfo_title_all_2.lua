...
function FromClient_CharInfoTitle_List2Update(content, key)
  ...
  local newIcon = UI.getChildControl(rdo_name, "Static_TitleName_NewIcon")
  ...
  if newIcon ~= nil then
    if titleWrapper:isAcquiredRecent() == true then
      newIcon:SetShow(true)
    else
      newIcon:SetShow(false)
    end
  end
  ToClient_insertNewTitleUI(titleWrapper:getKey():get(), content)
  ...
  if titleWrapper:isExistColor() == true then
    fontColor = titleWrapper:getColorValue()
  end
  if isAcquired == false then
    local r = math.floor(fontColor / 65536) % 256
    local g = math.floor(fontColor / 256) % 256
    local b = math.floor(fontColor) % 256
    r = 10 + math.floor(r * 0.5)
    g = 10 + math.floor(g * 0.5)
    b = 10 + math.floor(b * 0.5)
    fontColor = 4278190080 + r * 65536 + g * 256 + b
  end
...
function HandleEventLUp_CharInfoTitle_SelectFilter()
  ...
  PaGlobal_CharInfoTitle_All:setToggleText()
  ...
  if nil == isShow then
    ...
    if isShow == true then
      if true == Panel_CharacterInfo_All:IsUISubApp() then
        ...
        PaGlobal_CharInfoTitle_All._ui.stc_TitleTooltip:SetPosX(PaGlobal_CharInfoTitle_All._ui.list2_Title:GetPosX())
        PaGlobal_CharInfoTitle_All._ui.stc_TitleTooltip:SetPosY(PaGlobal_CharInfoTitle_All._ui.list2_Title:GetSizeY() / 2)
      end
    end
  end
end
...
function PaGlobal_CharinfoTitle_SetTitleUIForGlobalService(panel, titleControl)
  if panel == nil or titleControl == nil then
    return
  end
  ...
  if CppEnums.ServiceResourceType.eServiceResourceType_KR ~= serviceType and CppEnums.ServiceResourceType.eServiceResourceType_CN ~= serviceType and CppEnums.ServiceResourceType.eServiceResourceType_Dev ~= serviceType then
    panel._ui.txt_TitleCondi:SetShow(false)
    panel._ui.stc_TitleBg:SetSize(panel._ui.stc_TitleBg:GetSizeX(), titleControl:GetSizeY())
    panel._ui.stc_TitleBg:SetSpanSize(panel._ui.stc_TitleBg:GetSpanSize().x, 0)
    panel._ui.stc_TitleBg:ComputePos()
    panel._ui.txt_TitleName:SetTextMode(__eTextMode_AutoWrap)
    panel._ui.txt_TitleName:SetSize(panel._ui.txt_TitleName:GetSizeX(), panel._ui.txt_TitleName:GetSizeY() * 2)
    panel._ui.txt_TitleName:SetSpanSize(panel._ui.txt_TitleName:GetSpanSize().x, 40)
    panel._ui.txt_TitleName:ComputePos()
    panel._ui.btn_Apply:SetSpanSize(panel._ui.btn_Apply:GetSpanSize().x, panel._ui.btn_Apply:GetSpanSize().y + 15)
    panel._ui.txt_Desc_ForGlobal:SetSize(panel._ui.txt_Desc_ForGlobal:GetSizeX(), panel._ui.txt_Desc_ForGlobal:GetSizeY() + 30)
    panel._ui.txt_Desc_ForGlobal:SetSpanSize(panel._ui.txt_Desc_ForGlobal:GetSpanSize().x, panel._ui.txt_Desc_ForGlobal:GetSpanSize().y - 20)
    panel._ui.txt_Desc_ForGlobal:ComputePos()
  end
end
