function PaGlobal_CharInfoChallenge_All:initialize()
  ...
  self._ui.stc_solareBanner:SetShow(_ContentsGroup_SolareRankSeason)
...
function PaGlobal_CharInfoChallenge_All:tabMenuSetting()
  ...
  if isGameTypeNA() or isGameTypeTaiwan() or isGameTypeSA() or _ContentsOption_CH_GameType or isGameTypeTR() or isGameTypeTH() or isGameTypeASIA() or isGameTypeID() or self._isConsole then
    countrySet = false
  else
    countrySet = true
  end
  ...
  for idx = 0, self._MAXBGCOUNT - 1 do
    do
      ...
      for mainIdx = 0, self._MAXBASESLOT - 1 do
        ...
        if false == self._isConsole then
          baseUI._bg:ChangeOnTextureInfoNameAsync("combine/frame/combine_frame_00.dds")
