function PaGlobal_CharInfoTitle_All:initialize()
  ...
  self._ui.stc_TitleExpirationDate = UI.getChildControl(stc_Title, "Static_Time_Date")
  self._ui.txt_ExpirationDate = UI.getChildControl(self._ui.stc_TitleExpirationDate, "StaticText_Time")
  self._ui.stc_ExpirationTimeIcon = UI.getChildControl(self._ui.stc_TitleExpirationDate, "Static_TimeIcon")
  ...
  self._ui.stc_Tab_World_NewIcon = UI.getChildControl(self._ui.rdo_Tab_World, "Static_TitleCatergory_NewIcon")
  self._ui.rdo_Tab_Combat = UI.getChildControl(stc_TitleTabBg, "RadioButton_Top_Combat")
  self._ui.stc_Tab_Combat_NewIcon = UI.getChildControl(self._ui.rdo_Tab_Combat, "Static_TitleCatergory_NewIcon")
  self._ui.rdo_Tab_Life = UI.getChildControl(stc_TitleTabBg, "RadioButton_Top_Product")
  self._ui.stc_Tab_Life_NewIcon = UI.getChildControl(self._ui.rdo_Tab_Life, "Static_TitleCatergory_NewIcon")
  self._ui.rdo_Tab_Fish = UI.getChildControl(stc_TitleTabBg, "RadioButton_Top_Fish")
  self._ui.stc_Tab_Fish_NewIcon = UI.getChildControl(self._ui.rdo_Tab_Fish, "Static_TitleCatergory_NewIcon")
  ...
  self._ui.stc_FilterText = UI.getChildControl(self._ui.stc_FilterBg, "StaticText_TitleFilter_ConsoleUI")
  ...
  PaGlobal_CharinfoTitle_SetTitleUIForGlobalService(self, stc_Title)
  ...
  self._ui.stc_FilterText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_TITLE_FILTER_ALL"))
...
function PaGlobal_CharInfoTitle_All:update()
  ...
  self._ui.stc_Tab_World_NewIcon:SetShow(ToClient_CheckAcquireRecentTitleInCatergory(self._eTITLETYPE._WORLD))
  self._ui.stc_Tab_Combat_NewIcon:SetShow(ToClient_CheckAcquireRecentTitleInCatergory(self._eTITLETYPE._COMBAT))
  self._ui.stc_Tab_Life_NewIcon:SetShow(ToClient_CheckAcquireRecentTitleInCatergory(self._eTITLETYPE._LIFE))
  self._ui.stc_Tab_Fish_NewIcon:SetShow(ToClient_CheckAcquireRecentTitleInCatergory(self._eTITLETYPE._FISH))
  local lastCount = self._curCategoryCount
  if lastCount < 1 then
    return
  end
  if false == self._isFiltered then
    local titleList = Array.new()
    ...
    for titleIndex = 0, lastCount - 1 do
      ...
      if titleWrapper ~= nil then
        local isAcquiredRecent = titleWrapper:isAcquiredRecent()
        if isAcquiredRecent == true then
          self._ui.list2_Title:getElementManager():pushKey(toInt64(0, titleIndex))
        else
          titleList:push_back(titleIndex)
        end
      end
    end
    for ii = 1, titleList:length() do
      self._ui.list2_Title:getElementManager():pushKey(toInt64(0, titleList[ii]))
    end
  else
...
function PaGlobal_CharInfoTitle_All:setToggleText()
  self._ui.stc_FilterText:SetText(self._ui.stc_FilterBg:GetTextFromSelectItem())
  UI.setLimitTextToReferenceAndAddTooltip(self._ui.stc_FilterBg, self._ui.stc_FilterBg:GetTextFromSelectItem(), "", self._ui.stc_FilterText)
end
...
function PaGlobal_CharInfoTitle_All:updateFilteredList()
  ...
  for index = 0, listCount - 1 do
    ...
    local key32 = Int64toInt32(titleIdx)
    local titleWrapper = ToClient_GetTitleStaticStatusWrapper(key32)
    if titleWrapper ~= nil then
      local isAcquiredRecent = titleWrapper:isAcquiredRecent()
      if isAcquiredRecent == true then
        self._ui.list2_Title:getElementManager():pushKey(toInt64(0, key32))
      else
        titleList:push_back(key32)
      end
    end
  end
  for ii = 1, titleList:length() do
...
function PaGlobal_CharInfoTitle_All:setAppliedTitleButton()
  ...
  if true == self._selfPlayer:checkToTitleKey() then
    ...
    self._ui.txt_TitleName:SetFontColor(4294294074)
...
function PaGlobal_CharInfoTitle_All:setTitleText(wrapper)
  ...
  local fontColor = __eOverHeadUI_TitleDefaultColor
  if wrapper:isExistColor() == true then
    fontColor = wrapper:getColorValue()
  end
  ...
  local expireDate = wrapper:getExpirationDate()
  local leftTime = wrapper:getExpirationDate() - getServerUtc64()
  if expireDate > Defines.s64_const.s64_0 and leftTime >= Defines.s64_const.s64_0 then
    self._ui.stc_TitleExpirationDate:SetShow(true)
    self._ui.txt_ExpirationDate:SetText(convertStringFromDatetimeType2(leftTime))
    local posX = (self._ui.stc_TitleExpirationDate:GetSizeX() - (self._ui.txt_ExpirationDate:GetTextSizeX() + self._ui.stc_ExpirationTimeIcon:GetSizeX())) / 2
    self._ui.txt_ExpirationDate:SetPosX(posX + self._ui.stc_ExpirationTimeIcon:GetSizeX())
    self._ui.stc_ExpirationTimeIcon:SetPosX(posX)
  else
    self._ui.stc_TitleExpirationDate:SetShow(false)
  end
end
