...
function ItemMarket:Initialize()
  ...
  if isRussiaArea then
    self.btn_BuyMaid:addInputEvent("Mouse_LUp", "PaGlobalFunc_EasyBuy_Open( 67 )")
...
function Itemmarket_CategoryUpdate(contents, key)
  ...
  if indexMap._isMain then
    categoryIcon:ChangeTextureInfoNameAsync("Renewal/UI_Icon/Console_Icon_ItemMarket_00.dds")
...
function Itemmarket_ListUpdate(contents, key)
  ...
  if true == self.isGrouplist then
    ...
    if nil ~= itemMarketSummaryInfo then
      ...
      local itemNameStr = self:SetNameAndEnchantLevel(enchantLevel, iess:getItemType(), iess:getName(), iess:getItemClassify(), iess:isSpecialEnchantItem(), iess:isSpecialAccessory())
...
function Itemmarket_SpecialListUpdate(contents, key)
  ...
  if true == self.isGrouplist then
    ...
    if nil ~= itemMarketSummaryInfo then
      ...
      elseif enchantLevel > 0 and CppEnums.ItemClassifyType.eItemClassify_Accessory == itemMarketSummaryInfo:getItemClassify() and (itemMarketSummaryInfo:isSpecialEnchantItem() == false or itemMarketSummaryInfo:isSpecialAccessory() == true) then
...
function FGlobal_ItemMarket_Open_ForWorldMap(territoryKeyRaw, escMenu)
  ...
  if isGameTypeKorea() or isGameTypeJapan() or isRussiaArea or isGameTypeNA() or isGameTypeTaiwan() or isGameTypeSA() or isGameTypeTR() then
