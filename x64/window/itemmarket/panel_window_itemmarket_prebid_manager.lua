...
function ItemMarketPreBid_Manager:UpdateSlot(dataIdx, uiIdx)
  ...
  if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and (itemSSW:isSpecialEnchantItem() == false or itemSSW:isSpecialAccessory() == true) then
    ui.level:SetText(HighRomaEnchantLevel_ReplaceString(enchantLevel + 15))
  elseif enchantLevel > 0 and enchantLevel < 26 then
    ui.level:SetText(HighRomaEnchantLevel_ReplaceString(enchantLevel))
  end
  ui.icon:ChangeTextureInfoNameAsync("Icon/" .. iconPath)
