function PaGlobal_Window_OceanSiegeResult_All:initialize()
  if self._initialize == true then
    return
  end
  self._personalDataList = {}
  self._ui._stc_topMainArea = UI.getChildControl(Panel_Window_OceanSiegeResult_All, "Static_TopMainArea")
  ...
  self._ui._stc_middleSubArea = UI.getChildControl(Panel_Window_OceanSiegeResult_All, "Static_MiddleSubArea")
  ...
  self._ui._stc_firstInfo = UI.getChildControl(Panel_Window_OceanSiegeResult_All, "Static_FirstInfo")
  ...
  self._ui._stc_secondInfo = UI.getChildControl(Panel_Window_OceanSiegeResult_All, "Static_SecondInfo")
  ...
  self._ui._stc_thirdInfo = UI.getChildControl(Panel_Window_OceanSiegeResult_All, "Static_ThirdInfo")
  ...
  self._ui._stc_fifthInfo = UI.getChildControl(Panel_Window_OceanSiegeResult_All, "Static_FifthInfo")
  ...
  SlotItem.new(slot.base, "SailorRestore_ItemSlot", ii, slot.bg, self.slotConfig)
...
function PaGlobal_Window_OceanSiegeResult_All:registEventHandler()
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
  self._ui._btn_close:addInputEvent("Mouse_LUp", "PaGlobalFunc_OceanSiegeResult_All_Close()")
  registerEvent("FromClient_ShowOceanSiegeResult", "PaGlobalFunc_OceanSiegeResult_All_ReqestNewestSiegeHistory()")
  registerEvent("FromClient_UpdateSiegeResult", "FromClient_UpdateOceanSiegeResult()")
end
function PaGlobal_Window_OceanSiegeResult_All:SetPosition()
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
...
function PaGlobal_Window_OceanSiegeResult_All:prepareOpen(isRecord, historyIndex)
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
  ...
  local result = self:update(isRecord, historyIndex)
  if result == true then
    self:open()
  end
  Panel_GuildMain_All:ignorePadSnapMoveToOtherPanelUpdate(true)
end
function PaGlobal_Window_OceanSiegeResult_All:open()
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
  Panel_Window_OceanSiegeResult_All:SetShow(true)
end
function PaGlobal_Window_OceanSiegeResult_All:prepareClose()
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
...
function PaGlobal_Window_OceanSiegeResult_All:close()
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
  Panel_Window_OceanSiegeResult_All:SetShow(false)
end
function PaGlobal_Window_OceanSiegeResult_All:update(isRecord, historyIndex)
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
  ...
  local guildDestroyOceanTentCount = 0
  local guildDestroyGalleyShipCount = 0
  local guildDestroyEnemyShipCount = 0
  local guildDestroyShipCount = 0
  ...
  if isRecord == false then
    ...
    for personalIndex = 1, personalRecordCount do
      ...
      if volunteerMemberInfo ~= nil then
        ...
        local memberKillCount = self._personalDataList[personalIndex].galleyShip + self._personalDataList[personalIndex].bigShip + self._personalDataList[personalIndex].carrackShip + self._personalDataList[personalIndex].battleShip + self._personalDataList[personalIndex].normalShip
        self._personalDataList[personalIndex].member = memberKillCount
        guildDestroyOceanTentCount = guildDestroyOceanTentCount + self._personalDataList[personalIndex].command + self._personalDataList[personalIndex].tower
        guildDestroyGalleyShipCount = guildDestroyGalleyShipCount + self._personalDataList[personalIndex].galleyShip
        guildDestroyEnemyShipCount = guildDestroyEnemyShipCount + self._personalDataList[personalIndex].member
        guildDestroyShipCount = guildDestroyShipCount + self._personalDataList[personalIndex].destroyMyShip
      end
    end
  else
  ...
  if rewardItemEnchantKey == nil or rewardItemEnchantKey:getItemKey() == 0 then
    ...
    if rewardItemWarpper ~= nil then
      ...
      self._ui._rewardItemSlot.base.icon:addInputEvent("Mouse_On", "PaGlobalFunc_OceanSiegeResult_All_RewardItemTooltip(true," .. historyIndex .. ")")
      self._ui._rewardItemSlot.base.icon:addInputEvent("Mouse_Out", "PaGlobalFunc_OceanSiegeResult_All_RewardItemTooltip(false)")
  ...
  self._ui._txt_firstTitleValue:SetText(guildDestroyOceanTentCount)
  self._ui._txt_secondTitleValue:SetText(guildDestroyGalleyShipCount)
  self._ui._txt_thirdTitleValue:SetText(guildDestroyEnemyShipCount)
  self._ui._txt_fifthTitleValue:SetText(guildDestroyShipCount)
  table.sort(self._personalDataList, function(a, b)
    return a.command + a.tower > b.command + b.tower
  end)
  for i = 1, 5 do
    if self._personalDataList[i] ~= nil and self._personalDataList[i].command + self._personalDataList[i].tower ~= 0 then
      self._ui._stc_firstInfo_txt_Ranking[i]:SetText(self._personalDataList[i].name)
    else
      self._ui._stc_firstInfo_txt_Ranking[i]:SetText("")
    end
  end
  table.sort(self._personalDataList, function(a, b)
    return a.galleyShip > b.galleyShip
  end)
  for i = 1, 5 do
    if self._personalDataList[i] ~= nil and self._personalDataList[i].galleyShip ~= 0 then
      self._ui._stc_secondInfo_txt_Ranking[i]:SetText(self._personalDataList[i].name)
    else
  ...
  for i = 1, 5 do
    if self._personalDataList[i] ~= nil and self._personalDataList[i].member ~= 0 then
      self._ui._stc_thirdInfo_txt_Ranking[i]:SetText(self._personalDataList[i].name)
    else
      self._ui._stc_thirdInfo_txt_Ranking[i]:SetText("")
    end
  end
  table.sort(self._personalDataList, function(a, b)
    return a.destroyMyShip > b.destroyMyShip
  end)
  for i = 1, 5 do
    if self._personalDataList[i] ~= nil and self._personalDataList[i].destroyMyShip ~= 0 then
      self._ui._stc_fifthInfo_txt_Ranking[i]:SetText(self._personalDataList[i].name)
    else
...
function PaGlobal_Window_OceanSiegeResult_All:validate()
  if Panel_Window_OceanSiegeResult_All == nil then
    return
  end
