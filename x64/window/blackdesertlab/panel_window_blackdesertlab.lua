...
function PaGlobal_BlackDesertLab_Show()
  ...
  if isGameTypeTaiwan() == true or isGameTypeTR() == true or isGameTypeASIA() == true or isGameTypeJapan() == true then
    ToClient_OpenChargeWebPage("https://blackdesert.pearlabyss.com/GlobalLab")
    return
  else
    self._url = ToClient_getGTAccountURL() .. "/GlobalLab/" .. tostring(ToClient_GetWebCultureCode()) .. "/Game/InGame/GlobalLabDump" .. "?_userNo=" .. tostring(getSelfPlayer():get():getUserNo()) .. "&_certKey=" .. getSelfPlayer():get():getWebAuthenticKeyCryptString() .. "&_serviceType=" .. Defines.ServiceTypeToString[getGameServiceType()] .. "&_region=" .. regionType .. "&_langType=" .. Defines.LanguageTypeToString[ToClient_getResourceType()]
  end
  FGlobal_SetCandidate()
  self._ui._web:SetUrl(604, 500, self._url, false, true)
