...
function PaGlobal_GameOption_All._elements.AutoFindTarget:EventException(value)
  if value == false then
    return
  end
  local messageboxData = {
    title = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS"),
    content = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_GAMEOPTION_COMBATSIMPLE_DESC", "maxLevel", ToClient_GetAccessibleSeasonNewbieChannelMaxLevel(), "level", ToClient_GetAllCharacterTotalLevel()),
...
function PaGlobal_GameOption_All._elements.UIFontType:set(value)
  if 1 == value and true == isGameTypeNA() then
    value = 2
  end
...
function PaGlobal_GameOption_All._elements.IsPartyRefuse:set(value)
  setIsPartyRefuse(value)
end
function PaGlobal_GameOption_All._elements.IsGuildRefuse:set(value)
...
function PaGlobal_GameOption_All._elements.PetNameTagVisible:set(value)
  if value == nil then
    return
  end
  local realValue = PaGlobal_GameOption_All._PetNameTagVisibleValue[value]
  if realValue ~= nil then
    setPetNameTagVisible(realValue)
  end
end
function PaGlobal_GameOption_All._elements.FairyNameTagVisible:set(value)
  if value == nil then
    return
  end
  local realValue = PaGlobal_GameOption_All._FairyNameTagVisibleValue[value]
  if realValue ~= nil then
    setFairyNameTagVisible(realValue)
  end
end
function PaGlobal_GameOption_All._elements.MaidActorNameTagVisible:set(value)
  if value == nil then
    return
  end
  local realValue = PaGlobal_GameOption_All._MaidActorNameTagVisibleValue[value]
  if realValue ~= nil then
    setMaidActorNameTagVisible(realValue)
  end
end
...
function PaGlobal_GameOption_All._elements.WorldMapSaveCameraPos:set(value)
  setWorldMapSaveCameraPos(value)
end
function PaGlobal_GameOption_All._elements.WorldMapSaveCameraRot:set(value)
...
function PaGlobal_GameOption_All._elements.SelfEffectAlpha:set(value)
  if _ContentsGroup_SelfPlayerEffectAlpha == true then
    value = PaGlobal_GameOption_All:FromSliderValueToRealValue(value, PaGlobal_GameOption_All._elements.SelfEffectAlpha._sliderValueMin, PaGlobal_GameOption_All._elements.SelfEffectAlpha._sliderValueMax)
    value = value * 0.01
...
function PaGlobal_GameOption_All._elements.AlertGuildCongratulation:set(value)
  ToClient_SetMessageFilter(PaGlobal_GameOption_All.ALERT.GuildCongratulation, not value)
end
