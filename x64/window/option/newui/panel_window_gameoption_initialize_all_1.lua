  ...
  if "GraphicQuality" == detail or "Quality" == detail then
    ...
    if nil ~= self._elements.Tessellation._eventGroup then
      ...
      if nil ~= button and nil ~= title and nil ~= desc then
        if self.GRAPHIC.VeryVeryLow == curGraphicOption or self.GRAPHIC.VeryLow == curGraphicOption or self.GRAPHIC.Low == curGraphicOption or self.GRAPHIC.Medium == curGraphicOption or self.GRAPHIC.High == curGraphicOption or self.GRAPHIC.VeryHigh == curGraphicOption or self.GRAPHIC.VeryVeryHigh == curGraphicOption then
          button:SetIgnore(true)
          title:SetMonoTone(true)
          desc:SetMonoTone(true)
        elseif self.GRAPHIC.UltraHigh == curGraphicOption or self.GRAPHIC.UltraLow == curGraphicOption then
...
function PaGlobal_GameOption_All:SetTitleAndDescription(groupBg, option)
    ...
    if nil ~= option.uiInputType or nil ~= option.actionInputType then
      ...
      if true == option._use2option then
        ...
        if true == ToClinet_isGraphicUltraAvailable() then
          ...
          title2:SetText(PAGetString(Defines.StringSheet_RESOURCE, option._title2))
          local splitedText = PAGetString(Defines.StringSheet_RESOURCE, option._desc2)
          local stringList = string.split(PAGetString(Defines.StringSheet_RESOURCE, option._desc2), "\\n")
          if stringList[2] ~= nil then
            splitedText = stringList[2]
          end
          desc2:SetText(splitedText)
          if desc2:GetTextSizeX() > desc2:GetSizeX() then
            desc2:SetText(PAGetString(Defines.StringSheet_RESOURCE, option._desc2))
            UI.setLimitTextAndAddTooltip(desc2, desc2:GetText(), "")
          else
            desc2:SetText(PAGetString(Defines.StringSheet_RESOURCE, option._desc2))
          end
          local radioButtonSizeX = 230
          local radioButtonSizeY = 50
          local startPosY = desc2:GetPosY() + 45
          for ii = 8, option._radioButtonMapping._count - 1 do
            local xIndex = (ii - 8) % 3
            local yIndex = math.floor((ii - 8) / 3)
  ...
  elseif option._controlType == __ePAUIControl_Slider then
    if true == option._isSubSlider then
      ...
      local minText = UI.getChildControl(groupBg, "StaticText_Min")
      local curText = UI.getChildControl(groupBg, "StaticText_Current")
      local maxText = UI.getChildControl(groupBg, "StaticText_Max")
      local slider = UI.getChildControl(groupBg, "Slider_Template")
      local bottomAlignPan = 25
      if nil == option._title then
        groupBg:SetSize(groupBg:GetSizeX(), 60)
        areaBG:SetSize(areaBG:GetSizeX(), 60)
      elseif option._name == "EffectAlpha" then
        groupBg:SetSize(groupBg:GetSizeX(), 95)
      ...
      groupBg:ComputePosAllChild()
      if option._name == "EffectAlpha" then
        minText:SetPosY(areaBG:GetSizeY() - bottomAlignPan - minText:GetSizeY())
        curText:SetPosY(areaBG:GetSizeY() - bottomAlignPan - curText:GetSizeY())
        maxText:SetPosY(areaBG:GetSizeY() - bottomAlignPan - maxText:GetSizeY())
        slider:SetPosY(areaBG:GetSizeY() - bottomAlignPan - slider:GetSizeY() + 10)
      end
      line:SetShow(false)
      desc:SetShow(false)
    else
