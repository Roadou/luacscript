...
function PaGlobal_GameOption_All:setDefaultValue()
  ...
  if ToClinet_isGraphicUltraAvailable() == false then
    PaGlobal_GameOption_All._elements.GraphicOption._defaultValue = 2
  end
  if ToClient_GetOnStartCloudGame() == true then
    self._ui.stc_firstBG:SetShow(false)
    self._ui.stc_secondBG:SetShow(false)
    self._ui.stc_thirdBG:SetHorizonCenter()
  end
end
...
function PaGlobal_GameOption_All:setContentsOption()
  ...
  local availableUIFontType = isGameServiceTypeDev() or isGameTypeKorea() or isGameTypeNA()
  ...
  if ToClient_isConsole() then
    ...
    self._elements.AlertGuildCongratulation._isShow = false
...
function PaGlobal_GameOption_All_CategoryListTreeUpdate(contents, key)
  ...
  if indexMap._isMain then
    ...
    categoryIcon:ChangeTextureInfoNameAsync("combine/etc/combine_etc_setting.dds")
