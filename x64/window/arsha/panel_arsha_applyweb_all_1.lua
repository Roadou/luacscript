...
function PaGlobal_ArshaApplyWeb_All:getURL(token)
  local selfPlayerWrp = getSelfPlayer()
  if selfPlayerWrp == nil then
    return
  end
  local url = ToClient_getWebUrl(__eWebUrl_IngameWeb)
  local cryptStr = selfPlayerWrp:get():getWebAuthenticKeyCryptString()
  local userNo = selfPlayerWrp:get():getUserNo()
  local characterNo = selfPlayerWrp:getCharacterNo_64()
  url = url .. "/Arsha/ApplyArsha?userNo=" .. tostring(userNo) .. "&characterNo=" .. tostring(characterNo) .. "&certKey=" .. tostring(cryptStr)
  return url
end
