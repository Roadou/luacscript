...
function FromClient_Arsha_Hud_All_UpdateFightState(fightState, isShowingResultMessage)
  local self = PaGlobal_Arsha_Hud_All
  if self == nil then
    return
  end
  ...
  if CppEnums.CompetitionFightState.eCompetitionFightState_Fight == fightState then
    ...
    ToClient_RequestNakMessageRenewel(__eNakMessage_CompetitionStart)
  elseif CppEnums.CompetitionFightState.eCompetitionFightState_Done == fightState then
    ...
    if CppEnums.CompetitionMatchType.eCompetitionMatchMode_Round == state_match then
      ...
      self:showTargetScoreDot(targetScore, 0, 0)
    end
    if false == isShowingResultMessage then
      ToClient_RequestNakMessageRenewel(__eNakMessage_CompetitionStop)
    end
    self:changeMatchType()
