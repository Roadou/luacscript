function PaGlobal_Arsha_Hud_All:initialize()
  if true == self._initialize or nil == Panel_Widget_Arsha_Hud_All then
    return
  end
...
function PaGlobal_Arsha_Hud_All:update()
  ...
  self:setTeamInfo()
  self:resetTimerTextToLimitTime()
end
function PaGlobal_Arsha_Hud_All:resetTimerTextToLimitTime()
  if ToClient_IsMyselfInArena() == false then
    self:close()
    return
  end
...
function PaGlobal_Arsha_Hud_All:setScoreForReplay(teamNum, scoreValue)
  ...
  self:showTargetScoreDot(ToClient_GetTargetScore(), tonumber(self._ui.txt_leftPoint:GetText()), tonumber(self._ui.txt_rightPoint:GetText()))
end
...
function PaGlobal_Arsha_Hud_All:setTeamInfo()
  ...
  if CppEnums.CompetitionMatchType.eCompetitionMatchMode_Round == state_match then
    ...
    self:showTargetScoreDot(targetScore, teamA, teamB)
  elseif CppEnums.CompetitionMatchType.eCompetitionMatchMode_FreeForAll == state_match then
...
function PaGlobal_Arsha_Hud_All:setShowBuffForPersonal(isShow, teamNo, buffIndex, iconPath)
  ...
  self._ui.stc_buff_list[teamNo][buffIndex]:ChangeTextureInfoNameAsync(iconPath)
