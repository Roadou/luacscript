    ...
    if QuestTabType.QuestTabType_Progress == progressingActiveTab then
      if true == questGroupInfo:isGroupQuest() then
        ...
        if 1 == questCount then
          ...
          if false == uiQuestInfo:isSpecialType() and false == uiQuestInfo:isRegionMonsterKillType() and false == uiQuestInfo:isSupplyServantType() and false == uiQuestInfo:isSeasonPassType() and false == uiQuestInfo:isGrowthPassType() and uiQuestInfo:isOlviaAcademyPassType() == false then
  ...
  for questIdx = QuestWindow.config.startSlotIndex, self.config.listCount do
    ...
    if 0 < self.config.listCount then
      ...
      if false == useArray[questIdx].isQuest then
        ...
        expandIcon:ChangeTextureInfoNameAsync("new_ui_common_forlua/default/default_buttons_03.dds")
        expandIcon:ChangeTextureInfoNameAsync("renewal/pcremaster/remaster_common_00.dds")
