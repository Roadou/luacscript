function PaGlobal_UIModify:initialize()
  ...
  self._prevRemasterHpBarShowCheck = false
...
function PaGlobal_UIModify:confrimSetting_Sub()
  ...
  for idx = 1, self._panelCount do
    ...
    if idx >= self._panelID.Chat0 and idx <= self._panelID.Chat4 or idx == self._panelID.Party or idx == self._panelID.LargeParty then
    ...
    elseif panelControl[idx].isChange == false then
...
function PaGlobal_UIModify:applyPresetInfo(presetIndex, isShortcuts)
  ...
  if self._prevRemasterHpBarShowCheck == true then
    isSetRemasterUI = true
    ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eSwapRemasterUISetting, true, CppEnums.VariableStorageType.eVariableStorageType_User)
  end
...
function PaGlobal_UIModify:applyDefaultSet(isShortcuts, isReplay)
  ...
  if false == self._ui.chk_remsaterUI:IsCheck() then
    local isSetRemasterUI = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eSwapRemasterUISetting)
    self._prevRemasterHpBarShowCheck = isSetRemasterUI
...
function PaGlobal_UIModify:getUiSettingPanelInfo(panelIndex, posX, posY, isShow, chatWindowIndex, relativePosX, relativePosY)
  if self._panelControl[panelIndex].posFixed == false or panelIndex >= self._panelID.Chat0 and panelIndex <= self._panelID.Chat4 or panelIndex == self._panelID.Party or panelIndex == self._panelID.LargeParty then
...
function PaGlobal_UIModify:changePanelBGTexture(idx, state)
  ...
  control:ChangeTextureInfoNameAsync("combine/etc/combine_etc_uicontrolpanel.dds")
...
function PaGlobal_UIModify:updatePanelPool(isRemasterSwap)
  for idx = 1, self._panelCount do
    ...
    if nil ~= slot and nil ~= slot.control then
      ...
      elseif idx == self._panelID.ClassResource then
        ...
        if PaGlobal_ClassResource_CanUseResourceIcon ~= nil and PaGlobal_ClassResource_CanUseResourceIcon(classType) == true then
          ...
          elseif __eClassType_PWGE == classType then
            self._panelControl[self._panelID.ClassResource].name = PAGetString(Defines.StringSheet_GAME, "LUA_CLASSRESOURCE_DEADEYE_NAME")
          elseif __eClassType_PJKD == classType then
            self._panelControl[self._panelID.ClassResource].name = PAGetString(Defines.StringSheet_GAME, "LUA_CLASSRESOURCE_PJKD_NAME")
          end
      ...
      elseif idx == self._panelID.UIMain and ToClient_isConsole() == true then
        slot.control:SetShow(false)
      end
      if self._panelControl[idx].isShow then
        if self._panelControl[idx].posFixed then
