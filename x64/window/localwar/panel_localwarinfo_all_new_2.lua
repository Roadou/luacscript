...
function HandleEventOnOut_LocalWarInfo_AllClickMainTree(isShow, type)
  ...
  PaGlobal_LocalWarInfo_All_New._ui._stc_picTooltip:ChangeTextureInfoNameAsync(textureName)
...
function PaGlobal_LocalWarInfo_All_New_UpdateServerList(list_content, key)
  ...
  local roomInfoWrapper = ToClient_InstanceFieldRoomInfoWrapperWithType(__eInstanceContentsType_RedBattleField, treeData._mapInfo._index)
  ...
  elseif 2 == getCurrentState then
    ...
    if warTimeMinute >= limitTime then
      btn_join:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWARINFO_JOIN"))
      btn_join:SetIgnore(false)
    elseif _ContentsGroup_RedBattleFieldMatching == true then
      btn_join:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWARINFO_JOIN"))
      btn_join:SetIgnore(false)
    else
...
function FromClient_LocalWarInfo_All_New_UpdateInstanceRoom(needUIRefresh)
  ...
  if needUIRefresh == true then
    ...
    for idx = 1, PaGlobal_LocalWarInfo_All_New._treeKeyIndex do
      local treeData = PaGlobal_LocalWarInfo_All_New._treeMapTable[idx]
      if treeData ~= nil and treeData._isMain == false then
        local uiContent = PaGlobal_LocalWarInfo_All_New._ui._list2_serverList:GetContentByKey(toInt64(0, treeData._keyIndex))
        local roomInfoWrapper = ToClient_InstanceFieldRoomInfoWrapperWithType(__eInstanceContentsType_RedBattleField, treeData._mapInfo._index)
        if uiContent ~= nil and roomInfoWrapper ~= nil then
          ...
          if bg ~= nil then
            ...
            txt_joinMemberCount:SetText(tostring(getJoinMemberCount) .. " / " .. tostring(getMatchingUserCount))
          end
        end
      end
    end
