function PaGlobal_House_InstallationList_All:initialize()
  ...
  self._ui._stc_bigHousePartList = UI.getChildControl(self._ui._stc_backGround, "Static_BigHousingPartListBG")
  self._ui._stc_bigHouseLeftButton = UI.getChildControl(self._ui._stc_bigHousePartList, "Button_LeftArrow")
  self._ui._stc_bigHousePartTemplate = UI.getChildControl(self._ui._stc_bigHousePartList, "Static_Image_Temp")
  self._ui._stc_bigHouseRightButton = UI.getChildControl(self._ui._stc_bigHousePartList, "Button_RightArrow")
  self._ui._stc_bigHouseItemListBg = UI.getChildControl(self._ui._stc_backGround, "Static_BigHousingItemListBG")
  self._ui._stc_bigHouseItemTemplete = UI.getChildControl(self._ui._stc_bigHouseItemListBg, "Static_Item_Templete")
  self._ui._scroll_bigHouseItemList = UI.getChildControl(self._ui._stc_bigHouseItemListBg, "Scroll_HousingItemList")
  ...
  self._ui._txt_housingPoint = UI.getChildControl(stc_priceInfoBg, "StaticText_HousingPointTitle")
  ...
  self._ui._stc_bigHousePartList:SetShow(false)
  self._ui._stc_bigHouseItemListBg:SetShow(false)
  self._ui._stc_bigHouseLeftButton:SetIgnore(false)
  self._ui._stc_bigHouseRightButton:SetIgnore(false)
  self._mansionitemStartPos.x = self._ui._stc_bigHouseItemTemplete:GetPosX()
  self._mansionitemStartPos.y = self._ui._stc_bigHouseItemTemplete:GetPosY()
  self._startPartPos.x = self._ui._stc_bigHousePartTemplate:GetPosX()
  self._startPartPos.y = self._ui._stc_bigHousePartTemplate:GetPosY()
  ...
  self:createMansionItemSlot()
  self:createMansionPartsSlot()
...
function PaGlobal_House_InstallationList_All:registEventHandler()
  ...
  UIScroll.InputEvent(self._ui._scroll_bigHouseItemList, "HandleEventScroll_House_InstallationList_All_UpdateScroll_Mansion")
  UIScroll.InputEventByControl(self._ui._stc_bigHouseItemListBg, "HandleEventScroll_House_InstallationList_All_UpdateScroll_Mansion")
  self._ui._stc_bigHouseLeftButton:addInputEvent("Mouse_LUp", "HandleEventLUp_House_Part_Select_Left()")
  self._ui._stc_bigHouseRightButton:addInputEvent("Mouse_LUp", "HandleEventLUp_House_Part_Select_Right()")
...
function PaGlobal_House_InstallationList_All:prepareOpen()
  ...
  if false == self._isMansion then
    self._isMansion = ToClient_VisitHouseIsMansion()
    if self._isMansion == true then
      self._mansionCharacterKey = ToClient_VisitHouseHoldCharacterKey()
      self:setMansionPartsSlot()
    end
  end
  if true == ToClient_VisitHouseIsMansionLand() then
    self._ui._rdo_tabMenu[self._eTabMenu.BaseMaterial]:SetIgnore(true)
    self._ui._rdo_tabMenu[self._eTabMenu.BaseMaterial]:SetMonoTone(true, true)
  else
    self._ui._rdo_tabMenu[self._eTabMenu.BaseMaterial]:SetIgnore(false)
    self._ui._rdo_tabMenu[self._eTabMenu.BaseMaterial]:SetMonoTone(false, false)
  end
...
function PaGlobal_House_InstallationList_All:createMansionItemSlot()
  local maxSlot = self._mansionItemSlotRow * self._mansionItemSlotCol
  local gabSize = self._ui._stc_bigHouseItemTemplete:GetSizeX() + 5
  for slot_Idx = 0, maxSlot - 1 do
    local tempArray = {}
    local itemBg = UI.cloneControl(self._ui._stc_bigHouseItemTemplete, self._ui._stc_bigHouseItemListBg, "Static_House_MansionItemBG_" .. slot_Idx)
    local checkMark = UI.getChildControl(itemBg, "Static_CheckMarkTemplete")
    local settedPearlBg = UI.getChildControl(itemBg, "Static_SettedPearlItemBG")
    local hotKeyNumber = UI.getChildControl(itemBg, "StaticText_ShortCut")
    local col = math.floor(slot_Idx % self._mansionItemSlotCol)
    local row = math.floor(slot_Idx / self._mansionItemSlotCol)
    itemBg:SetPosX(self._mansionItemStartPos.x + col * gabSize)
    itemBg:SetPosY(self._mansionItemStartPos.y + row * gabSize)
    local slot = {}
    SlotItem.new(slot, "Static_ItemSlot_" .. slot_Idx, slot_Idx, itemBg, self._slotConfig)
    slot:createChild()
    slot.icon:SetShow(false)
    ...
    tempArray._hotKeyNumber:SetText(tostring(slot_Idx + 1))
    tempArray._hotKeyNumber:SetShow(false)
    UIScroll.InputEventByControl(tempArray._slotItem.icon, "HandleEventScroll_House_InstallationList_All_UpdateScroll_Mansion")
    UIScroll.InputEventByControl(tempArray._itemBg, "HandleEventScroll_House_InstallationList_All_UpdateScroll_Mansion")
    tempArray._itemBg:addInputEvent("Mouse_On", "HandleEventOn_House_InstallationList_All_FocusSlot(" .. slot_Idx .. ")")
    if 0 == row and col < self._mansionItemSlotCol then
      tempArray._itemBg:registerPadEvent(__eConsoleUIPadEvent_DpadUp, "HandleEventScroll_House_InstallationList_All_UpdateScroll_Mansion(true)")
    elseif self._mansionItemSlotRow - 1 == row then
      tempArray._itemBg:registerPadEvent(__eConsoleUIPadEvent_DpadDown, "HandleEventScroll_House_InstallationList_All_UpdateScroll_Mansion(false)")
    end
    tempArray._itemBg:addInputEvent("Mouse_On", "HandleEventOn_House_InstallationList_All_FocusSlot(" .. slot_Idx .. ")")
    if 0 == row and col < self._mansionItemSlotCol then
      tempArray._itemBg:registerPadEvent(__eConsoleUIPadEvent_DpadUp, "HandleEventScroll_House_InstallationList_All_UpdateScroll_Mansion(true)")
    ...
    self._mansionSlotUIPool[slot_Idx] = tempArray
  end
  self._ui._stc_bigHouseItemTemplete:SetShow(false)
end
function PaGlobal_House_InstallationList_All:createMansionPartsSlot()
  local maxViewCount = self._maxViewPartsCount
  local gabSize = self._ui._stc_bigHousePartTemplate:GetSizeX() + 20
  for part_Idx = 0, maxViewCount - 1 do
    local tempArray = {}
    tempArray._type = 0
    tempArray._partIndex = 0
    local itemBg = UI.cloneControl(self._ui._stc_bigHousePartTemplate, self._ui._stc_bigHousePartList, "Static_Image_Temp" .. part_Idx)
    itemBg:SetPosX(self._startPartPos.x + gabSize * part_Idx)
    tempArray._itemBg = itemBg
    tempArray._itemBg:SetShow(false)
    self._mansionPartSlotUIPool[part_Idx] = tempArray
  end
  self._ui._stc_bigHousePartTemplate:SetShow(false)
end
function PaGlobal_House_InstallationList_All:setMansionPartsSlot()
  if false == self._isMansion then
    return
  end
  self._maxMansionPartCount = ToClient_GetSubMeshCount()
  if self._maxMansionPartCount <= self._maxViewPartsCount then
    self._ui._stc_bigHouseLeftButton:SetIgnore(true)
    self._ui._stc_bigHouseRightButton:SetIgnore(true)
    self._ui._stc_bigHouseLeftButton:SetMonoTone(true)
    self._ui._stc_bigHouseRightButton:SetMonoTone(true)
  else
    self._ui._stc_bigHouseLeftButton:SetIgnore(false)
    self._ui._stc_bigHouseRightButton:SetIgnore(false)
    self._ui._stc_bigHouseLeftButton:SetMonoTone(false)
    self._ui._stc_bigHouseRightButton:SetMonoTone(false)
  end
  local startViewIndex = self._currentViewIndex
  local lastViewIndex = self._currentViewIndex + self._maxViewPartsCount - 1
  local maxViewCount = self._maxViewPartsCount
  for part_Idx = 0, maxViewCount - 1 do
    local partSlot = self._mansionPartSlotUIPool[part_Idx]
    partSlot._itemBg:addInputEvent("Mouse_LUp", "")
    local realPartIndex = part_Idx + startViewIndex
    if lastViewIndex >= realPartIndex and part_Idx < self._maxMansionPartCount then
      local iconPath = ToClient_getPartIconPath(realPartIndex)
      if nil ~= iconPath then
        local icon = UI.getChildControl(partSlot._itemBg, "Static_Image_Icon")
        if nil ~= icon then
          icon:ChangeTextureInfoNameAsync(iconPath)
          icon:getBaseTexture():setUV(0, 0, 1, 1)
        end
      end
      partSlot._itemBg:addInputEvent("Mouse_LUp", "HandleEventLUP_House_MansionInstallationPartSelect(" .. realPartIndex .. ")")
      partSlot._itemBg:SetShow(true)
    else
      partSlot._itemBg:SetShow(false)
    end
  end
end
function PaGlobal_House_InstallationList_All:clearMansionPartsSlot()
  self:clearGuideTexture()
  for part_Idx = 0, self._maxMansionPartCount - 1 do
    local tempArray = self._mansionPartSlotUIPool[part_Idx]
    tempArray._itemBg:SetShow(false)
    partSlot._itemBg:removeInputEvent("Mouse_LUp")
    self._mansionPartSlotUIPool[part_Idx] = nil
  end
end
function PaGlobal_House_InstallationList_All:clearGuideTexture()
  if false == self._isMansion then
    return
  end
  if -1 ~= self._prevSelectPart then
    ToClient_SetGuideTexture(PaGlobal_House_InstallationList_All._prevSelectPart, false)
    PaGlobal_House_InstallationList_All._prevSelectPart = -1
    PaGlobal_House_InstallationList_All._currentViewIndex = 0
  end
end
...
function PaGlobal_House_InstallationList_All:updateMansionItemSlot(startSlotIndex)
  local houseWrapper = housing_getHouseholdActor_CurrentPosition()
  if nil == houseWrapper then
    return
  end
  local maxSlot = self._mansionItemSlotRow * self._mansionItemSlotCol
  for slot_Idx = 0, maxSlot - 1 do
    local uiSlot = self._mansionSlotUIPool[slot_Idx]
    uiSlot._slotItem.icon:addInputEvent("Mouse_On", "")
    uiSlot._slotItem.icon:addInputEvent("Mouse_Out", "")
    uiSlot._slotItem.icon:addInputEvent("Mouse_LUp", "")
    uiSlot._slotItem:clearItem()
  ...
  for slot_Idx = 0, maxSlot - 1 do
    ...
    if true == data._isInstalled then
      ...
      uiSlot._slotItem.icon:addInputEvent("Mouse_On", "HandleEventOn_House_InstallationList_All_InstalledTooltip(" .. data._invenSlotNo .. ", " .. slot_Idx .. ")")
      uiSlot._slotItem.icon:addInputEvent("Mouse_LUp", "HandleEventLUp_House_InstallationList_All_InstallFloorWallMansion(" .. data._invenSlotNo .. ")")
      uiSlot._checkMark:SetShow(true)
      uiSlot._settedPearlBg:SetShow(false)
    elseif not data._isCashProduct then
      if true == self._isConsole then
      ...
      uiSlot._slotItem.icon:addInputEvent("Mouse_On", "HandleEventOn_House_InstallationList_All_NormalItemTooltip(" .. data._invenType .. ", " .. data._invenSlotNo .. ", " .. slot_Idx .. ")")
      uiSlot._slotItem.icon:addInputEvent("Mouse_LUp", "HandleEventLUp_House_InstallationList_All_InstallFloorWallMansion(" .. data._invenType .. ", " .. data._invenSlotNo .. ", false, " .. 0 .. ")")
      uiSlot._checkMark:SetShow(false)
      uiSlot._settedPearlBg:SetShow(false)
    else
      if true == self._isConsole then
      ...
      uiSlot._slotItem.icon:addInputEvent("Mouse_On", "HandleEventOn_House_InstallationList_All_CacheItemTooltip(" .. data._productNoRaw .. ", " .. slot_Idx .. ")")
      uiSlot._slotItem.icon:addInputEvent("Mouse_LUp", "HandleEventLUp_House_InstallationList_All_InstallFloorWallMansion(" .. data._invenType .. ", " .. data._invenSlotNo .. ", true, " .. data._productNoRaw .. ")")
      uiSlot._checkMark:SetShow(false)
      uiSlot._settedPearlBg:SetShow(true)
    end
    uiSlot._slotItem:setItemByStaticStatus(data:getItemStaticStatusWrapper(), 0)
...
function PaGlobal_House_InstallationList_All:initMansionItemSlotScroll()
  self._startItemSlotIndex = 0
  self._ui._scroll_bigHouseItemList:SetControlTop()
  self._ui._scroll_bigHouseItemList:SetControlPos(0)
  self:updateMansionItemSetData(true)
end
function PaGlobal_House_InstallationList_All:updateMansionItemSetData(initflag)
  ToClient_Furniture_Setdata(initflag)
  self._maxItemDataCount = ToClient_GetFurnitureListSize()
  self:updateMansionScrollSize()
end
function PaGlobal_House_InstallationList_All:updateMansionScrollSize()
  local slotCount = self._mansionItemSlotRow * self._mansionItemSlotCol
  UIScroll.SetButtonSize(self._ui._scroll_bigHouseItemList, slotCount, self._maxItemDataCount)
end
...
function PaGlobal_House_InstallationList_All:updateTabMenu(tabIndex)
  ...
  if true == self._isMansion then
    self._ui._stc_itemListBg:SetShow(tabIndex ~= self._eTabMenu.BaseMaterial)
    self._ui._stc_bigHousePartList:SetShow(tabIndex == self._eTabMenu.BaseMaterial)
    self._ui._stc_bigHouseItemListBg:SetShow(tabIndex == self._eTabMenu.BaseMaterial)
    if self._currentTabIndex ~= self._eTabMenu.BaseMaterial and tabIndex == self._eTabMenu.BaseMaterial then
      if true == ToClient_VisitHouseIsMansionLand() then
        return
      end
  ...
  self:initMansionItemSlotScroll()
  self:updateMansionItemSlot(self._startItemSlotIndex)
...
function PaGlobal_House_InstallationList_All:updateTabAll(tabIndex)
  ...
  if self._eTabMenu.All == tabIndex then
    if true == self._isMansion then
      ToClient_Housing_List_Except_Filter_Floor()
      ToClient_Housing_List_Except_Filter_Wall()
    end
  elseif self._eTabMenu.Goods == tabIndex then
    ToClient_Housing_List_Filter_Table()
    if true == self._isMansion then
  ...
  if nil ~= houseWrapper then
    ...
    if nil ~= houseSS then
      ...
      if nil ~= houseObjectSS then
        isFixed = true == houseObjectSS:isFixedHouse() or true == houseObjectSS:isInnRoom() or true == houseObjectSS:isMansionLand()
      end
    end
  end
...
function PaGlobal_House_InstallationList_All:baseMaterialConfirm(invenType, invenSlotNo, iscash, productNo)
  local selectIndex = PaGlobal_House_InstallationList_All._prevSelectPart
  local function doit()
