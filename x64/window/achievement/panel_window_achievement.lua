...
function PaGlobalFunc_Achievement_OpenBookShelfQuest(questGroup, questNo, journalListIndex)
  ...
  if _ContentsGroup_AchievementQuest == false then
    return
  end
  ...
  if journalType == __eQuestJournalType_Normal then
    ...
    Achievement:update()
    Achievement:updateBookMarkButton()
  elseif journalType == __eQuestJournalType_Renew then
    if journalListIndex == nil then
      return
    end
    PaGlobal_Journal_Book_All_Open(journalKey, chapterKey, journalListIndex)
  else
...
function FromClient_EventQuestUpdateNotify_Achievement(isAccept, questNoRaw)
  if true == _panel:GetShow() then
    ...
    if false == isAccept then
      ...
      ToClient_RequestNakMessageRenewelWithString(__eNakMessage_SpecialQuestReward, _reward_str)
      if nil == self._questInfo[self._currentIndex] then
        return
      end
      local currentQuestNo = self._questInfo[self._currentIndex].questNo
...
function Achievement:setCurrentPage()
  ...
  for index = 0, #stringList - 1 do
    ...
    self._ui._stc_questImage:ChangeTextureInfoNameAsync(specialQuestImagePath)
  ...
  if self._ui._txt_reward:IsLimitText() == true then
    self._ui._txt_reward:SetIgnore(true)
  end
  local posY = self._ui._stc_objectiveBG[0]:GetPosY()
  local sizeY = self._ui._stc_objectiveBG[0]:GetSizeY()
...
function Achievement:setOtherPage(isNext)
  ...
  for index = 0, #stringList2 - 1 do
    ...
    self._ui._stc_questImage2:ChangeTextureInfoNameAsync(specialQuestImagePath)
...
function PaGlobal_Achievement_ChangeRewardButton(isIgnore)
  ...
  if false == isIgnore then
    self._ui._btn_reward:ChangeTextureInfoNameAsync("renewal/PcRemaster/Remaster_Adventurebook_01.dds")
    ...
    self._ui._btn_reward2:ChangeTextureInfoNameAsync("renewal/PcRemaster/Remaster_Adventurebook_01.dds")
