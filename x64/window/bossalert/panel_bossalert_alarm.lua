...
function PaGlobal_BossAlert_NewAlarmShow(deltaTime)
  ...
  if self.updateTime < 30 then
    return
  end
  local currentMinute = ToClient_GetServerLocalTime():GetMinute()
  if self.lastUpdateMinute == currentMinute then
    return
  end
  self.lastUpdateMinute = currentMinute
  ...
  if false == _ContentsGroup_UseDataBossAlert then
    ...
    for ii = 1, count do
      ...
      if ii == 1 then
        self._lastMinute = bossAlarm.lastMinute
      else
        self._lastMinute = math.max(bossAlarm.lastMinute, self._lastMinute)
      end
