...
function PaGlobalFunc_GuildAllianceInvite_All_RegistGuildName(index)
  local self = PaGlobal_GuildAllianceInvite_All
  if self == nil then
    return
  end
  local guildName = self._ui.guildInfo[index].edit_guildName:GetEditText()
  if guildName == "" then
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    ToClient_CheckChinaProhibitedWord(guildName, __eSceneIdRegistGuildNameMakingAlliance)
...
function FromClient_CheckChinaProhibitedWord_RegistGuildNameMakingAlliance(isPass, filteredText)
  ...
  if isPass == true then
    self._ui.guildInfo[self._selectedGuildIndex].edit_guildName:SetText(filteredText)
  else
    self._ui.guildInfo[self._selectedGuildIndex].edit_guildName:SetText("")
  end
  self._selectedGuildIndex = 0
end
function PaGlobalFunc_GuildAllianceInvite_All_RegistAllianceName()
  local allianceName = PaGlobal_GuildAllianceInvite_All._ui.edit_searchGuild:GetEditText()
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    ToClient_CheckChinaProhibitedWord(allianceName, __eSceneIdRegistAllianceName)
    ClearFocusEdit()
    return
  end
  PaGlobal_GuildAllianceInvite_All._allianceName = allianceName
  ClearFocusEdit()
end
function FromClient_CheckChinaProhibitedWord_RegistAllianceName(isPass, filteredText)
  if _ContentsOption_CH_CheckProhibitedWord == false then
    return
  end
  local self = PaGlobal_GuildAllianceInvite_All
  ...
  if isPass == true then
    ...
    self._ui.edit_searchGuild:SetText("")
  end
end
