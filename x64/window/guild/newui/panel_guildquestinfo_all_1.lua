...
function PaGlobal_GuildQuestInfo_All:prepareOpen(questNo, historyIdx)
  if true == self._isProgressingQuest then
    self:updateProgressingQuest()
  else
    self:update(questNo, historyIdx)
  end
  self:open()
end
...
function PaGlobal_GuildQuestInfo_All:ComputePos()
  if Panel_Window_GuildWareHouseHistory == nil then
    return
  end
  local screenX = getScreenSizeX()
  local screenY = getScreenSizeY()
  local posX = Panel_Window_GuildWareHouseHistory:GetPosX()
  local posY = Panel_Window_GuildWareHouseHistory:GetPosY()
  local offsetX = Panel_Window_GuildWareHouseHistory:GetSizeX()
  local sizeX = Panel_GuildQuestInfo_All:GetSizeX()
  if screenX < posX + offsetX + sizeX then
    Panel_GuildQuestInfo_All:SetPosX(posX - sizeX)
  else
    Panel_GuildQuestInfo_All:SetPosX(posX + offsetX)
  end
  Panel_GuildQuestInfo_All:SetPosY(posY)
end
function PaGlobal_GuildQuestInfo_All:update(questNo, historyIdx)
  if Panel_GuildQuestInfo_All == nil then
    return
  end
  local questInfo, requireGuildRank, questTitle, questDesc, questLimit
  if questNo == nil then
    questInfo = PaGlobal_GuildQuest_GetQuestInfo_ByIndex(self._currentQuestKey)
    self._acceptQuestIndex = questInfo._questIndex
    if questInfo == nil then
      return
    end
    requireGuildRank = questInfo:getGuildQuestGrade()
    questTitle = questInfo:getTitle()
    questDesc = questInfo:getDesc()
    questLimit = questInfo:getLimitMinute()
  else
    requireGuildRank = ToClient_GetGuildQuestGradeByQuestNo(questNo)
    questTitle = ToClient_GetGuildQuestNameByQuestNo(questNo)
    questDesc = ToClient_GetGuildQuestDescByQuestNo(questNo)
    questLimit = Int64toInt32(ToClient_GetGuildQuestLimitMinuteByQuestNo(questNo))
  end
  ...
  self._ui.txt_QuestTitle:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_GUILD_QUEST_LIMIT", "currentGuildQuestName", questTitle, "requireGuildRankStr", requireGuildRankStr))
  ...
  self._ui.txt_QuestDesc:SetText(questDesc)
  local isShowPreOcc = false
  if false == isShowPreOcc then
    local questTime = PaGlobal_GuildQuest_GetQuestTimeFormatString(questLimit)
  ...
  for baseIndex = 0, self._defaultSlotCount - 1 do
    local baseReward
    if questNo == nil then
      baseReward = questInfo:getGuildQuestBaseRewardAt(baseIndex)
    else
      baseReward = ToClient_GetGuildQuestBaseRewardAtByQuestNo(questNo, baseIndex)
    end
  ...
  if questNo ~= nil then
    if historyIdx == nil then
      return
    end
    local wrapper = ToClient_getGuildQuestAcceptHistoryWrapper(historyIdx)
    ...
    self._ui.btn_function:SetShow(true)
    self._ui.btn_function:SetText(dateString)
    self._ui.btn_function:removeInputEvent("Mouse_LUp")
    self._ui.btn_function:SetIgnore(true)
    ...
    if self._isConsole == true then
      ...
      if ToClient_isProgressingGuildQuest() or PaGlobal_GuildQuest_GetCurrentType() == -1 then
      ...
      self._ui.btn_function:SetShow(true)
      self._ui.btn_function:SetIgnore(false)
    end
...
function PaGlobal_GuildQuestInfo_All:setReward(reward, idx)
  ...
  elseif __eRewardExpGrade == rewardType then
    uiSlot.icon:ChangeTextureInfoNameAsync("Icon/New_Icon/03_ETC/12_DoApplyDirectlyItem/ExpGrade.dds")
  ...
  elseif __eRewardSkillExpGrade == rewardType then
    uiSlot.icon:ChangeTextureInfoNameAsync("Icon/New_Icon/03_ETC/12_DoApplyDirectlyItem/SkillExpGrade.dds")
  ...
  elseif __eRewardKnowledge == rewardType then
    ...
    uiSlot.icon:ChangeTextureInfoNameAsync("Icon/New_Icon/03_ETC/12_DoApplyDirectlyItem/00000000_know_icon.dds")
...
function PaGlobal_GuildQuestInfo_All:alignKeyGuides()
  ...
  local isProgressing = ToClient_isProgressingGuildQuest() or PaGlobal_GuildQuest_GetCurrentType() == -1
