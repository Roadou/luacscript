...
function PaGlobal_GuildMemberList_All:CreateControl()
  ...
  local function createListInfo(pIndex)
    ...
    if false == self._isConsole then
      tempMember._memberBg:ChangeTextureInfoNameAsync("")
    end
...
function PaGlobal_GuildMemberList_All:getClassText(classType)
  return NewClassData.newClass_String[classType]._classType2String
end
...
function PaGlobal_GuildMemberList_All:updateListMemeber(guildMember, index, dataIdx)
  ...
  if true == self._isConsole then
    ...
    if ToClient_isXBox() == true then
      guildMemberControl._memberBg:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandlePadEvent_ShowXBoxProfile(" .. index .. ")")
    end
...
function GuildMemberCountSet()
  ...
  if _joinMemCount > 0 or allianceJoinMemberCount > 0 then
    if _ContentsGroup_RoseWar == true and ToClient_IsRoseWarServer() == true and ToClient_GetRoseWarState() ~= __eRoseWar_GuildApplying and ToClient_GetRoseWarState() ~= __eRoseWar_Stop and ToClient_GetRoseWarState() ~= __eRoseWar_Count then
      if isAlliance == true then
        self._ui.txt_guildFund:SetText(PAGetStringParam6(Defines.StringSheet_GAME, "LUA_GUILDMEMBERCOUNT_ROSEWAR_1", "onlineMember", _isOnlineMember, "memberCount", memberCount, "joinMemCount", _joinMemCount, "memberCount2", memberCount, "onlineMember1", allianceJoinMemberCount, "memberCount1", allianceMemberCount))
      else
        self._ui.txt_guildFund:SetText(PAGetStringParam4(Defines.StringSheet_GAME, "LUA_GUILDMEMBERCOUNT_ROSEWAR", "onlineMember", _isOnlineMember, "memberCount", memberCount, "joinMemCount", _joinMemCount, "memberCount2", memberCount))
      end
