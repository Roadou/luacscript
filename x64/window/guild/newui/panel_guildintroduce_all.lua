PaGlobal_GuildIntroduce_All = {
  ...
  _isMaster = false,
  _initialize = false,
  _chatType = CppEnums.ChatType.World
}
...
function HandleEventLUp_GuildIntroduce_Promote()
  ...
  PaGlobal_GuildMain_All:SetMsgDataAndShow(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS"), PAGetString(Defines.StringSheet_GAME, "LUA_GUILD_PROMOTE_BTN_MESSAGE_DESC"), PaGlobalFunc_GuildIntroduce_All_confirmPromote, MessageBox_Empty_function, nil, nil, nil, true)
end
...
function PaGlobalFunc_GuildIntroduce_All_confirmPromote()
  local guildWrapper = ToClient_GetMyGuildInfoWrapper()
  local guildIntroduce = guildWrapper:getGuildIntrodution()
  ToClient_SetLinkedGuildInfoByGuild()
  chatting_sendMessageNoMatterEmpty("", guildIntroduce, PaGlobal_GuildIntroduce_All._chatType)
  ToClient_ClearLinkedGuildList()
end
function PaGlobalFunc_GuildIntroduce_All_SetChatType(idx)
  if idx == 1 then
    PaGlobal_GuildIntroduce_All._chatType = CppEnums.ChatType.World
  elseif idx == 2 then
    PaGlobal_GuildIntroduce_All._chatType = CppEnums.ChatType.WorldWithItem
  end
end
