...
function GuildRecruitment:Update()
  ...
  local replaceClassType = function(classNo)
    local returnValue = ""
    return NewClassData.newClass_String[classNo]._classType2String
  end
  ...
  for index = 0, playerCount - 1 do
    ...
    if nil ~= unjoinPlayerWrapper and 5 < unjoinPlayerWrapper:getLevel() and realIndex < self.maxSlotCount then
      ...
      if __eClassType_Lhan == playerClass or __eClassType_RangerMan == playerClass or __eClassType_ShyWaman == playerClass or __eClassType_Guardian == playerClass then
        slot.charPic:ChangeTextureInfoNameAsync("New_UI_Common_ForLua/Window/Lobby/Lobby_ClassSelect_01.dds")
      else
        slot.charPic:ChangeTextureInfoNameAsync("New_UI_Common_ForLua/Window/Lobby/Lobby_ClassSelect_00.dds")
      end
