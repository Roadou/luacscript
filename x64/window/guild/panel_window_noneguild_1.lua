...
function PaGlobal_Guild_NoneGuild:EventHandler()
  ...
  if _ContentsGroup_GuildRevamp == true then
    self._ui._btn_applyJoin:addInputEvent("Mouse_On", "PaGlobal_Guild_NoneGuild_SimpleTooltip(true)")
    self._ui._btn_applyJoin:addInputEvent("Mouse_Out", "PaGlobal_Guild_NoneGuild_SimpleTooltip(false)")
  end
...
function PaGlobal_Guild_NoneGuild:update()
  ...
  if _ContentsGroup_GuildRevamp == true then
    ...
    if remainTime > 0 then
      self._ui._btn_applyJoin:SetFontColor(Defines.Color.C_FFFF4B4B)
    else
      self._ui._btn_applyJoin:SetFontColor(Defines.Color.C_FFF5BA3A)
    end
  end
  if _ContentsGroup_RenewUI == true then
...
function PaGlobal_Guild_NoneGuild:requestGuildApplicant(selectedCategory, index)
  if _ContentsGroup_GuildRevamp then
    local remainTime = Int64toInt32(ToClient_GetGuildJoinableRemainTime())
    if remainTime > 0 then
      return
    end
