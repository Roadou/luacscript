...
function AgreementVolunteer:init()
  ...
  if nil ~= optionWrapper then
    local isGuildRefuse = optionWrapper:getIsGuildRefuse()
    self._ui._chk_alwaysRefuse:SetCheck(isGuildRefuse)
...
function HandleClicked_AgreementVolunteer_RefuseOption()
  ...
  setIsGuildRefuse(self._ui._chk_alwaysRefuse:IsCheck())
end
...
function FromClient_AgreementVolunteer_Open(guildNo, guildName, masterNo, masterName, payment, bonus)
  ...
  if false == isSet then
    self._ui._stc_guildIcon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Default/Default_Buttons.dds")
