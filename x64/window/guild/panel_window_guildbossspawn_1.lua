...
function PaGlobal_GuildBossSpawn:updateState(key)
  ...
  local summonableCount = maxCount - currentCount
  if summonableCount < 0 then
    summonableCount = 0
  end
  info.txt_count:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_GUILD_BOSSSUMON_SUMMONCOUNT", "summonCount", tostring(summonableCount) .. "/" .. tostring(maxCount)))
...
function PaGlobal_GuildBossSpawn:selectLevel(key, level)
  ...
  info.mainBg:ChangeTextureInfoNameAsync(optionWrapper:getImagePath())
  ...
  if nil ~= itemSSW then
    info.rewardIcon:ChangeTextureInfoNameAsync("Icon/" .. itemSSW:getIconPath())
