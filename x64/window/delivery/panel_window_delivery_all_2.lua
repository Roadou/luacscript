...
function HandleEnvetLUp_DeliveryRequest_SelectToDeliverer()
  ...
  PaGlobal_Delivery_All:sendRequest_Update()
  PaGlobal_Warehouse_All_SetFunctor(Delivery_WarehouseFilter, nil)
  PaGlobal_Warehouse_All_UpdateSlotData()
end
...
function Delivery_WarehouseFilter(slotNo, itemWrapper, stackCount)
  local selfPlayer = getSelfPlayer()
  if selfPlayer == nil then
    return false
  end
  if PaGlobal_Delivery_All == nil then
    return false
  end
  if PaGlobal_Delivery_All._selectDeliverer == nil then
    return true
  end
  local carriageIndex = PaGlobal_Delivery_All._selectCarriageKey[PaGlobal_Delivery_All._selectDeliverer]
  if carriageIndex == nil then
    return true
  end
  local isForTradeCarriage = itemWrapper:getStaticStatus():isBarterItem() or itemWrapper:getStaticStatus():isForJustTrade()
  if carriageIndex >= __eDeliveryTradeCarriageIndex then
    return not isForTradeCarriage
  end
  return isForTradeCarriage
end
