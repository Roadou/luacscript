function PaGlobal_Delivery_All:initialize()
  ...
  if true == self._isConsole then
    self._ui.stc_itemSlotTemplete:ChangeOnTextureInfoNameAsync("renewal/frame/console_frame_00.dds")
...
function PaGlobal_Delivery_All:deliveryInformation_CreateDeliveryList(content, key)
  ...
  if 0 ~= leftTime then
    if delivererIndex >= __eDeliveryTradeCarriageIndex then
      carriageNumber:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_TRADE_DELIVERY_REMAINTIME", "number", tostring(delivererIndex - __eDeliveryTradeCarriageIndex + 1) .. "", "time", Util.Time.timeFormatting_Minute(leftTime)))
    else
  ...
  elseif delivererIndex >= __eDeliveryTradeCarriageIndex then
    carriageNumber:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_DELIVERYNEW_FIVENUMBER_CARRIAGE", "number", tostring(delivererIndex - __eDeliveryTradeCarriageIndex + 1)))
  else
...
function PaGlobal_Delivery_All:sendRequest_updateCarriage(destination)
  ...
  for ii = 0, size - 1 do
    ...
    if true == isView then
      local str
      if ii >= __eDeliveryTradeCarriageIndex then
        str = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_DELIVERYNEW_FIVENUMBER_CARRIAGE", "number", tostring(ii - __eDeliveryTradeCarriageIndex + 1))
      else
