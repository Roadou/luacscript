...
function DeliveryView_ListControlCreate_New(content, key)
  ...
  if 0 ~= leftTime then
    if delivererIndex >= __eDeliveryTradeCarriageIndex then
      remainTime:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_TRADE_DELIVERY_REMAINTIME", "number", tostring(delivererIndex - __eDeliveryTradeCarriageIndex + 1) .. "", "time", Util.Time.timeFormatting_Minute(leftTime)))
    else
  ...
  elseif delivererIndex >= __eDeliveryTradeCarriageIndex then
    remainTime:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_DELIVERYNEW_FIVENUMBER_CARRIAGE", "number", tostring(delivererIndex - __eDeliveryTradeCarriageIndex + 1)))
  else
