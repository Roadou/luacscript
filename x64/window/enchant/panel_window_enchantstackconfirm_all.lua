...
function PaGlobalFunc_EnchantStackConfirm_All_Open(isGroupEnchant)
  ...
  if PaGlobal_EnchantStackConfirm_All._isConsole == false then
    HandleEventLUp_EnchantStackConfirm_All_ClearEdit()
  end
end
...
function HandleEventLUp_EnchantStackConfirm_All_isValidateText()
  if _ContentsGroup_EnchantRemaster == true and Panel_Widget_Enchant_Main_All:GetShow() == true then
    HandleEventLUp_EnchantStackConfirm_All_isValidateTextForRemaster()
    return
  end
...
function HandleEventLUp_EnchantStackConfirm_All_isValidateTextForRemaster()
  local userInput = PaGlobal_EnchantStackConfirm_All._ui.edit_inputMyStack:GetEditText()
  if "" == userInput then
    return
  end
  local failCount = ToClient_GetEnchantStack()
  local valksCount = ToClient_GetValksStack()
  local bonusCount = ToClient_GetBonusStack()
  local myStack = tostring(failCount + valksCount + bonusCount)
  if userInput == myStack then
    PaGlobal_EnchantMain_All_CheckIsDuelItem()
    PaGlobalFunc_EnchantStackConfirm_All_Close()
  end
end
