...
local function insertCircleLineAndObject(zodiacSignStaticStatusWrapper)
  uiConst.info_Zodiac:ChangeTextureInfoNameAsync(zodiacSignStaticStatusWrapper:getZodiacImagePath())
...
local function settingUIMainCard(isFullupdate)
  ...
  for index = 0, childCardCount - 1 do
    ...
    if isFullupdate then
      ...
      ui.cardGroup[index]._cardIcon:ChangeTextureInfoNameAsync(card:getPicture())
    end
...
local function settingCardInfo()
  ...
  if false == card:hasCard() then
    uiConst.info_Picture:ChangeTextureInfoNameAsync(card:getPicture())
