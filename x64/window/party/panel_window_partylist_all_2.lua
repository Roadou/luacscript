...
function HandleEventLUp_PartyList_All_DoSearch(str)
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    local result = ToClient_CheckChinaProhibitedWord(msg, __eSceneIdSearchParty)
    if result == true then
      if PaGlobal_PartyList_All._isPadSnap == false then
        PaGlobal_PartyList_All._ui.btn_search:SetIgnore(true)
      elseif ToClient_IsDevelopment() then
        PaGlobal_PartyList_All._ui.edit_search:RegistReturnKeyEvent("")
      end
    end
    return
  end
...
function PadEventLTYUp_PartyList_All_ShowXboxProfile(index)
  if ToClient_isXBoxSeries() == false then
    return
  end
  if Panel_PartyList_All == nil then
    return
  end
  local partyWrapper = PaGlobal_PartyList_All._partySortTable[index]
  ...
  local leaderXuid = partyWrapper._getXuid
  ...
  if PaGlobalFunc_PartList_All_IsBlockedLeader(leaderXuid) == true then
    local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_DONOTHAVE_PRIVILEGE")
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_WARNING"),
      content = messageBoxMemo,
  ...
  ToClient_showXboxFriendProfile(leaderXuid)
end
...
function FromClient_PartyList_All_RequestPartyJoin(guestUserNo, characterName, level, classType, partyType, platformType, platformNickname, serverNo)
  if nil == Panel_PartyList_All then
    return
  end
  ...
  local function partyReject()
    ToClient_requestSystemMessageToTarget(guestUserNo, serverNo, "eErrNoSystemMessageRejectParty")
  end
...
function FromClient_CheckChinaProhibitedWord_SearchParty(isPass, filteredText)
...
function HandleEventOnOut_PartyList_All_OffDefToolTip(isShow, type)
  local control
  ...
  if type == "off" then
    control = PaGlobal_PartyList_All._ui_pc.stc_offenceTitle_pc
  elseif type == "def" then
    control = PaGlobal_PartyList_All._ui_pc.stc_defenceTitle_pc
  else
    return
  end
