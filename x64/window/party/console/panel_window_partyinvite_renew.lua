...
function Panel_Window_PartyInvite_info:childControl()
  ...
  if ToClient_isXBox() == true then
    self._ui.staticText_ShowXboxFriendInfo_btn = UI.getChildControl(self._ui.static_BottomBg, "StaticText_ShowXboxFriendInfo_ConsoleUI")
    self._ui.staticText_ShowXboxFriendInfo_btn:SetShow(true)
    local strGot = PAGetString(Defines.StringSheet_GAME, "FRIEND_TEXT_XBOX_PROFILE")
    if strGot ~= nil then
      self._ui.staticText_ShowXboxFriendInfo_btn:SetText(strGot)
    end
    table.insert(tempBtnGroup2, self._ui.staticText_ShowXboxFriendInfo_btn)
    table.insert(tempBtnGroup2, self._ui.staticText_Invite_btn)
    table.insert(tempBtnGroup2, self._ui.staticText_Close_btn)
  else
    table.insert(tempBtnGroup2, self._ui.staticText_Invite_btn)
    table.insert(tempBtnGroup2, self._ui.staticText_Close_btn)
  end
...
function Panel_Window_PartyInvite_info:setContent()
  ...
  self:setList()
  if ToClient_isXBox() == true then
    if self._value.currentTabIndex == self._enum.eTAB_XBOXFRIEND then
      self._ui.staticText_ShowXboxFriendInfo_btn:SetShow(true)
    else
      self._ui.staticText_ShowXboxFriendInfo_btn:SetShow(false)
    end
  end
end
function Panel_Window_PartyInvite_info:setIcon(control, classType)
  local classSymbolInfo = NewClassData.newClass_Texture_ID[classType]._symbolTextureTID
  if nil ~= classSymbolInfo then
    control:ChangeTextureInfoTextureIDAsync(classSymbolInfo, 0)
...
function PaGlobalFunc_PartyInvite_ListSetting(list_content, key)
  ...
  if self._value.currentTabIndex == self._enum.eTAB_GUILD then
    ...
    if _ContentsGroup_PartyMatching_Limit_OtherServerInvite == false and guildMemberData.isSameChannel == false then
      button_CharaterName:addInputEvent("Mouse_LUp", "PaGlobalFunc_PartyInvite_ShowCannotInviteOtherChannel()")
    elseif self._value._partyType == CppEnums.PartyType.ePartyType_Large then
  ...
  elseif self._value.currentTabIndex == self._enum.eTAB_FRIEND then
    ...
    if _ContentsGroup_PartyMatching_Limit_OtherServerInvite == false and friendMemberData.isSameChannel == false then
  ...
  elseif self._value.currentTabIndex == self._enum.eTAB_XBOXFRIEND then
    ...
    button_CharaterName:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "PaGlobalFunc_PartyInvite_ShowXBoxProfile(" .. xboxIndex .. ")")
...
function PaGlobalFunc_LargePartyInvite_ClickInvite(index)
  ...
  elseif self._value.currentTabIndex == self._enum.eTAB_XBOXFRIEND then
    PaGlobalFunc_PartyInvite_InvitePartyXboxFriend()
  end
  _AudioPostEvent_SystemUiForXBOX(50, 1)
end
...
function PaGlobalFunc_PartyInvite_ShowXBoxProfile(index)
  local self = Panel_Window_PartyInvite_info
  if self._value.currentTabIndex ~= self._enum.eTAB_XBOXFRIEND then
    return
  end
