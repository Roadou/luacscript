...
function PaGlobal_ChangeNickName_All:initailze()
  ...
  if true == _ContentsGroup_ChinaFontColor then
    self._ui.stc_Illust:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/lobby/lobby_accountnickname_bg.dds")
...
function HandleEventLUp_ChangeNickNameAll_ChangeNickName()
  ...
  local msgTitle = ""
  local bottomDesc = PAGetString(Defines.StringSheet_GAME, "LUA_MAKENAME_WARNING2")
  if changeNameType._characterName == _nameType then
  ...
  elseif changeNameType._quickChangeNickName == _nameType then
    ...
    bottomDesc = PAGetString(Defines.StringSheet_GAME, "LUA_MAKENAME_WARNING3")
  end
  PaGlobal_ChangeNickName_All_UnSetFocusOnEditBox()
  local msgData = {
...
function FromClient_ChangeNickname_All_Show(param)
  ...
  if changeNameType._characterName == param then
    ...
    if isGameTypeKorea() or isGameTypeTaiwan() or isGameTypeTH() or isGameTypeASIA() or isGameTypeRussia() or isGameTypeID() or isGameTypeTR() or isGameTypeJapan() or isGameTypeNA() then
