...
function FromClient_ShowInventoryBag(bagType, bagSize, fromWhereType, fromSlotNo)
  ...
  if bagType == __eInventoryBagType_Cash then
  ...
  elseif bagType == __eInventoryBagType_Seed then
    bagType = CppEnums.ItemWhereType.eInventory
    _title = PAGetString(Defines.StringSheet_GAME, "LUA_SEED_BAG")
    self.descBg:SetShow(false)
    self.desc:SetShow(false)
  elseif bagType == __eInventoryBagType_CashBig then
    bagType = CppEnums.ItemWhereType.eCashInventory
    _title = PAGetString(Defines.StringSheet_GAME, "LUA_CLOTHINVENTORY_PEARLTITLE")
    self.descBg:SetShow(true)
    self.desc:SetShow(true)
  ...
  local useChangeAllButton = self.inventoryBagType == __eInventoryBagType_Equipment or self.inventoryBagType == __eInventoryBagType_Cash
...
function ClothInven_Filter(slotNo, itemWrapper, count, inventoryType)
  ...
  if __eInventoryBagType_Misc == self.inventoryBagType and false == itemSSW:isUserVested() and true == itemWrapper:get():isVested() then
    return true
  end
  if not ToClient_CheckToPushToInventoryBag(self.fromWhereType, self.fromSlotNo, self.fromWhereType, slotNo) then
    return true
  end
  if self.inventoryBagType == __eInventoryBagType_Misc or self.inventoryBagType == __eInventoryBagType_MiscForCash then
    return false
  end
