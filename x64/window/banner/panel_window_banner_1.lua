...
function PaGlobal_Banner:wholeUpdateBanner()
  if ToClient_UserHasOverLevelCharacter(60) == false and isGameTypeCH() == false then
    return
  end
  if ToClient_UserHasOverLevelCharacter(30) == false and isGameTypeCH() == true then
    return
  end
...
function PaGlobal_Banner:setRenderTexture(eBannerType)
  ...
  self._ui._stc_bannerArea:ChangeTextureInfoNameAsync(self._basicTexture[eBannerType])
...
function PaGlobal_Banner:isCheckCondition(eBannerType)
  ...
  elseif self._eBannerType._blackFriday == eBannerType then
    return false
  elseif self._eBannerType._newProduct == eBannerType then
    return "" ~= PaGlobal_Banner_GetNewProductURL() and ToClient_HardCoreChannelWithContensOption() == false
  end
  return false
end
