...
function HouseImageSection:setImage(houseInfo)
  self._Image:ChangeTextureInfoNameAsync(HouseControlManager._screenShotPath)
  ...
  for index = 1, contentsViewcount do
    do
      ...
      function uigroup:uiTextureControl(currentLevel, isSameReceipe)
        for key, value in pairs(self.ui._grade) do
          if value:GetShow() then
            value:ChangeTextureInfoNameAsync("Renewal/PcRemaster/Remaster_WorldMap_00.dds")
            local x1, y1, x2, y2
      ...
      function uigroup:SetTextureUV(pData, pX1, pY1, pX2, pY2)
        pData:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/houseinfo/housecontrol_00.dds")
...
function HouseControlManager:WorkinUseTypeIconTexture(index, iconType)
  ...
  control:ChangeTextureInfoNameAsync("/New_UI_Common_forLua/Window/HouseInfo/useType/" .. fileName)
        ...
        if esSSW:isSet() then
          ...
          if nil ~= itemStatic then
            ...
            if gradeType > 0 and gradeType <= #UI.itemSlotConfig.borderTexture then
              self.borderList[index]:ChangeTextureInfoNameAsync(UI.itemSlotConfig.borderTexture[gradeType].texture)
          ...
          self.iconList[index]:ChangeTextureInfoNameAsync(workIcon)
  ...
  if eWorkType.craft == workType then
    ...
    for idx = 0, workingcnt - 1 do
      ...
      if esSSW:isSet() then
        ...
        HouseProgressSection._Icon_Working:ChangeTextureInfoNameAsync("icon/" .. workingIcon)
    ...
    for index = 1, workCount do
      if currentKey == ToClient_getWorkableExchangeKeyByIndex(index - 1) then
        ...
        if esSSW:isSet() then
          ...
          HouseProgressSection._Icon_Working:ChangeTextureInfoNameAsync(workIcon)
          HouseProgressSection._WorkName:SetText(workName)
          HouseProgressSection._Icon_BG:SetShow(true)
          HouseProgressSection._Icon_Working:SetShow(true)
...
function HouseProgressSection_SetBaseTextureUV(pData, pPath, pX1, pY1, pX2, pY2)
  pData:ChangeTextureInfoNameAsync(pPath)
  local x1, y1, x2, y2 = setTextureUV_Func(pData, pX1, pY1, pX2, pY2)
  pData:getBaseTexture():setUV(x1, y1, x2, y2)
  pData:setRenderTexture(pData:getBaseTexture())
end
