...
function PaGlobal_FriendInfoList_All:partyInvite()
  ...
  if nil ~= userCharacterName and "" ~= userCharacterName then
    ...
    if isSelfPlayerPlayingPvPMatch == false and isEnterPersonalField == false and isEnterPersonalRelayField == false then
...
function PaGlobal_FriendInfoList_All:messangerOpen(messangerId, friendUserNo)
  ...
  if isOnline == true and characterName ~= "" then
...
function PaGlobal_FriendInfoList_All:setPosMessageUI(isMe)
  ...
  while true do
    ...
    if nil ~= info.content then
      local tempSizeX = curTextSizeX + info.content:GetSizeX() + 15
  ...
  if true == _ContentsGroup_ConsoleIntegration then
    ...
    if selfPlayerPlatform == friendPlatformType then
      if __eGAME_PLATFORM_TYPE_PS == selfPlayerPlatform then
        targetPlatformIconControl:ChangeTextureInfoNameAsync("combine/commonicon/crossplay_icon_ps.dds")
      elseif __eGAME_PLATFORM_TYPE_XB == selfPlayerPlatform then
        targetPlatformIconControl:ChangeTextureInfoNameAsync("combine/commonicon/crossplay_icon_xb.dds")
      end
    else
      targetPlatformIconControl:ChangeTextureInfoNameAsync("combine/commonicon/crossplay_icon_other.dds")
    end
