...
function PaGlobal_Friend_GroupRename_All:prepareClose()
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    if _ContentsGroup_UsePadSnapping == true then
      ...
      PaGlobal_Friend_GroupRename_All._ui.edit_GroupName:RegistReturnKeyEvent("PaGlobal_Friend_GroupRename_All_Confirm()")
      PaGlobal_Friend_GroupRename_All._ui.btn_Confirm:SetIgnore(false)
    end
  end
...
function PaGlobal_Friend_GroupRename_All_RenameGroup()
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    local result = ToClient_CheckChinaProhibitedWord(editText, __eSceneIdRenameFriendGroup)
    if result == true then
      if _ContentsGroup_UsePadSnapping == true then
        PaGlobal_Friend_GroupRename_All._ui.edit_GroupName:RegistReturnKeyEvent("")
        Panel_Friend_GroupRename_All:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
      else
        PaGlobal_Friend_GroupRename_All._ui.edit_GroupName:RegistReturnKeyEvent("")
        PaGlobal_Friend_GroupRename_All._ui.btn_Confirm:SetIgnore(true)
      end
    end
    return
  end
...
function PaGlobal_Friend_GroupRename_All_AddGroup()
  local editText = PaGlobal_Friend_GroupRename_All._ui.edit_GroupName:GetEditText()
  if _ContentsOption_CH_CheckProhibitedWord == true then
    local result = ToClient_CheckChinaProhibitedWord(editText, __eSceneIdAddFriendGroup)
    if result == true then
      if _ContentsGroup_UsePadSnapping == true then
        PaGlobal_Friend_GroupRename_All._ui.edit_GroupName:RegistReturnKeyEvent("")
        Panel_Friend_GroupRename_All:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
...
function FromClient_CheckChinaProhibitedWord_AddFriendGroup(isPass, filteredText)
  ...
  if _ContentsGroup_UsePadSnapping == true then
    self._ui.edit_GroupName:RegistReturnKeyEvent("PaGlobal_Friend_GroupRename_All_Confirm()")
    Panel_Friend_GroupRename_All:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobal_Friend_GroupRename_All_Confirm()")
  else
    self._ui.edit_GroupName:RegistReturnKeyEvent("PaGlobal_Friend_GroupRename_All_Confirm()")
...
function FromClient_CheckChinaProhibitedWord_RenameFriendGroup(isPass, filteredText)
  if _ContentsOption_CH_CheckProhibitedWord == false then
    return
  end
  local self = PaGlobal_Friend_GroupRename_All
