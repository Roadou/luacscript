...
function Panel_Join_Update(index)
  ...
  join._headIcon:ChangeTextureInfoNameAsync(self._iconTextureUrl)
  MyRankRangeAndPoint(index)
  if 0 == index and 0 < ToClient_GetMyTeamNoRedBattleField() then
    local teamName
    ...
    if join.radioBtn[idx]:IsCheck() then
      if 0 == idx then
        do
          ...
          if join._conditionCheck then
            if 0 == ToClient_GetMyTeamNoRedBattleField() then
              ToClient_JoinRedBattleField()
              break
            end
            do
              local pcPosition = getSelfPlayer():get():getPosition()
              local regionInfo = getRegionInfoByPosition(pcPosition)
              ToClient_UnJoinRedBattleField()
            end
            break
          end
          if 0 == ToClient_GetMyTeamNoRedBattleField() then
...
function FromClient_JoinRedBattleField(teamNo)
  local teamName
  ...
  local mainStr = PAGetString(Defines.StringSheet_GAME, "LUA_JOIN_LOCALWAR_WELCOME_1")
  local subStr = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_JOIN_LOCALWAR_WELCOME_2", "team", teamName)
  ToClient_RequestNakMessageRenewelWithString(__eNakMessage_RedBattleFieldEnter, mainStr, subStr)
...
function FromClient_UnJoinRedBattleField(unJoinType)
  ...
  if 1 == unJoinType then
    msg.sub = PAGetString(Defines.StringSheet_GAME, "LUA_JOIN_LOCALWAR_LEAVEMSG_2")
  elseif 2 == unJoinType then
    msg.sub = PAGetString(Defines.StringSheet_GAME, "LUA_JOIN_LOCALWAR_LEAVEMSG_3")
  else
    msg.sub = " "
  end
  ToClient_RequestNakMessageRenewelWithString(__eNakMessage_RedBattleFieldLeave, msg.main, msg.sub)
  local dialogShow = false
  if true == Panel_Npc_Dialog_All:GetShow() then
    dialogShow = true
  end
...
function Panel_Join_RegisterEvent()
  ...
  registerEvent("FromClient_JoinRedBattleField", "FromClient_JoinRedBattleField")
  registerEvent("FromClient_UnJoinRedBattleField", "FromClient_UnJoinRedBattleField")
end
