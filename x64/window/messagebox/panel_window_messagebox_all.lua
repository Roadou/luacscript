...
local radioButtonList = {
  UI.getChildControl(Panel_Window_MessageBox_All, "Radiobutton_Select1"),
  UI.getChildControl(Panel_Window_MessageBox_All, "Radiobutton_Select2")
}
...
local function updateRefuseType(refuseType)
  ...
  if _refuseType.PARTY == refuseType and false == isConsole then
    local isPartyRefuse = optionWrapper:getIsPartyRefuse()
    refuseInvite:SetCheck(isPartyRefuse)
  ...
  elseif _refuseType.SOLARECUSTOMGAME == refuseType then
    ...
    if isConsole == true then
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_Y, "PaGlobalFunc_Clicked_RefuseButton(7)")
      static_consoleUI_Y:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_ALWAYSREFUSE"))
      static_consoleUI_Y:SetShow(true)
      PaGlobalFunc_MessageBox_RefreshConsoleKeyGuide()
    else
...
local function messageBox_Resize(refuseType)
  ...
  local selectGradeGap = 0
  if selectGradeBg:GetShow() == true then
    selectGradeGap = selectGradeBg:GetSizeY()
  end
  local radioButtonGap = 0
  if _currentMessageBoxData ~= nil and _currentMessageBoxData.radioButtonTextList ~= nil then
    local radioButtonCount = #_currentMessageBoxData.radioButtonTextList
    radioButtonGap = (radioButtonCount + 1) / 2 * 30
    radioButtonList[1]:ComputePos()
    radioButtonList[2]:ComputePos()
  end
  Panel_Window_MessageBox_All:SetSize(Panel_Window_MessageBox_All:GetSizeX(), textBG:GetSizeY() + 97 + padding + radioButtonGap + selectGradeGap)
...
function setCurrentMessageData(currentData, position, refuseType)
  if currentData ~= nil then
    ...
    if currentData.radioButtonTextList ~= nil then
      radioButtonCount = #currentData.radioButtonTextList
    end
    if radioButtonCount > 0 then
      for idx = 1, radioButtonCount do
        radioButtonList[idx]:addInputEvent("Mouse_LUp", "messageBox_ToggleRadioButton(" .. tostring(idx) .. ")")
        radioButtonList[idx]:SetText(currentData.radioButtonTextList[idx])
        radioButtonList[idx]:SetShow(true)
      end
    end
    if isConsole then
      local keyGuideTable = {
    ...
    messageBox_ToggleRadioButton(1)
    messageBox_Resize(refuseType)
  end
end
...
function MessageBox.showMessageBox(MessageData, position, isGameExit, keyUse, refuseType)
  ...
  checkCustom2:SetShow(false)
  checkCustom3:SetShow(false)
  for idx = 1, #radioButtonList do
    radioButtonList[idx]:SetShow(false)
  end
  local Front = list
  local preList
...
function PaGlobal_MessageBox_RefuseOption(refuseType)
  if _refuseType.PARTY == refuseType then
    setIsPartyRefuse(refuseInvite:IsCheck())
...
function messageBox_ToggleRadioButton(idx)
  if _currentMessageBoxData == nil then
    return
  end
  local radioButtonCount = 0
  if _currentMessageBoxData.radioButtonTextList ~= nil then
    radioButtonCount = #_currentMessageBoxData.radioButtonTextList
  end
  if radioButtonCount == 0 then
    return
  end
  if idx > radioButtonCount then
    return
  end
  for i = 1, radioButtonCount do
    radioButtonList[i]:SetCheck(false)
  end
  radioButtonList[idx]:SetCheck(true)
  if _currentMessageBoxData.radioButtonFunction ~= nil then
    _currentMessageBoxData.radioButtonFunction(idx)
  end
end
...
function Event_MessageBox_NotifyMessage_FreeButton(message, isUseLoadingIndicator)
  local priority, messageboxData
  if true == _ContentsGroup_UsePadSnapping then
    priority = UI_PP.PAUIMB_PRIORITY_LOW
  else
    priority = UI_PP.PAUIMB_PRIORITY_1
  end
  if "boolean" == type(isUseLoadingIndicator) and true == isUseLoadingIndicator then
    messageboxData = {
      title = "",
      content = message,
      priority = priority,
  ...
  elseif 1 == tonumber(isUseLoadingIndicator) then
    messageboxData = {
      title = "",
      content = message,
      priority = priority,
...
function messageBox_UpdatePerFrame(deltaTime)
  ...
  if nil ~= list.data.useLoadingIndicator and true == list.data.useLoadingIndicator then
    local loadingSuffix = ""
    if elapsedTime % 3 < 1 then
      loadingSuffix = ""
    elseif elapsedTime % 3 < 2 then
      loadingSuffix = "."
    else
      loadingSuffix = ".."
    end
    textContent:SetText(list.data.content .. loadingSuffix)
  end
  if nil ~= list.data.countTime and elapsedTime < list.data.countTime then
...
function PaGlobalFunc_MessageBox_BarterOrTradeItemFilter(slotNo, itemWrapper)
  ...
  if itemWrapper:getStaticStatus():get():getContentsEventType() == CppEnums.ContentsEventType.ContentsType_InventoryBag then
    ...
    if bagType == __eInventoryBagType_Fish then
