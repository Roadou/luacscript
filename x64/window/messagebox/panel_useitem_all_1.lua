...
function PaGlobal_UseItem_All:prepareOpen()
  ...
  if GetUIMode() == Defines.UIMode.eUIMode_SnowBoardInMirrorField then
    return
  end
...
function PaGlobal_UseItem_All:fillPopupUseItemData(itemEnchatWrapper)
  ...
  if 0 ~= popupItemSize then
    ...
    for idx = 1, popupItemSize do
      ...
      if nil == itemESS then
        ...
        self._popupItem.slotInfo[idx].icon:ChangeTextureInfoNameAsync("icon/new_icon/03_etc/item_unknown.dds")
      else
  ...
  for idxBuff = 1, popupBuffSize do
    ...
    if nil == buffSSW then
      self._popupItem.slotInfoForBuff[idxBuff]:ChangeTextureInfoNameAsync("icon/new_icon/03_etc/item_unknown.dds")
    else
      self._popupItem.slotInfoForBuff[idxBuff]:ChangeTextureInfoNameAsync("icon/" .. buffSSW:getIconPath())
