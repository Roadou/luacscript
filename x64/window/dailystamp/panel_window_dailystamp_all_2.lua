...
function FromClient_AttendanceNoticeView_DailyStamp()
  ToClinet_setShowRewardNotice(false)
end
...
function FromClient_OpenDailyStamp()
  if ToClinet_getShowRewardNotice() == false then
...
function PaGlobalFunc_DailyStamp_All_CheckAttandance()
  ...
  if _ContentsGroup_RenewUI == true and GetUIMode() ~= Defines.UIMode.eUIMode_Default then
    return
  end
  for index = 1, PaGlobal_DailyStamp_All._dailyStampCount do
    if nil ~= PaGlobal_DailyStamp_All._dailyStampKeys[index][1] and PaGlobal_DailyStamp_All._dailyStampKeys[index][1]:isPossibleAttendance() then
