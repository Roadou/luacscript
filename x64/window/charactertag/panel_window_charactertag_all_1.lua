function PaGlobal_CharacterTag_All:initialize()
  ...
  self._ui.mainImg1.txt_region = UI.getChildControl(self._ui.mainImg1.stc_bg, "StaticText_Region")
  ...
  self._ui.mainImg2.txt_region = UI.getChildControl(self._ui.mainImg2.stc_bg, "StaticText_Region")
  ...
  self._ui.btn_SyncPosition = UI.getChildControl(Panel_CharacterTag_All, "Check_Temp")
  self._ui.btn_SyncPosition:SetShow(_ContentsGroup_DuelCharacterPositionDesync)
  self._ui.btn_SyncPosition:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERTAG_INDIVIDUALACTIONBTN"))
  UI.setTextAndAddTooltip(self._ui.btn_SyncPosition, self._ui.btn_SyncPosition:GetText(), PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERTAG_INDIVIDUALACTION_TOOLTIPDESC"))
...
function PaGlobal_CharacterTag_All:dataclear()
  ...
  self._ui.mainImg2.stc_img:ChangeTextureInfoNameAsync("")
  self._ui.mainImg2.stc_img:setRenderTexture(self._ui.mainImg2.stc_img:getBaseTexture())
  self._ui.mainImg2.txt_region:SetText("")
...
function PaGlobal_CharacterTag_All:update()
  ...
  self:loadList()
  self:expInfoShow()
  FromClient_CharacterTag_All_UpdateSyncBtn(true)
end
  ...
  if self.LOCAL_DEFINE.NODUEL == duelCharIndex then
    ...
    if false == isSetLeft or false == isSetRight then
      ...
      if false == isSetRight then
        ...
        self._ui.mainImg2.txt_region:SetShow(false)
      end
    end
  else
    isSetLeft = true
...
function PaGlobal_CharacterTag_All:loadList()
  ...
  for idx = 0, 5 do
    ...
    elseif pcDeliveryRegionKeyRaw ~= 0 and serverUtc64 > characterData._arrivalTime then
      local deliveryRegionInfo
      if _ContentsGroup_WorldMapFamilyCharacterManagement == true then
        deliveryRegionInfo = ToClient_getRegionInfoWrapperByWaypoint(pcDeliveryRegionKeyRaw)
      else
        local arrivalRegionKey = RegionKey(pcDeliveryRegionKeyRaw)
        deliveryRegionInfo = getRegionInfoByRegionKey(arrivalRegionKey)
      end
      if deliveryRegionInfo ~= nil then
        if selfPlayerRegionInfoKey == deliveryRegionInfo:getRegionKey() then
          self:setMonoToneIgnore(targetUI, targetImage, false)
          targetState:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_TAG_CANDO"))
        else
          self:setMonoToneIgnore(targetUI, targetImage, true)
          targetState:SetText(tostring(deliveryRegionInfo:getAreaName()))
        end
      end
...
function PaGlobal_CharacterTag_All:setLeftFace(idx, isRegionKey)
  ...
  self._ui.mainImg1.txt_region:SetText(tostring(regionInfo:getAreaName()))
end
function PaGlobal_CharacterTag_All:setDuelRegionText()
  local duelRegion_Key = ToClient_GetSelfPlayerTagRegionKey()
  local regionInfo = getRegionInfoByRegionKey(duelRegion_Key)
  if regionInfo ~= nil then
    self._ui.txt_tagAreaValue:SetText(regionInfo:getAreaName())
  else
...
function PaGlobal_CharacterTag_All:setRightFace(idx)
  ...
  self._ui.mainImg2.txt_region:SetText(tostring(regionInfo:getAreaName()))
  self._ui.mainImg2.txt_region:SetShow(true)
...
function PaGlobal_CharacterTag_All:showManager(showModeNum, CharLV)
  ...
  if true == ToClient_doHaveDuelExp() then
    ...
    if true == _ContentsGroup_DuelCharacterCopyEquip then
      self._ui.btn_takeEXP:SetSpanSize(-125, self._ui.stc_titleBg:GetSizeY() + self._ui.stc_topBg:GetSizeY() - self._ui.btn_takeEXP:GetSizeY())
      self._ui.btn_charTAG:SetSpanSize(-125, self._ui.stc_titleBg:GetSizeY() + self._ui.stc_topBg:GetSizeY() - self._ui.btn_takeEXP:GetSizeY())
  ...
  if true == self._currentTagState then
    ...
    if true == _ContentsGroup_DuelCharacterCopyEquip then
      ...
      self._ui.btn_takeEXP:SetSpanSize(0, self._ui.stc_titleBg:GetSizeY() + self._ui.stc_topBg:GetSizeY() - self._ui.btn_takeEXP:GetSizeY())
      self._ui.btn_ItemCopy:SetShow(false)
    end
  end
end
...
function PaGlobal_CharacterTag_All:faceSetting(targetImage, char_Type)
  if _ContentsGroup_isUsedNewCharacterInfo == false then
    if char_Type == __eClassType_Warrior then
      targetImage:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/GameExit/GameExit_CharSlot_00.dds")
    ...
    elseif char_Type == __eClassType_Valkyrie then
      targetImage:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/GameExit/GameExit_CharSlot_01.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(targetImage, 1, 1, 156, 201)
      targetImage:getBaseTexture():setUV(x1, y1, x2, y2)
      targetImage:setRenderTexture(targetImage:getBaseTexture())
    ...
    elseif char_Type == __eClassType_DarkElf then
      targetImage:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/GameExit/GameExit_CharSlot_02.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(targetImage, 1, 1, 156, 201)
      targetImage:getBaseTexture():setUV(x1, y1, x2, y2)
      targetImage:setRenderTexture(targetImage:getBaseTexture())
    ...
    if DefaultFace ~= nil then
      targetImage:ChangeTextureInfoTextureIDAsync(DefaultFace, 0)
    end
  end
end
function PaGlobal_CharacterTag_All:registEventHandler()
  ...
  self._ui.btn_SyncPosition:addInputEvent("Mouse_LUp", "HandleEventLUp_CharacterTag_All_SetPositionSyncState()")
  ...
  registerEvent("FromClient_UpdateDuelCharacterPositionSyncState", "FromClient_CharacterTag_All_UpdateSyncBtn")
  registerEvent("FromClient_DoSyncDuelCharacterPositionDelay", "FromClient_CharacterTag_All_ShowDelay")
  registerEvent("selfPlayer_regionChanged", "PaGlobalFunc_CharacterTag_All_UpdateCurrentRegionText")
