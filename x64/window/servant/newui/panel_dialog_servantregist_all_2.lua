...
function HandleEventLUp_ServantRegist_All_SelectMapae(idx)
  ...
  if nil ~= PaGlobal_ServantRegist_All._itemSlotTable[idx]._itemData then
    ...
    local creatorName = getItemNaming(itemWrapper:getItemNoRaw())
    if creatorName == nil or creatorName == "" then
      PaGlobal_ServantRegist_All._ui._txt_creatorName:SetShow(false)
    else
      PaGlobal_ServantRegist_All._ui._txt_creatorName:SetShow(true)
      PaGlobal_ServantRegist_All._ui._txt_creatorName:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SHIPINFO_MADEBY_TITLE") .. " : " .. creatorName)
    end
    local hpTitle = ""
    local staminaTitle = ""
    local lifeTitle = ""
