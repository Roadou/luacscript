...
function HandleEventOn_ServantList_All_SelectServant(eType, slotNo)
  ...
  if Panel_Dialog_ServantLvUpMessageBox_All ~= nil and Panel_Dialog_ServantLvUpMessageBox_All:GetShow() == true then
    PaGlobal_ServantLvUpMessageBox_All_Close()
  end
  ...
  if true == PaGlobal_ServantList_All._isConsole then
    ToClient_padSnapGroupClearCurrentControl(Panel_Dialog_ServantInfo_All, PaGlobal_ServantInfo_All._ui.frame_category)
...
function HandleEventLUp_ServantList_All_Recovery_UnSealed_Servant()
  ...
  if 0 == currentNPCType or 2 == currentNPCType then
    servantWrapper = temporaryWrapper:getUnsealVehicle(__eServantTypeVehicle)
  end
...
function HandleEventLUp_ServantList_All_SellServant()
  ...
  local currentServantNo = PaGlobal_ServantList_All:stableList_SelectSlotNo(PaGlobal_ServantList_All._selectSlotNo)
  if currentServantNo == nil then
    return
  end
  local function sellServant()
...
function HandleEventLUp_ServantList_All_Regist_To_ServantMarket()
  ...
  if false == servantInfo:isTrade() then
    ...
    do
      ...
      local function showRegistMarketMessageBox()
      ...
      if servantInfo ~= nil and PaGlobalFunc_ServantList_All_CanStackLvUp(servantInfo:getServantNo()) then
        local title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE")
        local desc = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_LVUP_REGIST_WARNING")
        local confirm = showRegistMarketMessageBox
        local cancel = MessageBox_Empty_function
        local priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
        messageboxData = {
          title = title,
...
function HandleEventLUp_ServantList_All_ResetServant()
  ...
  if __eVehicleType_Horse ~= servantInfo:getVehicleType() then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantUseItemServantType"))
    return
  end
  ...
  local function easyBuySlot()
    if nil ~= Panel_IngameCashShop_EasyPayment then
      if _isNpcType == PaGlobal_ServantList_All._ENUM_NPC_TYPE._LAND then
        PaGlobalFunc_EasyBuy_Open(15, getCurrentWaypointKey())
      else
        PaGlobalFunc_EasyBuy_Open(78, getCurrentWaypointKey())
      end
    end
  end
...
function HandleEventLUp_ServantList_All_ReturnRent()
  ...
  local servantNo = servantInfo:getServantNo()
  local function returnRentConfirm()
    ToClient_RegisterServantForReturn(servantNo)
  end
  local function showReturnMessageBox()
    local title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE")
    local desc = PAGetString(Defines.StringSheet_GAME, "LUA_WINDOW_PRPOMOTEMARKET_RETURN_ALERT1")
    local confirm = returnRentConfirm
    local cancel = MessageBox_Empty_function
    local priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    messageboxData = {
      title = title,
  ...
  if PaGlobalFunc_ServantList_All_CanStackLvUp(servantNo) == true then
    local title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE")
    local desc = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_LVUP_REGIST_WARNING")
    local confirm = showReturnMessageBox
    local cancel = MessageBox_Empty_function
    local priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    messageboxData = {
      title = title,
...
function HandleEventLUp_ServantList_All_CargoLoadingOpen()
  ...
  if _ContentsGroup_EmployeePresetJob == true and PaGlobalFunc_SailorPresetManager_All_Close ~= nil then
    PaGlobalFunc_SailorPresetManager_All_Close()
  end
...
function PaGlobalFunc_ServantList_All_OceanShipChageItemConfirm()
  if _ContentsGroup_OceanShipChangeItem == false then
    return
  end
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() or nil == PaGlobal_ServantList_All._selectSlotNo then
    return
  ...
  local servantNo = servantInfo:getServantNo()
  ToClient_oceanShipChangeItem(servantNo)
end
function HandleEventLUp_ServantList_All_OceanShipChageItem()
  if _ContentsGroup_OceanShipChangeItem == false then
    return
  end
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
  ...
  local feeMoney = makeDotMoney(ToClient_oceanShipChangeItemFeeMoney())
  local _title = PAGetString(Defines.StringSheet_GAME, "LUA_OCEAN_SHIP_TRADE_ITEM_01")
  local _desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_OCEAN_SHIP_TRADE_ITEM_02", "needSilver", feeMoney)
  local _confirmFunction = PaGlobalFunc_ServantList_All_OceanShipChageItemConfirm
  local _cancel = MessageBox_Empty_function
  local _priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  local servantInfo
  local resultMoney = 0
  ...
  local servantType = servantInfo:getServantType()
  if servantType ~= __eServantTypeShip then
    return
  end
  messageboxData = {
    title = _title,
...
function HandleEventLUp_ServantList_All_DecreaseServantLevel()
  if Panel_Dialog_ServantList_All == nil or Panel_Dialog_ServantList_All:GetShow() == false then
    return
  end
  if _ContentsGroup_ServantLevelDown == false then
    return
  end
  PaGlobal_ServantList_All:subMenuClose()
  HandleEventOut_ServantList_All_HideChangeServantRegionTooltip()
  ...
  if PaGlobal_ServantList_All._currentSealType == PaGlobal_ServantList_All._ENUM_TYPESEALED then
    if PaGlobal_ServantList_All._selectSlotNo == nil then
      return
    end
    local currentSelectServantNo = PaGlobal_ServantList_All:stableList_SelectSlotNo(PaGlobal_ServantList_All._selectSlotNo)
    if currentSelectServantNo == nil then
      return
    end
    servantInfo = stable_getServant(currentSelectServantNo)
  elseif PaGlobal_ServantList_All._currentSealType == PaGlobal_ServantList_All._ENUM_TYPEUNSEALED then
    local temporaryWrapper = getTemporaryInformationWrapper()
    if temporaryWrapper == nil then
      return
    end
...
function FromClient_ServantList_All_DecreaseServantLevel()
  if nil == Panel_Dialog_ServantList_All then
    return
  end
  if _ContentsGroup_ServantLevelDown == false then
  ...
  PaGlobal_ServantInfo_All:update(PaGlobal_ServantList_All._ENUM_UPDATE_FLAG._ALL)
  PaGlobal_ServantList_All:update()
end
function PaGlobalFunc_ServantList_getFirstHorseServantNo()
  if Panel_Dialog_ServantList_All == nil then
    return nil
  end
  local self = PaGlobal_ServantList_All
  if self == nil then
    return nil
  end
  for ii = 1, #self._servantList do
    if self._servantList[ii]._vehicleType == __eVehicleType_Horse then
      return self._servantList[ii]._servantNo
    end
  end
  return nil
end
function PaGlobalFunc_ServantList_All_CanStackLvUp(currentServantNo)
  if _ContentsGroup_ServantLevelUpRenewal == false then
    return false
  end
  local servantInfo = stable_getServantByServantNo(currentServantNo)
  if servantInfo == nil then
    return false
  end
  local needExp_s64 = servantInfo:getNeedExperienceForLvUp()
  local expStack_s64 = servantInfo:getExperienceStack()
  if Int64toInt32(expStack_s64) > Int64toInt32(needExp_s64) then
    return true
  end
  return false
end
