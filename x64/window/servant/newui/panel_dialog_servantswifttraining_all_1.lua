...
function PaGlobal_ServantSwiftTraining_All:prepareOpen()
  ...
  if false == self._isSkillIconSet then
    for i = 0, skillCount - 1 do
      ...
      if nil ~= skillWrapper then
        self._trainingSlot[i]._skillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillWrapper:getIconPath())
      end
    end
    local itemSSW = stable_getStallionTrainingCompleteRequiredItem()
    if nil ~= itemSSW then
      self._ui._stc_CrogdaloSlot:ChangeTextureInfoNameAsync("Icon/" .. itemSSW:getIconPath())
    end
...
function PaGlobal_ServantSwiftTraining_All:update()
  ...
  if _ContentsGroup_GuaranteeHorseFusion == true then
    local ninethHorseMaximumStack = ToClient_getNinethHorseMaximumStack()
    if ninethHorseMaximumStack ~= nil and ninethHorseMaximumStack > 0 then
      if statckCount >= ninethHorseMaximumStack then
        successRate = 100
        ...
        self._ui._btn_Check_Cron:SetCheck(false)
      end
      self._ui._txt_Stack:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_SERVANT_SWIFTTRAINING_RATE", "count", tostring(statckCount) .. " / " .. tostring(ninethHorseMaximumStack), "rate", tostring(successRate)))
    end
  end
end
