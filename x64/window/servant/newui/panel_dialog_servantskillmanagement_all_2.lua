function PaGlobalFunc_ServantSkillManagement_All_Open(currentServantNo, skillChangeOnly)
  local self = PaGlobal_ServantSkillManagement_All
  if self == nil then
    return
  end
  if stable_getServantByServantNo(currentServantNo) == nil then
    return
  end
  if skillChangeOnly == nil then
    return
  end
  self._currentServantNo = currentServantNo
  self._skillChangeOnly = skillChangeOnly
  self:prepareOpen()
end
...
function HandleEventLUp_ServantSkillManagement_All_SkillListClick(slotIdx, isWishSkill, isStallionWithStallionSkill)
  ...
  local servantInfo = stable_getServantByServantNo(PaGlobal_ServantSkillManagement_All._currentServantNo)
  if servantInfo == nil then
    return
  end
  ...
  if false == _ContentsGroup_UsePadSnapping then
    ...
    if 0 == itemCount then
      local EasyBuyOpen = function()
        ...
        PaGlobalFunc_EasyBuy_Open(easyBuyUniqueNo)
      end
      local messageboxData = {
...
function FromClient_ServantSkillManagement_All_SkillChangeFinish(oldSkillKey, newSkillKey)
  ...
  if servantInfo ~= nil then
    local mainStr = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_STABLE_CHANGESKILL_MSG_MAIN_CHANGESKILL", "oldSkill", oldSkillWrapper:getName(), "newSkill", skillWrapper:getName())
    local subStr = PAGetString(Defines.StringSheet_GAME, "LUA_STABLE_CHANGESKILL_MSG_SUB_CONGRATULATION")
    ToClient_RequestNakMessageRenewelWithString(__eNakMessage_ChangeSkill, mainStr, subStr)
  end
  local skillChangeConfirmedCount = servantInfo:getServantSkillChangeConfirmedCount()
  if true == _ContentsGroup_UsePadSnapping then
    if nil == servantInfo or skillChangeConfirmedCount > 0 then
