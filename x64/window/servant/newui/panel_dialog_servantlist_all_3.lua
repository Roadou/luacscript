...
function FromClient_ServantList_All_Servant_SealFinish(servantNo, regionKey, servantWhereType)
  ...
  local key = PaGlobal_ServantList_All:stableList_GetKeyByServentNo(servantNo)
  if key ~= nil then
    HandleEventOn_ServantList_All_SelectServant(PaGlobal_ServantList_All._ENUM_TYPESEALED, key - 1)
  end
...
function FromClient_ServantList_All_ServantToReward(treasureComponentItemKey)
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  if treasureComponentItemKey == __eItemKeyUndefined then
    PaGlobal_ImperialSupplyMSG_Close()
  else
    PaGlobal_ImperialSupplyMSG_ShowTreasureComponentEffect(treasureComponentItemKey)
  end
