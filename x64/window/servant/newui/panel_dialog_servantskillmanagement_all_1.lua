function PaGlobal_ServantSkillManagement_All:initialize()
  ...
  self._ori_WishBgSizeY = self._ui._stc_Center_WishSKillBg:GetSizeY()
  self._ori_WishBgSpanY = self._ui._stc_Center_WishSKillBg:GetSpanSize().y
...
function PaGlobal_ServantSkillManagement_All:prepareOpen()
  ...
  if _ContentsGroup_ServantLevelUpRenewal == true then
    if self._skillChangeOnly == false then
...
function PaGlobal_ServantSkillManagement_All:resize()
  ...
  local radioBtnSize = self._ui._stc_TabGroup:GetSizeY()
  ...
  elseif self._tabType == self._eType.notLearn then
    local addSizeY = 0
    local addSpanY = 0
    if self._ori_DescY < self._ui._txt_Desc:GetTextSizeY() + gap then
      addSizeY = self._ui._txt_Desc:GetTextSizeY() + gap - self._ori_DescY
    ...
    if self._skillChangeOnly == true then
      addSizeY = addSizeY - radioBtnSize
      addSpanY = addSpanY - radioBtnSize
      self._ui._stc_TabGroup:SetShow(false)
    else
      self._ui._stc_TabGroup:SetShow(true)
    end
    ...
    self._ui._stc_Center_WishSKillBg:SetSize(self._ui._stc_Center_WishSKillBg:GetSizeX(), self._ori_WishBgSizeY + addSizeY)
    self._ui._stc_Center_WishSKillBg:SetSpanSize(self._ui._stc_Center_WishSKillBg:GetSpanSize().x, self._ori_WishBgSpanY + addSpanY)
  end
  self._ui._stc_BottomBg_Console:SetPosY(Panel_Dialog_ServantSkillManagement_All:GetSizeY())
  self._ui._stc_Center_WishSKillBg:ComputePos()
...
function PaGlobal_ServantSkillManagement_All:updateList2_UnLearnSKill(content, key)
  ...
  if false == self._isShowSkillChangeRate then
    if nil ~= skillWrapper and servantInfo:getStateType() ~= CppEnums.ServantStateType.Type_SkillTraining and false == skillWrapper:isTrainingSkill() then
      ...
      unLearn_skillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillWrapper:getIconPath())
...
function PaGlobal_ServantSkillManagement_All:updateWishskill(isWishSkill, isRefreshRate)
  ...
  if true == isWishSkill then
    ...
    self._ui._stc_WishSkill_SkillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillWrapper:getIconPath())
    if false == self._isConsole then
    ...
    self._ui._stc_Delete_SkillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillWrapper:getIconPath())
    if false == self._isConsole then
...
function PaGlobal_ServantSkillManagement_All:getServantInfo()
  ...
  return stable_getServantByServantNo(self._currentServantNo)
end
