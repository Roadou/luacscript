...
function PaGlobal_ServantList_All:registerEventHandler(isConsole)
  ...
  if _ContentsGroup_ServantLevelDown == true then
    registerEvent("FromClient_DecreaseServantLevel", "FromClient_ServantList_All_DecreaseServantLevel")
  end
...
function PaGlobal_ServantList_All:prepareOpen()
  ...
  if self._ENUM_NPC_TYPE._SEA == currentNPCType or self._ENUM_NPC_TYPE._GUILD_SEA == currentNPCType then
    ...
    self._ui._btn_BuyWeight:SetShow(false)
    if true == _ContentsGroup_TransportBackward then
      if true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_HorseRacing) or CppEnums.ServantWhereType.ServantWhereTypePcRoom == stable_getServantWhereType() then
...
function PaGlobal_ServantList_All:update()
  ...
  local crogCount = 0
  local maxCount = ToClient_getServantMaxCountByType(CppEnums.ServantWhereType.ServantWhereTypeCrogdaloStable)
  ...
  if true == PaGlobalFunc_ServantFunction_All_GetIsGuild() then
    ...
    self._ui._btn_BuyWeight:SetShow(false)
    if PaGlobalFunc_ServantFunction_All_Get_NpcType() ~= self._ENUM_NPC_TYPE._GUILD_SEA then
      self._ui._btn_IncreaseSlot:SetShow(false)
    end
  else
...
function PaGlobal_ServantList_All:setDataList2MYServant(content, key)
  ...
  btn:ChangeTextureInfoTextureIDAsync("Combine_Frame_GradationSlot_Normal", 0)
  btn:ChangeTextureInfoTextureIDAsync("Combine_Frame_GradationSlot_Over", 1)
  ...
  if nil ~= servantInfo then
    ...
    image:ChangeTextureInfoNameAsync(servantInfo:getIconPath1())
    ...
    if vehicleType == CppEnums.VehicleType.Type_Horse or vehicleType == CppEnums.VehicleType.Type_RaceHorse then
      if isMutationHorse == true then
        ...
        horseIcon:SetMonoTone(false, false)
        local x1, y1, x2, y2
        horseIcon:ChangeTextureInfoNameAsync("combine/etc/combine_etc_imperial_horse.dds")
      ...
      elseif servantInfo:getTier() < 9 and false == servantInfo:isPcroomOnly() and self._contentsOption._isContentsStallionEnable then
        ...
        local x1, y1, x2, y2
        horseIcon:ChangeTextureInfoNameAsync("combine/etc/combine_etc_stable.dds")
    ...
    if key32 == self._selectSlotNo then
      btn:ChangeTextureInfoTextureIDAsync("Combine_Frame_GradationSlot_OverClick", 0)
      btn:ChangeTextureInfoTextureIDAsync("Combine_Frame_GradationSlot_OverClick", 1)
      btn:setRenderTexture(btn:getBaseTexture())
    end
    btn:SetShow(true)
    ...
    for ii = 0, maxCount - 1 do
      ...
      if nil ~= servantInfo then
        ...
        if isInsert == true then
          ...
          self._servantList[lastInsertedIndex]._vehicleType = vehicleType
          self._servantList[lastInsertedIndex]._hasLevel = PaGlobal_ServantList_All:checkHasLevel(servantInfo)
          self._servantList[lastInsertedIndex]._hasGender = PaGlobal_ServantList_All:checkHasGender(servantInfo)
          self._servantList[lastInsertedIndex]._canMating = CppEnums.VehicleType.Type_Horse == vehicleType
...
function PaGlobal_ServantList_All:stableList_GetKeyByServentNo(servantNo)
  if Panel_Dialog_ServantList_All == nil then
    return nil
  end
  for key, value in pairs(self._servantList) do
    if value._servantNo == servantNo then
      return key
    end
  end
  return nil
...
function PaGlobal_ServantList_All:setMyServant()
  ...
  if servantInfo ~= nil and servantInfo:isMasterlessHorse() == false then
    ...
    self._ui._icon_CurrentServant:ChangeTextureInfoNameAsync(servantInfo:getIconPath1())
    self._ui._txt_CurrentServantLocate:SetShow(false)
    ...
    if servantType == CppEnums.VehicleType.Type_Horse or servantType == CppEnums.VehicleType.Type_RaceHorse then
      if isMutationHorse == true then
        ...
        local x1, y1, x2, y2
        self._ui._icon_CurrentServant_Swift:ChangeTextureInfoNameAsync("combine/etc/combine_etc_imperial_horse.dds")
      ...
      elseif servantTier < 9 and false == isPcroomOnly and self._contentsOption._isContentsStallionEnable then
        ...
        local x1, y1, x2, y2
        self._ui._icon_CurrentServant_Swift:ChangeTextureInfoNameAsync("combine/etc/combine_etc_stable.dds")
    ...
    if (servantType == CppEnums.VehicleType.Type_Horse or servantType == CppEnums.VehicleType.Type_RaceHorse) and true == servantInfo:isDisplayGender() and (0 == npcType or 1 == npcType) then
      ...
      self._ui._icon_CurrentServantGender:ChangeTextureInfoNameAsync("combine/etc/combine_etc_stable.dds")
    ...
    elseif __eServantTypeShip == stable_getServantType() then
      local servantWrapper = temporaryWrapper:getUnsealVehicle(__eServantTypeShip)
      ...
      if nil ~= servantInfo then
        if false == PaGlobalFunc_ServantFunction_All_GetIsGuild() then
          if servantInfo:getVehicleType() ~= CppEnums.VehicleType.Type_BabyElephant then
            ...
            self._ui._stc_WildHorseImage:ChangeTextureInfoNameAsync(servantInfo:getIconPath1())
...
function PaGlobal_ServantList_All:subMenuOpen(eType)
  ...
  if Panel_Dialog_HorseLevelDecrease_All ~= nil and Panel_Dialog_HorseLevelDecrease_All:GetShow() == true then
    PaGlobal_Dialog_HorseLevelDecrease_All_Close()
  end
  ...
  if eType == self._ENUM_TYPESEALED then
    ...
    elseif regionName == servantRegionName then
      ...
      if true == isLinkedHorse and stallionTraining ~= getState then
        ...
        if false == isPcroomOnly and servantInfo:doImprint() and stallionTraining ~= getState and false == hasRentOwnerFlag and vehicleType == eServantType.Type_Horse then
          PaGlobal_ServantList_All:setSubMenuData(self._subMenuGroup[self._subMenuCount], self._ENUM_SUBMENUTYPE._IMPRINT, index)
        end
        if servantInfo:isChangeableLookUp() == true and false == isPcroomOnly and eServantType.Type_Horse == vehicleType and nowMating ~= getState and regMarket ~= getState and regMating ~= getState and training ~= getState and regionName == servantRegionName and self._contentsOption._isContentsEnable and stallionTraining ~= getState and false == hasRentOwnerFlag and vehicleSubType ~= __eTVehicleSubType_MutationHorse and (servantInfo:isDreamAdooanat() == false or 0 < getServantFormManager():getFormCountByCharacterKey(servantInfo:getCharacterKey())) then
        ...
        if true == _ContentsGroup_SetBeginningLevelServant and false == hasRentOwnerFlag and eServantType.Type_Horse == vehicleType and false == isPcroomOnly and (eServantType.Type_Horse == vehicleType or eServantType.Type_Donkey == vehicleType or eServantType.Type_Camel == vehicleType or eServantType.Type_RidableBabyElephant == vehicleType) and stable == getState and regionName == servantRegionName and isMutationHorse == false then
        ...
        if ToClient_IsPossibleServantLevelDown(servantInfo:getServantNo()) == true and stable == getState and regionName == servantRegionName then
          PaGlobal_ServantList_All:setSubMenuData(self._subMenuGroup[self._subMenuCount], self._ENUM_SUBMENUTYPE._DECREASESERVANTLEVEL, index)
        end
      end
  ...
  elseif eType == self._ENUM_TYPEUNSEALED then
    ...
    if false == isSiegeStable() then
      ...
      if _ContentsGroup_ServantLevelUpRenewal == true and ToClient_IsPossibleServantLevelDown(unSealServantInfo:getServantNo()) == true then
        PaGlobal_ServantList_All:setSubMenuData(self._subMenuGroup[self._subMenuCount], self._ENUM_SUBMENUTYPE._DECREASESERVANTLEVEL, index)
      end
...
function PaGlobal_ServantList_All:shipSubMenuOpen(eType)
  ...
  if eType == self._ENUM_TYPESEALED then
    ...
    if regionName == servantRegionName then
      ...
      if _ContentsGroup_OceanShipChangeItem == true then
        local isPossible = ToClient_oceanShipChangeItemPossible(servantInfo:getCharacterKeyRaw())
        if isPossible == true and CppEnums.VehicleType.Type_Carrack == vehicleType then
          PaGlobal_ServantList_All:setSubMenuData(self._subMenuGroup[self._subMenuCount], self._ENUM_SUBMENUTYPE._OCEANSHIPCHANGEITEM, index)
        end
      end
    elseif nowHp <= 0 then
      PaGlobal_ServantList_All:setSubMenuData(self._subMenuGroup[self._subMenuCount], self._ENUM_SUBMENUTYPE._REPAIR, index)
...
function PaGlobal_ServantList_All:guildShipSubMenuOpen(eType)
  ...
  if eType == self._ENUM_TYPESEALED then
    ...
    local getState = servantInfo:getStateType()
    local deadCount = servantInfo:getDeadCount()
    local showChangeRegionButtonFlag = not servantInfo:isChangingRegion()
    ...
    elseif regionName == servantRegionName then
      ...
      if showChangeRegionButtonFlag == true then
        PaGlobal_ServantList_All:setSubMenuData(self._subMenuGroup[self._subMenuCount], self._ENUM_SUBMENUTYPE._TRANSFER, index)
      end
    end
  end
...
function PaGlobal_ServantList_All:setSubMenuData(control, menutype, slotIndex)
  ...
  elseif menutype == self._ENUM_SUBMENUTYPE._SAILORMANGER then
    ...
    if _ContentsGroup_EmployeePresetJob == true then
      control:addInputEvent("Mouse_LUp", "PaGlobalFunc_SailorPresetManager_All_Open(false)")
    else
  ...
  elseif menutype == self._ENUM_SUBMENUTYPE._OCEANSHIPCHANGEITEM then
    self._isImportantMenu = true
    control = self._importantMenuGroup[self._importantMenuCount]
    control:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_OCEAN_SHIP_TRADE_ITEM_01"))
    control:addInputEvent("Mouse_LUp", "HandleEventLUp_ServantList_All_OceanShipChageItem()")
  elseif menutype == self._ENUM_SUBMENUTYPE._DECREASESERVANTLEVEL then
    control:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_LEVEL_DOWN"))
    control:addInputEvent("Mouse_LUp", "HandleEventLUp_ServantList_All_DecreaseServantLevel()")
  end
...
function PaGlobal_ServantList_All:hilightSelectedServant()
  ...
  if PaGlobal_ServantList_All._currentSealType == PaGlobal_ServantList_All._ENUM_TYPEUNSEALED then
    ...
    self._ui._list2_Servant:requestUpdateByKey(self._selectSlotNo)
  end
end
