...
function HandleEventLUp_CargoSell_Sell()
  ...
  local CargoSellComfirm = function()
    ToClient_SellCargoSellListItem(PaGlobal_CargoSell._selectWaypointKey)
  end
...
function HandleEventRUp_PaGlobal_CargoSell_Item(index)
  ...
  if itemSSW:isStackable() == false and _ContentsGroup_BarterConvenienceRenewal == true then
    local itemCount = ToClient_GetNonStackItemCountInCargoWareHouse(slotNo)
    itemCount = toInt64(0, itemCount)
    Panel_NumberPad_Show(true, itemCount, nil, PaGlobal_CargoSell_NonStackToCargoSellList)
  else
    Panel_NumberPad_Show(true, itemWrapper:get():getCount_s64(), slotNo, PaGlobal_CargoSell_ToCargoSellList)
  end
end
function PaGlobal_CargoSell_NonStackToCargoSellList(s64_count)
  ToClient_PushCargoSellListNonStackItems(s64_count)
end
