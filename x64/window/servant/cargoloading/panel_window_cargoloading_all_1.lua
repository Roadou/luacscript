function PaGlobal_CargoLoading:initialize()
  ...
  self._ui._stc_keyGuideLT_X = UI.getChildControl(self._ui._stc_keyGuideBG, "StaticText_Console_LT_X")
  self._ui._btn_unloadAll = UI.getChildControl(Panel_Window_CargoLoading_ALL, "Button_UnLoadAll")
  self._ui._btn_unloadAll:SetShow(false)
  if _ContentsGroup_BarterConvenienceRenewal == true and self._isConsole == false then
    self._ui._btn_unloadAll:SetShow(true)
  end
  self._blankSlots = {}
  self._slots = {}
...
function PaGlobal_CargoLoading:registEventHandler()
  ...
  self._ui._btn_unloadAll:addInputEvent("Mouse_LUp", "PaGlobal_CargoLoading_ShipUnLoadAllItemToWarehouse()")
...
function PaGlobal_CargoLoading:setConsoleUI()
  ...
  if true == self._isConsole then
    ...
    self._ui._stc_keyGuideY:SetShow(false)
    self._ui._stc_keyGuideLT_X:SetShow(false)
  end
end
function PaGlobal_CargoLoading:alignKeyGuide()
  ...
  if self._ui._stc_keyGuideLT_X:GetShow() == true then
    keyGuide[1] = self._ui._stc_keyGuideLT_X
  end
...
function PaGlobal_CargoLoading:validate()
  ...
  self._ui._stc_keyGuideLT_X:isValidate()
end
  ...
  for ii = 0, self._config.slotCount_s - 1 do
    ...
    if nil == slot.slotNo then
      ...
      if nil ~= itemWrapper then
        ...
        if itemWrapper:getStaticStatus():get():getContentsEventType() == CppEnums.ContentsEventType.ContentsType_InventoryBag and slot.bagIcon ~= nil then
...
function PaGlobal_CargoLoading_requestNonStackItemPopToInventory(s64_count, slotNo)
  local selfPlayer = getSelfPlayer()
  if PaGlobal_CargoLoading._eSlotType.inventory == PaGlobal_CargoLoading._clickedSlotType then
    if PaGlobal_CargoLoading._eMySlot.myInven == PaGlobal_CargoLoading._currentMySlotType then
      local itemCount = ToClient_getNonStackItemCount(CppEnums.ItemWhereType.eInventory, slotNo, false, PaGlobal_CargoLoading._currentWaypointKey)
      itemCount = toInt64(0, itemCount)
      Panel_NumberPad_Show(true, itemCount, nil, PaGlobal_CargoLoading_MoveNonStackItemFromPlayerToShip)
    elseif PaGlobal_CargoLoading._eMySlot.myWarehouse == PaGlobal_CargoLoading._currentMySlotType then
      local itemCount = ToClient_getNonStackItemCount(CppEnums.ItemWhereType.eWarehouse, slotNo, false, PaGlobal_CargoLoading._currentWaypointKey)
      itemCount = toInt64(0, itemCount)
      Panel_NumberPad_Show(true, itemCount, slotNo, PaGlobal_CargoLoading_PopNonStackItemFromWarehouse)
    end
  elseif PaGlobal_CargoLoading._eSlotType.servant == PaGlobal_CargoLoading._clickedSlotType then
    if PaGlobal_CargoLoading._eMySlot.myInven == PaGlobal_CargoLoading._currentMySlotType then
      local itemCount = ToClient_getNonStackItemCountInVehicleInventory(slotNo, PaGlobal_CargoLoading._servantActorKey)
      itemCount = toInt64(0, itemCount)
      Panel_NumberPad_Show(true, itemCount, nil, PaGlobal_CargoLoading_MoveNonStackItemFromShipToPlayer)
    elseif PaGlobal_CargoLoading._eMySlot.myWarehouse == PaGlobal_CargoLoading._currentMySlotType then
      local itemCount = ToClient_getNonStackItemCountInVehicleInventory(slotNo, PaGlobal_CargoLoading._servantActorKey)
      itemCount = toInt64(0, itemCount)
      Panel_NumberPad_Show(true, itemCount, slotNo, PaGlobal_CargoLoading_PushNonStackItemToWarehouse)
    end
  end
end
function PaGlobal_CargoLoading_PopNonStackItemFromWarehouse(s64_count, slotNo)
  warehouse_popToInventoryByWharf(slotNo, s64_count, PaGlobal_CargoLoading._servantActorKey, true)
end
function PaGlobal_CargoLoading_MoveNonStackItemFromPlayerToShip(s64_count)
  local selfPlayer = getSelfPlayer()
  local selfActorKeyRaw = selfPlayer:getActorKey()
  moveNonStackInventoryItemFromActorToActor(selfActorKeyRaw, PaGlobal_CargoLoading._servantActorKey, CppEnums.ItemWhereType.eInventory, s64_count)
end
function PaGlobal_CargoLoading_MoveNonStackItemFromShipToPlayer(s64_count)
  local selfPlayer = getSelfPlayer()
  local selfActorKeyRaw = selfPlayer:getActorKey()
  moveNonStackInventoryItemFromActorToActor(PaGlobal_CargoLoading._servantActorKey, selfActorKeyRaw, CppEnums.ItemWhereType.eServantInventory, s64_count)
end
function PaGlobal_CargoLoading_PushNonStackItemToWarehouse(s64_count, slotNo)
  warehouse_pushFromInventoryItemByWharf(CppEnums.ItemWhereType.eServantInventory, slotNo, s64_count, PaGlobal_CargoLoading._servantActorKey, true)
end
  ...
  if nil == warehouseWrapper then
    if true == self._isConsole then
      ...
      if _ContentsGroup_BarterConvenienceRenewal == true then
        Panel_Window_CargoLoading_ALL:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "")
      end
    ...
    self._ui._btn_unloadAll:SetShow(false)
    self._ui._btn_barterInfo:SetPosX(Panel_Window_CargoLoading_ALL:GetSizeX() / 2 - self._ui._btn_barterInfo:GetSizeX())
    self._ui._btn_confirm:SetPosX(Panel_Window_CargoLoading_ALL:GetSizeX() / 2 + 10)
    ...
    if true == self._isConsole then
      ...
      self._ui._stc_keyGuideRB:SetShow(true)
      if _ContentsGroup_BarterConvenienceRenewal == true then
        self._ui._stc_keyGuideLT_X:SetShow(true)
        Panel_Window_CargoLoading_ALL:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobal_CargoLoading_ShipUnLoadAllItemToWarehouse()")
      end
      Panel_Window_CargoLoading_ALL:registerPadEvent(__eConsoleUIPadEvent_LB, "HandleEventPad_CargoLoading_UpdateSlot(false)")
      Panel_Window_CargoLoading_ALL:registerPadEvent(__eConsoleUIPadEvent_RB, "HandleEventPad_CargoLoading_UpdateSlot(true)")
      self._ui._btn_unloadAll:SetShow(false)
    ...
    self._ui._btn_barterInfo:SetPosX(Panel_Window_CargoLoading_ALL:GetSizeX() / 2 - self._ui._btn_barterInfo:GetSizeX() / 2)
    self._ui._btn_confirm:SetPosX(Panel_Window_CargoLoading_ALL:GetSizeX() / 2 + self._ui._btn_barterInfo:GetSizeX() / 2 + 10)
