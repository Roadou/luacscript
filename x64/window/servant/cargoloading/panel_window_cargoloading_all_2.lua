...
function PaGlobal_CargoLoading_PopToSomewhere(s64_count, slotNo)
  local itemWrapper = PaGlobal_CargoLoading_GetMoveItem(slotNo)
  if itemWrapper == nil then
    return
  end
  if itemWrapper:getStaticStatus():isStackable() == false and _ContentsGroup_BarterConvenienceRenewal == true then
    PaGlobal_CargoLoading_requestNonStackItemPopToInventory(s64_count, slotNo)
  elseif s64_count > toInt64(0, 1) then
...
function HandleEventMLUp_CargoLoading_ToggleBarterInfoForPC()
  ...
  if true == Panel_Window_Barter_Search:IsShow() then
    ...
    PaGlobal_BarterInfoSearch._isOpenedByCargoLoading = true
...
function PaGlobal_CargoLoading_ShipUnLoadAllItemToWarehouse()
  warehouse_pushFromInventoryAllItemsByWharf(CppEnums.ItemWhereType.eServantInventory, PaGlobal_CargoLoading._servantActorKey, PaGlobal_CargoLoading._currentWaypointKey)
end
