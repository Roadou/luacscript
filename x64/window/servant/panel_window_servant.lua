...
function servantIcon:update()
  ...
  if nil ~= landVehicleWrapper then
    self._slots[0].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Horse/servant_icon_01.dds")
    if UI_VT.Type_Horse == landVehicleWrapper:getVehicleType() then
      ...
      if isPremiumPcRoom and (isGameTypeKorea() or isGameTypeJapan()) then
        if doHaveContentsItem(16, UI_VT.Type_Horse) then
          ...
          self._slots[0].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Horse/servant_icon_02.dds")
  ...
  if nil ~= seaVehicleWrapper and UI_VT.Type_SailingBoat ~= seaVehicleWrapper:getVehicleType() then
    self._slots[1].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Horse/servant_icon_01.dds")
  ...
  for summon_idx = 0, summonCount - 1 do
    ...
    if characterKey >= 60028 and characterKey <= 60037 or 60068 == characterKey then
      ...
      self._slots[2].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Horse/servant_icon_01.dds")
    ...
    elseif 60134 == characterKey or 60137 == characterKey or 60136 == characterKey or 60135 == characterKey then
      ...
      self._slots[4].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Horse/servant_icon_02.dds")
  ...
  for index = 0, elephantCount - 1 do
    ...
    if UI_VT.Type_Elephant == vehicleType or UI_VT.Type_Train == vehicleType or UI_VT.Type_SailingBoat == vehicleType or UI_VT.Type_PersonalBattleShip == vehicleType or UI_VT.Type_CashPersonalBattleShip == vehicleType or UI_VT.Type_SiegeTower == vehicleType or UI_VT.Type_LargeSiegeTower == vehicleType or UI_VT.Type_GuildDragon == vehicleType then
      self._slots[3].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Window/Horse/servant_icon_01.dds")
...
function Servant_Call(index)
  ...
  if true == _ContentsGroup_PersonalField and (ToClient_IsEnterMirrorField(__eMirrorFieldContentsKey_PersonalField) == true or ToClient_IsEnterMirrorField(__eMirrorFieldContentsKey_PersonalRelayField) == true) then
...
function Servant_Navi(index)
  ...
  elseif servantIcon.const.Pet == index and isCheckPetNavi == false then
    local servantInfo = temporaryWrapper:getUnsealVehicle(__eServantTypePet)
    if nil == servantInfo then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLELIST_NOT_UNSEAL_SERVANT"))
      return
    end
...
function ServantIcon_Open()
  ...
  local landVehicleWrapper = temporaryWrapper:getUnsealVehicle(__eServantTypeVehicle)
  local seaVehicleWrapper = temporaryWrapper:getUnsealVehicle(__eServantTypeShip)
  local petWrapper = temporaryWrapper:getUnsealVehicle(__eServantTypePet)
