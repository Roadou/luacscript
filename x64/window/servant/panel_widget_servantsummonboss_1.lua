function PaGlobal_ServantSummonBoss:initialize()
  ...
  if vehicleType == __eVehicleType_MonsterRide or vehicleType == __eVehicleType_GuildDragon then
...
function PaGlobal_ServantSummonBoss:prepareOpen()
  ...
  local vehicleType = vehicleProxy:get():getVehicleType()
  if vehicleType ~= __eVehicleType_MonsterRide and vehicleType ~= __eVehicleType_GuildDragon then
    return
  end
  local isHideHorseHP
  ...
  if isHideHorseHP ~= nil and isHideHorseHP == true then
  ...
  self:commandTextSet()
  self:update()
  local instanceMapKey = ToClient_getJoinRedBattleFieldMapKey()
  if instanceMapKey == __eInstanceRedBattleField_PapuCrio then
    local teamNo = ToClient_GetMyTeamNoRedBattleField()
    if 0 == teamNo then
