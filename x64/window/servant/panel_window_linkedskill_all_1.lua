function PaGlobal_LinkedSkill_All:initialize()
  ...
  self._ui._txt_skillBg = UI.getChildControl(Panel_Window_LinkedSkill_All, "StaticText_SkillTitle")
  self._ui._txt_skillTitle = UI.getChildControl(self._ui._txt_skillBg, "StaticText_SkillTitleName")
  self._ui._btn_skillChange = UI.getChildControl(self._ui._txt_skillBg, "Button_SkillChange")
  ...
  self._ui._btn_detailStatus = UI.getChildControl(Panel_Window_LinkedSkill_All, "Button_DetailStatus")
  self._ui._btn_stackLvUp = UI.getChildControl(Panel_Window_LinkedSkill_All, "Button_Stack_LvUp")
  self._ui._btn_lvDown = UI.getChildControl(Panel_Window_LinkedSkill_All, "Button_LvDown")
  self:init_skill()
  self:init_equipSlot()
  self:setShowControl()
...
function PaGlobal_LinkedSkill_All:registEventHandler()
  ...
  self._ui._btn_detailStatus:addInputEvent("Mouse_LUp", "PaGlobal_LinkedSkill_OpenHorseSimulator()")
  local panelSizeY = Panel_Window_LinkedSkill_All:GetSizeY()
  if _ContentsGroup_ServantLevelUpRenewal == false then
    panelSizeY = panelSizeY - (self._ui._btn_stackLvUp:GetSizeY() + 10)
    self._ui._btn_detailStatus:SetSpanSize(self._ui._btn_detailStatus:GetSpanSize().x, 60)
  end
  Panel_Window_LinkedSkill_All:SetSize(Panel_Window_LinkedSkill_All:GetSizeX(), panelSizeY)
  if ToClient_isUsePadSnapping() == true then
    self._ui._console._stc_keyGuideBG:SetPosY(Panel_Window_LinkedSkill_All:GetSizeY())
  end
  self._ui._btn_skillChange:addInputEvent("Mouse_LUp", "PaGlobal_LinkedSkill_All_SkillChange()")
  self._ui._btn_stackLvUp:addInputEvent("Mouse_LUp", "PaGlobal_LinkedSkill_All_StackLvUp()")
  self._ui._btn_lvDown:addInputEvent("Mouse_LUp", "PaGlobal_LinkedSkill_All_LevelDecrease()")
  ...
  registerEvent("FromClient_DecreaseServantLevel", "FromClient_LinkedSkill_All_UpdateSkill()")
  registerEvent("FromClient_ServantChangeSkill", "FromClient_LinkedSkill_All_UpdateSkill()")
end
...
function PaGlobal_LinkedSkill_All:updateSkill()
    ...
    if nil ~= skillWrapper then
      if startIndexCheck == self._skillStartIndex then
        if learnSkillCount < self._skillConfig.count then
          ...
          slot._stc_skillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillWrapper:getIconPath())
    ...
    if nil ~= skillWrapperXXX then
      if startIndexCheck == self._skillStartIndex then
        if learnSkillCount < self._skillConfig.count then
          local slot = self._skillSlot[learnSkillCount]
          slot._stc_skillBg:SetShow(true)
          slot._stc_skillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillWrapperXXX:getIconPath())
  ...
  if _ContentsGroup_ServantLevelUpRenewal == true then
    ...
    if ToClient_IsPossibleServantLevelUpByExpStack(servantNo, false) == true then
...
function PaGlobal_LinkedSkill_All:openHorseSimulator()
  if Panel_Window_LinkedSkill_All == nil then
    return
  end
  if _ContentsGroup_VehicleLearnSkill_Renewal == false or _ContentsGroup_HorseSimulator == false then
    return
  end
  if self._currentIndex == nil or self._actorKeyRaw == nil then
    return
  end
  if PaGlobalFunc_ServantSimulator_IsShow() == true then
    ...
    local servantNo = servantInfo:getServantNo()
    PaGlobalFunc_ServantSimulator_Open(openTabList, servantNo, nil)
  end
end
