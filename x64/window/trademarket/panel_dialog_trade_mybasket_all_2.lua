...
function PaGlobalFunc_TradeMyBasket_All_ToMyVehicle()
  ...
  if PaGlobal_TradeMyBasket_All._ENUM_ALL_VEHICLE == PaGlobal_TradeMyBasket_All._vehicleStatus then
    if true == btn_Land:IsChecked() then
      myVehicleActorKey = myLandVehicle:getUnsealVehicle(__eServantTypeVehicle):getActorKeyRaw()
    elseif true == btn_Sea:IsChecked() then
      myVehicleActorKey = myLandVehicle:getUnsealVehicle(__eServantTypeShip):getActorKeyRaw()
    end
...
function PaGlobalFunc_TradeMyBasket_All_ConfirmSell(inputNumber, slotIdx)
  ...
  local eServantType = __eServantTypeVehicle
  ...
  elseif PaGlobal_TradeMyBasket_All._ENUM_FISH_BAG == itemData.itemFromWhere then
    ...
    if PaGlobal_TradeMyBasket_All._ENUM_LAND_VEHICLE == itemData.itemFromWhere then
      ...
      if nil ~= servantInfo then
        ...
        eServantType = __eServantTypeVehicle
      end
    elseif PaGlobal_TradeMyBasket_All._ENUM_SEA_VEHICLE == itemData.itemFromWhere then
      servantInfo = temporaryWrapper:getUnsealVehicle(__eServantTypeShip)
      if nil ~= servantInfo then
        ...
        eServantType = __eServantTypeShip
      end
    end
  end
  ...
  local itemCommerceType = itemWrapper:getStaticStatus():getCommerceType()
  local itemCategory = __eSupplyItemType_Count
  if itemCommerceType == enCommerceType.enCommerceType_Grocery then
    itemCategory = __eSupplyItemType_Cook
  elseif itemCommerceType == enCommerceType.enCommerceType_Medicine then
    itemCategory = __eSupplyItemType_Alchemy
  end
  local sellData = {
    tradeType = tradeType,
  ...
  if false == isLinkedNode and 0 ~= regionInfo._waypointKey then
    ...
    local rv
    if 5 == tradeType then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eGuildWarehouse, itemCategory, 0)
    elseif PaGlobal_TradeMyBasket_All._ENUM_INVENTORY == itemData.itemFromWhere then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eMoneyInventory, itemCategory, 0)
    elseif PaGlobal_TradeMyBasket_All._ENUM_FISH_BAG == itemData.itemFromWhere then
    elseif PaGlobal_TradeMyBasket_All._ENUM_LAND_VEHICLE == itemData.itemFromWhere then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eMoneyInventory, itemCategory, 0)
    elseif PaGlobal_TradeMyBasket_All._ENUM_SEA_VEHICLE == itemData.itemFromWhere then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eMoneyInventory, itemCategory, 1)
    end
  end
end
function PaGlobalFunc_TradeMyBasket_All_SellComplete()
  ...
  local itemCategory = sellData.itemCategory
  local rv
  if 5 == sellData.tradeType then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eGuildWarehouse, itemCategory, 0)
  elseif PaGlobal_TradeMyBasket_All._ENUM_INVENTORY == itemFromWhere then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eMoneyInventory, itemCategory, 0)
  ...
  elseif PaGlobal_TradeMyBasket_All._ENUM_SEA_VEHICLE == itemFromWhere then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eMoneyInventory, itemCategory, 1)
  end
  if 0 == rv then
...
function PaGlobalFunc_TradeMyBasket_All_SellAllContinue()
  ...
  for goodsIdx = 0, myInventortySellAbleCount do
    ...
    if nil ~= shopItemWrapper then
      local itemCommerceType = shopItemWrapper:getStaticStatus():getCommerceType()
      local itemCategory = __eSupplyItemType_Count
      if itemCommerceType == enCommerceType.enCommerceType_Grocery then
        itemCategory = __eSupplyItemType_Cook
      elseif itemCommerceType == enCommerceType.enCommerceType_Medicine then
      ...
      s64_inventoryItemCount = inventory:getItemCountByItemNo_s64(s64_TradeItemNo)
      if 5 == tradeType then
        rv = npcShop_doSellInTradeShop(goodsIdx, Int64toInt32(s64_inventoryItemCount), 0, 14, itemCategory, 0)
        ...
        rv = npcShop_doSellInTradeShop(goodsIdx, Int64toInt32(s64_inventoryItemCount), 0, toWhere, itemCategory, 0)
        sellCount = sellCount + 1
      end
    end
  end
  ...
  for fishIdx = 0, myFishBagSellAbleCount - 1 do
    ...
    if shopItemWrapper ~= nil then
      ...
      s64_inventoryItemCount = toInt64(0, 1)
      if 5 == tradeType then
        rv = npcShop_doSellInTradeShop(fishIdx, Int64toInt32(s64_inventoryItemCount), CppEnums.ItemWhereType.eInventoryBag, 14, itemCategory, 0)
        priceIndex = priceIndex + 1
        sellCount = sellCount + 1
      else
        local realPrice = PaGlobal_TradeMyBasket_All._realPriceList[itemIdx]
        ...
        rv = npcShop_doSellInTradeShop(fishIdx, Int64toInt32(s64_inventoryItemCount), CppEnums.ItemWhereType.eInventoryBag, CppEnums.ItemWhereType.eMoneyInventory, itemCategory, 0)
        priceIndex = priceIndex + 1
        sellCount = sellCount + 1
      end
    end
          ...
          if not servertinventory:empty(slotCount) then
            ...
            if nil ~= s64_VehicleItemNo then
              ...
              if nil ~= servantItemWrapper then
                local realPrice = PaGlobal_TradeMyBasket_All._realPriceList[itemIdx]
                local itemCommerceType = servantItemWrapper:getStaticStatus():getCommerceType()
                local itemCategory = __eSupplyItemType_Count
                if itemCommerceType == enCommerceType.enCommerceType_Grocery then
                  itemCategory = __eSupplyItemType_Cook
                elseif itemCommerceType == enCommerceType.enCommerceType_Medicine then
                ...
                local limitCount = math.floor((PaGlobal_TradeMyBasket_All._LIMITED_SELLMONEY - myInvenMoney) / realPrice)
                priceIndex = priceIndex + 1
                rv = npcShop_doSellInTradeShop(slotCount - 2 - emptyCount, vehicleItemCount, 4, CppEnums.ItemWhereType.eMoneyInventory, itemCategory, servantType)
                sellCount = sellCount + 1
              end
            end
          else
