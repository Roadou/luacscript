function PaGlobal_TradeMarketItemList_All:initialize()
  ...
  self._ui._txt_NoItem = UI.getChildControl(Panel_Dialog_Trade_MarketItemList_All, "StaticText_NoItem")
...
function PaGlobal_TradeMarketItemList_All:updateCategoryButtonData(contents, key)
  ...
  if categoryIndex32 == enCommerceType.enCommerceType_Max then
    rdoButton:ChangeTextureInfoTextureIDAsync("Combine_Frame_GradationSlot_Normal_Point")
    rdoButton:setRenderTexture(rdoButton:getBaseTexture())
  end
...
function PaGlobal_TradeMarketItemList_All:update()
  ...
  if true == isTerritorySupplyCheck then
    ...
    if true == characterStaticStatus:isSupplyMerchant() then
      ...
      if self._currentCategoryIdx == enCommerceType.enCommerceType_Grocery then
        categoryListIdx = __eSupplyItemType_Cook
      elseif self._currentCategoryIdx == enCommerceType.enCommerceType_Medicine then
        categoryListIdx = __eSupplyItemType_Alchemy
      end
      local leftCount = tostring(ToClient_getTradeSupplyLeftCount(self._currShopType, categoryListIdx))
  ...
  if nil ~= enCommerceType.enCommerceType_Max and nil ~= Panel_Dialog_Scene then
    if self._currentNpcType == 2 then
      self._mySellableItem = PaGlobalFunc_TradeMarketItemList_All_getMySellableItem()
      self._ui._list2_Category:getElementManager():pushKey(toInt64(0, enCommerceType.enCommerceType_Max))
      self._currentCategoryIdx = enCommerceType.enCommerceType_Max
    end
...
function PaGlobal_TradeMarketItemList_All:getItemListByCurrentCategory()
  ...
  for ii = 1, enCommerceType.enCommerceType_Max do
  ...
  if 1 == self._currentNpcType then
    ...
    if currentCategoryIdx == enCommerceType.enCommerceType_Max then
      commerceCount = #self._mySellableItem
    else
  ...
  local isCommecrceEmpty = commerceCount == 0
  self._ui._txt_NoItem:SetShow(isCommecrceEmpty == true)
  if self._currentNpcType == 1 then
    self._ui._list2_TradeItem_Buy:SetShow(isCommecrceEmpty == false)
  else
    self._ui._list2_TradeItem_Sell:SetShow(isCommecrceEmpty == false)
  end
  for itemIdx = 0, commerceCount - 1 do
    local itemKey = 0
    if currentCategoryIdx == enCommerceType.enCommerceType_Max then
      itemKey = self._mySellableItem[itemIdx + 1]
    else
      itemKey = npcShop_GetCommerceItemByIndexAndSellOrBuy(currentCategoryIdx, self._currentNpcType, itemIdx)
    end
...
function PaGlobal_TradeMarketItemList_All:setMoneyTexture(control)
  ...
  if __eTradeSupplyShopType_Shell == self._currShopType then
    ...
    control:ChangeTextureInfoNameAsync("combine/icon/combine_basic_icon_00.dds")
