function HandleEventLUp_Warehouse_All_PopMoney()
  ...
  if isGuildWare == true then
    Proc_ShowMessage_Ack(PAGetStringSymNo(930905507))
    return
  end
...
function HandleEventLUp_Warehouse_All_LDownClick(index)
  ...
  local Execute_EasyBuy = function()
    PaGlobalFunc_EasyBuy_Open(17, PaGlobal_WareHouse_All.savedWayPointKey)
  end
...
function FromClient_RemoveAllGuildBuffInstallation()
  if PaGlobal_WareHouse_All_GetShow() == false then
    return
  end
  Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoUpdateGuildWarehouseAndCloseUi"))
  PaGlobal_Warehouse_All_Close()
end
...
function PaGlobal_Warehouse_All_GetInstallationActorKeyRaw()
  return PaGlobal_WareHouse_All._installationActorKeyRaw
end
...
function PaGlobal_Warehouse_All_MoveNonStackItemCheck()
  return PaGlobal_WareHouse_All:isNpc() == true or PaGlobal_WareHouse_All:isMaid() == true or PaGlobal_WareHouse_All:isGuildHouse() == true or PaGlobal_WareHouse_All:isInstallation() == true
end
function PaGlobal_Warehouse_All_IsGuildHouse()
  if PaGlobal_WareHouse_All == nil then
    return false
  end
  return PaGlobal_WareHouse_All:isGuildHouse()
end
...
function PaGlobal_Warehouse_All_PopToSomewhere(s64_count, slotNo, toActorKeyRaw)
  ...
  elseif toActorKeyRaw == getSelfPlayer():getActorKey() then
    ...
    if nil ~= itemWrapper then
      ...
      if true == _ContentsGroup_MoveNonStackItem and false == itemSSW:isMoney() and false == itemSSW:isStackable() and true == PaGlobal_Warehouse_All_MoveNonStackItemCheck() then
        local whereType = CppEnums.ItemWhereType.eWarehouse
        if PaGlobal_WareHouse_All:isGuildHouse() == true then
          whereType = CppEnums.ItemWhereType.eGuildWarehouse
        end
        local nonStackItemCount = ToClient_getNonStackItemCount(whereType, slotNo, false, PaGlobal_WareHouse_All._currentWaypointKey)
  ...
  local isUseMaid = PaGlobal_WareHouse_All:isGuildHouse() == true and ToClient_checkGuildWarehouseUseMaid() == true
  if isUseMaid == true then
    ...
    local warehouseWrapper = PaGlobal_WareHouse_All:getWarehouse()
    local itemWrapper = warehouseWrapper:getItem(slotNo)
    s64_count = ToClient_getMoveCountWhenUseMaid(itemWrapper:getStaticStatus():getItemKey(), s64_count, true)
    if s64_count == Defines.s64_const.s64_0 then
      return
    end
  end
...
function PaGlobal_Warehouse_All_PopItemFromItemSearch(itemCount_s64, slotNo, actorKey, waypointKey, moveCountOnce)
  if moveCountOnce == nil then
    ToClient_popNonStackItemFromWarehouse(actorKey, true, itemCount_s64, waypointKey)
  else
    warehouse_popToInventoryByMaid(slotNo, itemCount_s64, actorKey, moveCountOnce, waypointKey)
  end
...
function PaGlobal_Warehouse_All_GetCurrentWaypoint()
  return PaGlobal_WareHouse_All._selectWaypointKey
end
function PaGlobal_Warehouse_All_ResetCurrentWaypoint()
  PaGlobal_WareHouse_All._selectWaypointKey = getCurrentWaypointKey()
end
...
function PaGlobal_Warehouse_All_GetFromType()
  return PaGlobal_WareHouse_All._fromType
end
