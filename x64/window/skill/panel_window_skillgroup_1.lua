function PaGlobal_SkillGroup:init()
  ...
  Panel_Window_SkillGroup:registReloadEvent(__eUIReloadEventType_SwapCharacter)
end
...
function PaGlobal_SkillGroup:updateViewController(isWindow, isForced)
  ...
  if false == isWindow then
    ...
    PaGlobal_SkillGroup_Controller:open(Panel_Window_SkillGroup)
...
function PaGlobal_SkillGroup:controlInit_All()
  ...
  if false == _ContentsGroup_NewUI_BlackSpiritSkillLock_All or ToClient_HardCoreChannelWithContensOption() == true then
...
function PaGlobal_SkillGroup:createUpgradeSkillListContent(content, key)
  ...
  if nil ~= skillSSW then
    ...
    if btn_reinforceSkill:GetShow() == true then
      ...
      btn_reinforceSkill:addInputEvent("Mouse_LUp", "PaGlobalFunc_PaGlobal_SkillGroup_All_OpenReinforce(" .. playerSkillType .. ", " .. skillNo .. "," .. index .. "," .. self._reinforceIndex .. ")")
      self._reinforceIndex = self._reinforceIndex + 1
      btn_skillChange:SetShow(true)
      btn_skillChange:addInputEvent("Mouse_LUp", "PaGlobalFunc_PaGlobal_SkillGroup_All_ChangeReinforce(" .. playerSkillType .. ", " .. skillNo .. "," .. index .. "," .. self._reinforceIndex .. ")")
    end
  else
    ...
    if btn_reinforceSkill:GetShow() == true then
      ...
      btn_reinforceSkill:addInputEvent("Mouse_LUp", "PaGlobalFunc_PaGlobal_SkillGroup_All_OpenReinforceNew(" .. playerSkillType .. "," .. self._reinforceIndex .. ")")
    end
  end
end
function PaGlobal_SkillGroup:open()
  ...
  if playerLevel >= 50 and ToClient_HardCoreChannelWithContensOption() == false then
...
function PaGlobal_SkillGroup:updateStat()
  ...
  local skillExpRate
  local _skillPoint = tonumber(selfPlayer:getRemainSkillPoint())
  local skillPointNeedExp = selfPlayer:getSkillPointNeedExperience()
  if 0 ~= skillPointNeedExp then
    skillExpRate = selfPlayer:getSkillPointExperience() / skillPointNeedExp
  end
...
function PaGlobal_SkillGroup:selectTab(index)
  ...
  if self._tab[self.eTabIndex.upgrade]:GetShow() == true then
    if index == self.eTabIndex.upgrade then
      ...
      if _ContentsGroup_DarkMoonAdditionalBuff == true then
        local selfPlayer = selfPlayerWrapper:get()
        if selfPlayer ~= nil and selfPlayer:isApplyChargeSkill(CppEnums.UserChargeType.eUserChargeType_BookOfCombat) == true then
          self._findSkillNpcButton:SetShow(false)
        end
      end
      self:refreshUpdateSkillList()
  ...
  for ii = 1, maxSkillCCTypeCount - 1 do
    if true == skillSSW:isSetSkillCCType(ii - 1) and nil ~= skillCCTypeConfig.uv[ii] then
      if idxOfSkillCCType < 6 then
        ...
        control:ChangeTextureInfoNameAsync(skillCCTypeConfig.iconPath)
...
function PaGlobal_SkillGroup:registEventHandler()
  ...
  registerEvent("FromClient_SuccessSkillAwaken", "FromClient_SuccessSkillReinforceInSkillGroup")
  registerEvent("FromClient_ChangeSkillAwakeningBitFlag", "FromClient_SuccessSkillReinforceInSkillGroup")
  registerEvent("FromClient_ChangeAwakenSkill", "FromClient_SuccessSkillReinforceInSkillGroup")
end
...
function PaGlobal_SkillGroup:selectTree(index)
  ...
  if PaGlobal_TotalPreset_UpdateClassType ~= nil then
    PaGlobal_TotalPreset_UpdateClassType()
  end
end
