...
function PaGlobal_SkillFilter_MouseLUpSelectFilter(key)
  ...
  if _ContentsGroup_ShowOtherClassSkill == true then
    PaGlobalFunc_SkillGroup_All_UpdateSkillFilter()
  else
...
function PaGlobalFunc_SkillFilter_All_ListControlCreate(control, key)
  ...
  if nil ~= self._skillCCTypeConfig._uv[key32] then
    ...
    stc_icon:ChangeTextureInfoNameAsync(self._skillCCTypeConfig._iconPath)
...
function PaGlobalFunc_SkillFilter_All_isFilterSkill(skillSSW, skillNo)
  ...
  elseif 1 == self._selectFilterType then
    ...
    elseif Panel_Window_SkillGroup ~= nil and Panel_Window_SkillGroup:GetShow() == true and blackSkillTypeSS ~= nil and blackSkillTypeSS:isValidLocalizing() == true and blackSkillTypeSS:isUsableClass() == true then
      return true
    end
