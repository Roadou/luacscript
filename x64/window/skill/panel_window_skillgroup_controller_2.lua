function PaGloabl_SkillGroup_ShowDesc(skillNo, isBlackSpirit)
  ...
  if nil ~= skillInfo then
    ...
    local TooltipYPos = 0
    local elementGap = 10
    ui._stc_skillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillInfo._iconPath)
    ...
    if true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI then
      isShowCommand = "" ~= skillInfo._consoleCommand
    end
    ui._txt_skillCommandDesc:SetShow(isShowCommand)
    if true == isShowCommand then
      if true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI then
        ui._txt_skillCommandDesc:SetText(skillInfo._consoleCommand)
      else
    ...
    if true == self._isShowBlackSpiritSkill then
      ...
      local remainSkillPoint = 0
      if _ContentsGroup_ShowOtherClassSkill == true then
        remainSkillPoint = PaGlobal_SkillGroup_All._remainSkillPoint
      else
        remainSkillPoint = PaGlobal_SkillGroup._remainSkillPoint
      end
      if remainSkillPoint >= skillInfo._needSkillPoint then
        needResource = needResource .. "<PAColor0xFFEEEEEE>"
      else
        needResource = needResource .. "<PAColor0xFFDDA309>"
      end
    ...
    if false == PaGlobal_SkillGroup_Controller._isConsole then
    ...
    local presetSizeX = 0
    if _ContentsGroup_ShowOtherClassSkill == true then
      presetSizeX = PaGlobal_SkillGroup_All._ui._stc_presetBG:GetSizeX()
    else
      presetSizeX = PaGlobal_SkillGroup._skillPreset.stc_presetBG:GetSizeX()
    end
    local needResourcePosX = 0
    if _ContentsGroup_ShowOtherClassSkill == true then
      needResourcePosX = getScreenSizeX() - Panel_Window_SkillGroup_All:GetSizeX() - ui._txt_skillNeedResource:GetTextSizeX() - presetSizeX - 100
    else
      needResourcePosX = getScreenSizeX() - Panel_Window_SkillGroup:GetSizeX() - ui._txt_skillNeedResource:GetTextSizeX() - presetSizeX - 100
    end
    local leftDescSize = needResourcePosX - ui._frame_skillDesc:GetPosX() - 10
    ui._txt_skillEffectDesc:SetText(skillInfo._buffDesc)
    ui._frame_skillDesc:SetSize(leftDescSize, ui._txt_skillEffectDesc:GetTextSizeY())
    ui._frame_content:SetSize(leftDescSize, ui._txt_skillEffectDesc:GetTextSizeY())
    ui._txt_skillEffectDesc:SetSize(leftDescSize, ui._txt_skillEffectDesc:GetTextSizeY())
    ui._frame_skillDesc:ComputePosAllChild()
    ui._txt_skillEffectTitle:SetPosY(ui._frame_skillDesc:GetPosY() - ui._txt_skillEffectDesc:GetTextSizeY() + ui._txt_skillEffectDesc:GetSizeY() - ui._txt_skillEffectTitle:GetSizeY() - 10)
    if true == PaGlobal_SkillGroup_Controller._isConsole then
      local topEndPosY = ui._txt_skillCommandDesc:GetPosY() + ui._txt_skillCommandDesc:GetTextSizeY()
      local bottomStartPosY = ui._txt_skillEffectTitle:GetPosY()
      local gap = 30
      if bottomStartPosY < topEndPosY + gap then
        local diff = topEndPosY + gap - bottomStartPosY
        ui._frame_skillDesc:SetSize(ui._frame_skillDesc:GetSizeX(), ui._frame_skillDesc:GetSizeY() - diff)
        ui._frame_skillDesc:ComputePos()
        ui._txt_skillEffectTitle:SetPosY(ui._txt_skillEffectTitle:GetPosY() + diff)
        UIScroll.SetButtonSize(ui._frame_vscroll, ui._frame_skillDesc:GetSizeY(), ui._frame_content:GetSizeY())
        ui._txt_keyGuide_Scroll:SetShow(true)
        ui._txt_keyGuide_Scroll:ComputePos()
        ui._frame_vscroll_ctrlButton:SetAlpha(1)
        PaGlobal_SkillGroup_Controller._changedScroll = false
        PaGlobal_SkillGroup_Controller._checkHideTime = 0
      else
        ui._txt_keyGuide_Scroll:SetShow(false)
      end
    end
    ui._frame_skillDesc:UpdateContentScroll()
    ui._frame_vscroll:SetControlTop()
    ui._frame_skillDesc:UpdateContentPos()
    ui._txt_skillNeedResource:ComputePos()
    ui._txt_skillNeedResource:SetPosX(needResourcePosX)
    ui._txt_skillNeedResource:SetPosY(ui._frame_skillDesc:GetPosY() - ui._txt_skillNeedResource:GetTextSizeY() + ui._frame_skillDesc:GetSizeY())
...
function PaGlobal_SkillGroup_Controller_UpdatePerFrame(deltaTime)
  if nil == Panel_Window_SkillGroup_Controller then
    return
  end
  if false == PaGlobal_SkillGroup_Controller._changedScroll then
    return
  end
  PaGlobal_SkillGroup_Controller._checkHideTime = PaGlobal_SkillGroup_Controller._checkHideTime + deltaTime
  if PaGlobal_SkillGroup_Controller._checkHideTime >= 0.5 then
    local alpha = (1 - PaGlobal_SkillGroup_Controller._checkHideTime) * 2
    PaGlobal_SkillGroup_Controller._ui._frame_vscroll_ctrlButton:SetAlpha(alpha)
  elseif PaGlobal_SkillGroup_Controller._checkHideTime >= 1 then
    PaGlobal_SkillGroup_Controller._changedScroll = false
  end
end
function PaGlobal_SkillGroup_Controller_ScrollEffectDesc(isUp)
  local vscroll = PaGlobal_SkillGroup_Controller._ui._frame_vscroll
  if nil == vscroll then
    return
  end
  local isShow = vscroll:GetShow()
  if false == isShow then
    return
  end
  if true == isUp then
    vscroll:ControlButtonUp()
  else
    vscroll:ControlButtonDown()
  end
  PaGlobal_SkillGroup_Controller._ui._frame_skillDesc:UpdateContentPos()
  PaGlobal_SkillGroup_Controller._ui._frame_vscroll_ctrlButton:SetAlpha(1)
  PaGlobal_SkillGroup_Controller._changedScroll = true
  PaGlobal_SkillGroup_Controller._checkHideTime = 0
end
...
function PaGlobal_SkillGroup_Controller_Resize()
  ...
  panel:ComputePos()
  if _ContentsGroup_ShowOtherClassSkill == true then
    panel:SetSize(getScreenSizeX() - Panel_Window_SkillGroup_All:GetSizeX() - 10, getScreenSizeY())
  else
...
function HandleClicked_SkillGroupController_SetCharacterRotate_End(isLDown)
  ...
  if true == isLDown then
    ...
    elseif PaGlobal_SkillGroup._isDraggingFromTree == false and DragManager.groundClickFunc ~= nil then
      DragManager.groundClickFunc(DragManager.dragWhereTypeInfo, DragManager.dragSlotInfo)
      if DragManager.dragStartPanel == Panel_Window_SkillGroup_QuickSlot then
        DragManager:clearInfo()
        PaGlobal_SkillGroup_QuickSlot:update()
