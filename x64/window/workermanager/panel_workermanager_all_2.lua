...
function PaGlobalFunc_WorkerManager_All_ComboBoxResetAdd()
  ...
  for ii = 1, #PaGlobal_WorkerManager_All._townList do
    local waypointKey = PaGlobal_WorkerManager_All._townList[ii]
    local townText = ""
    if _ContentsGroup_WorkerImprovement == true then
      ...
      townText = ToClient_GetNodeNameByWaypointKey(waypointKey) .. " (" .. waitWorkerCount .. "/" .. maxWorkerCount .. ")"
    else
      townText = ToClient_GetNodeNameByWaypointKey(waypointKey)
    end
    PaGlobal_WorkerManager_All._ui.combo_town:AddItem(townText)
  end
  ...
  if -1 ~= PaGlobal_WorkerManager_All._selectFilterTown and 0 < PaGlobal_WorkerManager_All._selectFilterTown then
    local waypointKey = PaGlobal_WorkerManager_All._townList[PaGlobal_WorkerManager_All._selectFilterTown]
    local townText = ""
    if _ContentsGroup_WorkerImprovement == true then
      local plantkey = ToClient_convertWaypointKeyToPlantKey(waypointKey)
      local waitWorkerCount = ToClient_getPlantWaitWorkerListCount(plantkey, 0)
    ...
    PaGlobal_WorkerManager_All._ui.combo_town:SetText(townText)
  else
...
function HandleEventLUp_WorkerManager_All_SetComboTown()
  ...
  if PaGlobal_WorkerManager_All._TAB.WORKER == PaGlobal_WorkerManager_All._selectTabIdx then
...
function HandleEventLUp_WorkerManager_All_UpgradeNow(workerIdx)
  if nil == Panel_Window_WorkerManager_All or false == Panel_Window_WorkerManager_All:IsShow() then
    return
  end
  PaGlobalFunc_WorkerManagerTooltip_All_Close()
  PaGlobal_WorkerManager_All:upgradeWorkerNow(workerIdx)
  PaGlobal_WorkerManager_All:update()
end
...
function HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(isShow, isUpgradable, key, isUpgrading)
  if nil == Panel_Window_WorkerManager_All then
    return
  end
  if true == isShow then
    if _ContentsGroup_WorkerImprovement == true and isUpgrading ~= nil and isUpgrading == true then
      PaGlobal_WorkerManager_All._ui_console.stc_normal_detail:SetText(PaGlobal_WorkerManager_All._keyGuideA.COMPLETENOW)
      PaGlobal_WorkerManager_All._ui_console.stc_normal_detail:SetShow(true)
    elseif true == isUpgradable then
...
function PaGlobalFunc_WorkerManager_All_UpdateNode(plantKey)
  local regionKey = ToClient_GetRegionKeyRawByWaypointKey(plantKey:getWaypointKey())
  if ToClient_checkHireWorkerByRegionKey(regionKey) == false then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantManageWorkerAtRegion"))
    return
  end
