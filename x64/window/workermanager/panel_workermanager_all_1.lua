...
function PaGlobal_WorkerManager_All:ControlAll_Init()
  ...
  if _ContentsGroup_WorkerImprovement == true then
    self._ui.stc_selectedWorkerSlot:SetShow(false)
    self._ui.list2_advence:SetPosY(self._ui.stc_selectedWorkerSlot:GetPosY())
    self._ui.list2_advence:SetSize(self._ui.list2_advence:GetSizeX(), self._ui.list2_worker:GetSizeY())
  else
    self._ui.stc_selectedWorkerSlot:SetShow(true)
  end
end
...
function PaGlobal_WorkerManager_All:ControlSetShow()
  ...
  if true == self._isPadSnap then
    ...
    local townComboBackStatic = UI.getChildControl(self._ui.combo_townlist, "Combobox_List_Template")
    townComboBackStatic:SetSize(comboSizeForPad, townComboBackStatic:GetSizeY())
    local gradeComboBackStatic = UI.getChildControl(self._ui.combo_gradelist, "Combobox_List_Template")
...
function PaGlobal_WorkerManager_All:registEventHandler()
  ...
  registerEvent("WorldMap_WorkerDataUpdateByPalaceManage", "FromClient_WorkerManager_All_WorkerDataUpdate")
...
function PaGlobal_WorkerManager_All:updateUpgradeData()
  ...
  self._ui.stc_image:ChangeTextureInfoNameAsync(workerWrapperLua:getWorkerIcon())
...
function PaGlobal_WorkerManager_All:upgradeWorkerNow(workerIdx)
  local workerNoRaw = 0
  if _ContentsGroup_WorkerImprovement == true and workerIdx ~= nil then
    workerNoRaw = self._filteredArray[workerIdx]
  else
    workerNoRaw = self._upgradingWorkerNoRaw
  end
...
local SetWorkingStateDesc = function(parentControl, workerWrapperLua)
  ...
  if _ContentsGroup_UsePadSnapping == true then
    if txt_startTown:IsLimitText() == true then
      txt_startTown:SetIgnore(true)
    end
    if txt_storageTown:IsLimitText() == true then
      txt_storageTown:SetIgnore(true)
    end
  end
end
...
function PaGlobal_WorkerManager_All:workerlistCreate(content, key)
  ...
  stc_image:ChangeTextureInfoNameAsync(workerWrapperLua:getWorkerIcon())
  ...
  if _ContentsGroup_WorkerImprovement == false and index == self._upgradingWorkerNoRaw or _ContentsGroup_WorkerImprovement == true and workerWrapperLua:getWorkingType() == CppEnums.NpcWorkingType.eNpcWorkingType_Upgrade then
  ...
  if true == self._isPadSnap then
    ...
    if txt_name:IsLimitText() == true then
      txt_name:SetIgnore(true)
    end
    txt_town:SetTextMode(__eTextMode_LimitText)
    txt_town:SetText(txt_town:GetText())
    if txt_town:IsLimitText() == true then
      txt_town:SetIgnore(true)
    end
    txt_working:SetTextMode(__eTextMode_LimitText)
    txt_working:SetText(txt_working:GetText())
    if txt_working:IsLimitText() == true then
      txt_working:SetIgnore(true)
    end
...
function PaGlobal_WorkerManager_All:upgradelistCreate(content, key)
  ...
  btn_advence:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_ARSHA_SUBMENU_UPGRADE"))
  stc_stop.btn:addInputEvent("Mouse_LUp", "")
  stc_image:addInputEvent("Mouse_On", "HandleEventOnOut_WorkerManager_WorkerInfoTooltip(true," .. tostring(key32) .. ")")
  stc_image:addInputEvent("Mouse_Out", "HandleEventOnOut_WorkerManager_WorkerInfoTooltip(false," .. tostring(key32) .. ")")
  ...
  local isUpgrading = workerWrapperLua:getWorkingType() == CppEnums.NpcWorkingType.eNpcWorkingType_Upgrade
  ...
  if _ContentsGroup_WorkerImprovement == true then
    if workerUpgradeCount > 0 and workerWrapperLua:isUpgradable() == true then
      if true ~= isWorking then
        isUpgradable = true
      else
        isUpgradable = false
  ...
  elseif workerUpgradeCount > 0 and workerWrapperLua:isUpgradable() == true and self._upgradingWorkerNoRaw == -1 then
    if true ~= isWorking then
      isUpgradable = true
    else
      isUpgradable = false
  ...
  if true == self._isPadSnap then
    ...
    if _ContentsGroup_WorkerImprovement == true and isUpgrading == true then
      btn_advence:SetMonoTone(false, true)
      btn_advence:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLE_INFO_BTN_MATINGIMMEDIATELY"))
      stc_btnBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "HandleEventLUp_WorkerManager_All_UpgradeNow(" .. tostring(key32) .. ")")
      stc_btnBg:addInputEvent("Mouse_On", "HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(true, false," .. tostring(key32) .. ",true)")
      stc_btnBg:addInputEvent("Mouse_Out", "HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(false, false," .. tostring(key32) .. ",true)")
    end
  else
    UI.setLimitTextAndAddTooltip(txt_name)
    UI.setLimitTextAndAddTooltip(txt_town)
    ...
    if _ContentsGroup_WorkerImprovement == true and isUpgrading == true then
      btn_advence:SetMonoTone(false, true)
      btn_advence:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLE_INFO_BTN_MATINGIMMEDIATELY"))
      btn_advence:addInputEvent("Mouse_LUp", "HandleEventLUp_WorkerManager_All_UpgradeNow(" .. tostring(key32) .. ")")
      btn_advence:addInputEvent("Mouse_On", "")
      btn_advence:addInputEvent("Mouse_Out", "")
    end
  end
  SetWorkingStateDesc(stc_btnBg, workerWrapperLua)
end
