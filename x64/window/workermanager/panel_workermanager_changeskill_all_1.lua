...
function PaGlobal_WorkerManagerChangeSkill_All:update()
  ...
  for skillIdx = 0, skillCount - 1 do
    ...
    self._skillList[skillIdx].icon:ChangeTextureInfoNameAsync(skillSSW:getIconPath())
...
function PaGlobal_WorkerManagerChangeSkill_All:setEffectOnNewSkill(workerNoRaw, workerSkillKeyRaw)
  if Panel_Window_WorkerManagerChangeSkill_All == nil then
    return
  end
  ...
  for skillIdx = 0, skillCount - 1 do
    local skillSSW = workerWrapperLua:getSkillSSW(skillIdx)
    local skillKeyRaw = skillSSW:getKeyRaw()
    if skillKeyRaw == workerSkillKeyRaw then
      self._skillList[skillIdx].bg:EraseAllEffect()
      self._skillList[skillIdx].bg:AddEffect("fUI_NewSkill_Worker_ChangeSkill_01A", false, 0, 0)
      return
    end
  end
end
