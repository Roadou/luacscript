...
function pearlShopProductInfo:update()
  ...
  local itemListTotalCount = itemListCount + additionalSubItemCount
  ...
  if itemListTotalCount > self._ui._itemSlotControlListCountPerLine then
    itemListLineCount = math.floor(itemListTotalCount / self._ui._itemSlotControlListCountPerLine) + 1
  end
  local firstPosX = self._ui._itemSlotBgTemplateControl:GetPosX() - 0.5 * (self._ui._itemSlotBgTemplateControl:GetSizeX() + 5) * (self._ui._itemSlotControlListCountPerLine - 1)
  for i = 0, self._ui._itemSlotControlListSize - 1 do
    local control = self._ui._itemSlotControlList[i]
    control:SetPosX(firstPosX + (self._ui._itemSlotBgTemplateControl:GetSizeX() + self._slotGapX) * math.floor(i % self._ui._itemSlotControlListCountPerLine))
    ...
    if showFlag then
      ...
      if i == self._focusedIndex then
        ...
        if true == self._isChooseProduct then
          ...
          self._ui._bannerControl:ChangeTextureInfoNameAsync(info:getPackageIcon())
        else
