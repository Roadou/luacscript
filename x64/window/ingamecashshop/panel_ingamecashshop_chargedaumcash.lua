...
function chargeDaumCash_Open()
  ...
  if _ContentsGroup_PearlBoxWebBuy == true then
    ToClient_PurchasePageAutoLogin(__eRequestEncryteKeyType_Purchase)
    return
  end
...
function ChangeCashOpen()
  ...
  if false == ToClient_PurchasePageAutoLogin(__eRequestEncryteKeyType_None) then
...
function FromClient_OpenPearlDirectWeb(url, type, restoreCount)
  if type == __eRequestEncryteKeyType_Purchase then
    FGlobal_Window_PearlBox_Purchase_All_Show(url, restoreCount)
  end
end
...
registerEvent("FromClient_OpenPearlDirectWeb", "FromClient_OpenPearlDirectWeb")
