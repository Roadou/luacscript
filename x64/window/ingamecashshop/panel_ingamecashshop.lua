...
local scrollAreaSizeRatio = 0.98
...
local inGameShop = {
  ...
  _static_Bottom_PurchaseArea = UI.getChildControl(Panel_IngameCashShop, "Static_Bottom_PurchaseArea"),
...
inGameShop._staticText_PearlYello = UI.getChildControl(inGameShop._static_Bottom_PurchaseArea, "StaticText_PearlYellow")
inGameShop._staticText_PearlValue = UI.getChildControl(inGameShop._staticText_PearlYello, "StaticText_PearlValue")
inGameShop._staticText_MileagePurple = UI.getChildControl(inGameShop._static_Bottom_PurchaseArea, "StaticText_MilieagePurple")
inGameShop._staticText_MileageValue = UI.getChildControl(inGameShop._staticText_MileagePurple, "StaticText_MileageValue")
inGameShop._btn_PurchasePearl = UI.getChildControl(inGameShop._static_Bottom_PurchaseArea, "Button_PurchasePearl")
...
local function tag_changeTexture(slot, tagType)
  ...
  if tagType >= 8 then
    control:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/CashShop_05.dds")
  elseif 7 == tagType then
    control:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/CashShop_04.dds")
  else
    control:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/CashShop_03.dds")
  end
...
local function sale_changeTexture(slot)
  ...
  control:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/CashShop_SoldOutBanner_00.dds")
...
function cashIcon_changeTexture(control, contry)
  control:ChangeTextureInfoNameAsync("Combine/Icon/Combine_Cashshop_Icon_00.dds")
...
function cashIcon_changeTextureForList(control, contry, iconType)
  ...
  control:ChangeTextureInfoNameAsync(cashIconPath)
...
function inGameShop:init()
  ...
  for ii = 0, maxSlotCount - 1 do
    ...
    slot.hasCoupon = UI.createAndCopyBasePropertyControl(Panel_IngameCashShop, "TemplateList_Static_Coupon", slot.static, "InGameShop_Slot_HasCoupon_" .. ii)
    ...
    slot.discount:addInputEvent("Mouse_LUp", "IngameCashShop_SelectedItem(" .. ii .. ", 0" .. ")")
    slot.discount:addInputEvent("Mouse_UpScroll", "InGameShop_ScrollEvent( true )")
    slot.discount:addInputEvent("Mouse_DownScroll", "InGameShop_ScrollEvent( false )")
    slot.name:addInputEvent("Mouse_LUp", "IngameCashShop_SelectedItem(" .. ii .. ", 0" .. ")")
    slot.name:addInputEvent("Mouse_UpScroll", "InGameShop_ScrollEvent( true )")
    slot.name:addInputEvent("Mouse_DownScroll", "InGameShop_ScrollEvent( false )")
    ...
    slot.price:addInputEvent("Mouse_LUp", "IngameCashShop_SelectedItem(" .. ii .. ", 0" .. ")")
...
function inGameShop:serviceTypeCheckControl()
  ...
  if _ContentsGroup_PearlBoxWebBuy == true then
    self._btn_HowUsePearl:SetShow(false)
    self._nowCash:SetShow(false)
    self._haveCashBoxBG:SetShow(false)
    self._btn_BuyDaum:SetShow(false)
    self._pearlBox:SetShow(false)
    self._silverBox:SetShow(false)
    self._silver:SetShow(false)
    ...
    self._cashBox:SetShow(false)
    self._ch_banner:SetShow(false)
    self._static_Bottom_PurchaseArea:SetShow(true)
  end
end
...
function inGameShop:updateMoney()
  ...
  if _ContentsGroup_PearlBoxWebBuy == true then
    self._staticText_PearlValue:SetText(makeDotMoney(pearl))
    self._staticText_MileageValue:SetText(makeDotMoney(mileage))
  else
...
function inGameShop:setElement(index, productNoRaw, slot)
  ...
  if nil == cashProduct:getPackageIcon() then
    slot.productIcon:ChangeTextureInfoNameAsync(nilProductIconPath)
  else
    slot.productIcon:ChangeTextureInfoNameAsync(cashProduct:getPackageIcon())
  end
  ...
  if nil == cashProduct:getIconPath() then
    slot.icon:ChangeTextureInfoNameAsync("Icon/" .. nilIconPath)
  else
    slot.icon:ChangeTextureInfoNameAsync("Icon/" .. cashProduct:getIconPath())
  end
  ...
  slot.hasCoupon:SetShow(cashProduct:hasCoupon())
  return true
end
  ...
  for ii = 0, self._listCount - 1 do
    ...
    if self:isSelectProductGroup(productNoRaw) then
      ...
      if self._currentPos < pos + slot.static:GetSizeY() + self._goodDescBG:GetSizeY() and pos + slot.static:GetSizeY() < self._currentPos + areaSizeY and 1 < self._goodDescBG:GetSizeY() then
        ...
        self.desc._btn_BigBuy:SetSpanSize(20, 7)
        self.desc._btn_BigBuy_C:SetSpanSize(20, 7)
        self.desc._btn_BigBuy_M:SetSpanSize(20, 7)
        self.desc._btn_BigBuy_Silver:SetSpanSize(20, 7)
        ...
        local buttonListForSpan = Array.new()
        if self.desc._btn_BigCart:GetShow() then
          buttonListForSpan:push_back(self.desc._btn_BigCart)
        end
        if self.desc._btn_BigECartSlot:GetShow() then
          buttonListForSpan:push_back(self.desc._btn_BigECartSlot)
        end
        if self.desc._btn_BigECart:GetShow() then
          buttonListForSpan:push_back(self.desc._btn_BigECart)
        end
        if self.desc._btn_BigGift:GetShow() then
          buttonListForSpan:push_back(self.desc._btn_BigGift)
        end
        if self.desc._btn_BigStyleAlbum:GetShow() then
          buttonListForSpan:push_back(self.desc._btn_BigStyleAlbum)
        end
        if self.desc._btn_SideDetail:GetShow() then
          buttonListForSpan:push_back(self.desc._btn_SideDetail)
        end
        local ButtonOffset = 3
        local buttonSpanSizeX = 20 + ButtonOffset
        local buttonWebLinkSpanSizeX = buttonSpanSizeX
        for ii = 1, buttonListForSpan:length() do
          local button = buttonListForSpan[ii]
          button:SetSpanSize(buttonSpanSizeX, 60)
          buttonSpanSizeX = buttonSpanSizeX + 40
          buttonWebLinkSpanSizeX = button:GetSpanSize().x + button:GetSizeX() - 2
        end
        if self.desc._btn_WebLink:GetShow() then
          self.desc._btn_WebLink:SetSpanSize(buttonWebLinkSpanSizeX, 60)
          self.desc._btn_WebLink:SetSize(self.desc._btn_BigBuy:GetSizeX() + self.desc._btn_BigBuy:GetSpanSize().x - buttonWebLinkSpanSizeX - ButtonOffset, self.desc._btn_WebLink:GetSizeY())
        end
      end
...
function inGameShop:registMessageHandler()
  ...
  self._btn_PurchasePearl:addInputEvent("Mouse_LUp", "InGameShop_BuyDaumCash()")
...
function InGameShop_TabEvent(tab)
  ...
  if 0 < self._currentTab and self._currentTab <= getCashMainCategorySize() then
  ...
  if tab > 0 and tab <= self._tabCount then
    ...
    if 0 < self._previousTab and self._previousTab <= self._tabCount then
...
function PaGlobal_InGameShop_FindProductIndex(productRaw)
  local self = inGameShop
  if self == nil then
    return nil
  end
  for ii = 0, self._slotCount - 1 do
    local slot = self._slots[ii]
    if slot ~= nil and slot.productNoRaw == productRaw then
      return ii
    end
  end
  return nil
...
function InGameShop_OpenHotAndNewList()
  ...
  self._scroll_IngameCash:SetSize(self._scroll_IngameCash:GetSizeX(), remainingSizeY * scrollAreaSizeRatio)
  self._static_ScrollArea:SetSize(self._static_ScrollArea:GetSizeX(), remainingSizeY * scrollAreaSizeRatio)
...
function InGameShop_ProductListContent_ChangeTexture(slot, isSelected)
  slot.static:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/cashshop_01.dds")
...
function _ingameCash_SetTabIconTexture(control, tabIdx, status)
  ...
  if tabIdx >= 1 and tabIdx <= categorySize then
    control:ChangeTextureInfoNameAsync(tabIndexList[tabIdx][4])
  elseif tabId.promotionTab == tabIdx then
    control:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/cashshopmenu/CashShopMenu_00.dds")
  elseif tabId.cart == tabIdx then
    control:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/ingamecashshop/cashshopmenu/CashShopMenu_10.dds")
...
function FGlobal_InGameShop_OpenByEventAlarm()
  ...
  if curChannelData:isMultiNationServer() == true then
...
function IngameCashShop_PearlBox_Open(slotNo, itemWrapper, count, inventoryType)
  ...
  if nil ~= Panel_IngameCashShop_MoonBlessing_Renew_All and true == Panel_IngameCashShop_MoonBlessing_Renew_All:GetShow() then
...
function IngameCashShop_Descinit()
  ...
  self._btn_WebLink = UI.getChildControl(inGameShop._goodDescBG, "Button_WebLink")
  ...
  self._btn_BigGift:addInputEvent("Mouse_On", "HandleOnOut_IngameCashShop_DescButtonToolTip('BigGift', true)")
  self._btn_BigGift:addInputEvent("Mouse_Out", "HandleOnOut_IngameCashShop_DescButtonToolTip('BigGift', false)")
  self._btn_BigCart:addInputEvent("Mouse_On", "HandleOnOut_IngameCashShop_DescButtonToolTip('BigCart', true)")
  self._btn_BigCart:addInputEvent("Mouse_Out", "HandleOnOut_IngameCashShop_DescButtonToolTip('BigCart', false)")
  self._btn_SideDetail:addInputEvent("Mouse_On", "HandleOnOut_IngameCashShop_DescButtonToolTip('SideDetail', true)")
  self._btn_SideDetail:addInputEvent("Mouse_Out", "HandleOnOut_IngameCashShop_DescButtonToolTip('SideDetail', false)")
  self._btn_WebLink:addInputEvent("Mouse_On", "HandleOnOut_IngameCashShop_DescButtonToolTip('WebLink', true)")
  self._btn_WebLink:addInputEvent("Mouse_Out", "HandleOnOut_IngameCashShop_DescButtonToolTip('WebLink', false)")
end
function IngameCashShop_DescUpdate()
  ...
  for ii = 0, itemListCount - 1 do
    ...
    slot.iconBG:SetPosX(itemConfig._startX + itemConfig._gapX * (ii % inGameShop._config._itemCountPerLine))
    slot.iconBG:SetPosY(itemConfig._startY + math.floor(ii / inGameShop._config._itemCountPerLine) * itemConfig._gapY)
    slot.icon:ChangeTextureInfoNameAsync("icon/" .. item:getIconPath())
  ...
  self._static_Slot:ChangeTextureInfoNameAsync("Icon/" .. cashProduct:getIconPath())
  ...
  local addSizeY = 10
  local dsecIconRow = 0
  dsecIconRow = math.ceil(subItemListCount / inGameShop._config._itemCountPerLine)
  if subItemListCount > inGameShop._config._itemCountPerLine then
    addSizeY = 56 * (dsecIconRow - 1)
  end
  ...
  local webKeyword = cashProduct:getWebKeyword()
  local webHashID = cashProduct:getWebHashID()
  local isShow = webKeyword ~= "" and webHashID ~= ""
  self._btn_WebLink:SetShow(isShow)
  if isShow == true then
    self._btn_WebLink:addInputEvent("Mouse_LUp", "PaGlobalFunc_ExpirienceWiki_WebDetailDescLinkOpen('" .. tostring(webKeyword) .. "','" .. tostring(tostring(webHashID)) .. "')")
  else
    self._btn_WebLink:addInputEvent("Mouse_LUp", "")
  end
end
  ...
  if cashProduct:isChooseCash() then
    ...
    for ii = 0, inGameShop._chooseProductListCount - 1 do
      ...
      if ii < validChooseCashProduct then
        ...
        radioBenner.staticBG:ChangeTextureInfoNameAsync(chooseCashProduct:getPackageIcon())
  ...
  if nil ~= cashProduct then
    ...
    if true == cashProduct:getGuildGiveawayKey():isDefined() then
      ...
      for ii = 0, __eGuildGiveawaySelectedItemMaxCount - 1 do
        ...
        if nil ~= itemSSW then
          slot.icon:ChangeTextureInfoNameAsync("Icon/" .. tostring(itemSSW:getIconPath()))
          slot.icon:SetText(tostring(itemCount))
...
function InGameShop_ChooseCashClickEvent(index, isUp)
  ...
  if InGameShopBuy_IsOpen ~= nil and InGameShopBuy_IsOpen() == true and InGameShopBuy_Close ~= nil then
    InGameShopBuy_Close()
  end
end
...
function IngameCashShop_DescSelectedBuyItem(productNoRaw)
  ...
  if UI_CCC.eCashProductCategory_Pearl == categoryType or UI_CCC.eCashProductCategory_Mileage == categoryType or true == cashProduct:isMoneyPrice() or true == cashProduct:getGuildGiveawayKey():isDefined() then
    ...
    if _ContentsGroup_PearlShopMoonBlessingRenew == true and PaGlobal_MoonBlessingRenew_IsOpenCheck() == true then
      PaGlobal_MoonBlessing_ChangeConsumePearl(Int64toInt32(cashProduct:getPrice()))
    end
...
function InGameShop_Resize()
  ...
  self._static_Bottom_PurchaseArea:ComputePos()
  ...
  if _ContentsGroup_PearlBoxWebBuy == true then
    remainingSizeY = remainingSizeY * 0.96
  end
...
function InGameCashshopDescUpdate(deltaTime)
  ...
  if false == self._isClick then
    if -1 ~= self._openProductKeyRaw then
      self._goodDescBG:SetSize(self._goodDescBG:GetSizeX(), self._goodDescBG:GetSizeY() + (self._maxDescSize - self._goodDescBG:GetSizeY()) * deltaTime * posCalibrateVal)
  ...
  elseif self._isClick then
    if self._goodDescBG:GetSizeY() > 1.5 then
      self._goodDescBG:SetSize(self._goodDescBG:GetSizeX(), self._goodDescBG:GetSizeY() - self._goodDescBG:GetSizeY() * deltaTime * posCalibrateVal)
      if self:getMaxPosition() - self._position < self._config._slot._gapY and self._scroll_IngameCash:GetShow() then
        self._position = self._position - self._goodDescBG:GetSizeY() * deltaTime * posCalibrateVal
      end
    else
      self._goodDescBG:SetSize(self._goodDescBG:GetSizeX(), 1)
...
function InGameShop_OpenActual()
  if ToClient_isConsole() == true and true == _ContentsGroup_XB_Obt then
...
function PaGlobal_InGameCashShop_CheckClose()
  ...
  if _ContentsGroup_PearlShopMoonBlessingRenew == false then
    ...
    local OpenMoonBlessing = function()
      HandleEventLUp_IngameCashShopMoonBlessing_Renew_All_Open()
    end
    if 0 < ToClient_GetMoonBlessingRewardCount(__eMoonBlessingRewardType_Rewarded) == true then
      local messageBoxTitle = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_ALERT")
      local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_CHECKMOONBLESSING")
      local messageBoxData = {
        title = messageBoxTitle,
...
function HandleOnOut_IngameCashShop_DescButtonToolTip(type, show)
  if show == false then
    TooltipSimple_Hide()
    return
  end
  local control, name, desc
  if type == "BigGift" then
    control = inGameShop.desc._btn_BigGift
    name = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GIFT")
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GIFT_DESC")
  elseif type == "BigCart" then
    control = inGameShop.desc._btn_BigCart
    name = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_CART")
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_CART_DESC")
  elseif type == "SideDetail" then
    control = inGameShop.desc._btn_SideDetail
    name = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_CHOICE")
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_CHOICE_DESC")
  elseif type == "WebLink" then
    control = inGameShop.desc._btn_WebLink
    name = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_DETAILINFO")
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_DETAILINFO_DESC")
  else
    return
  end
  TooltipSimple_Show(control, name, desc)
