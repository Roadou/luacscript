...
function PaGlobalFunc_Manufacture_Notify_All_Progress_Update(materialItemWhereType)
  ...
  for key, value in pairs(PaGlobal_Manufacture_Notify_All._data_Resource) do
    ...
    PaGlobal_Manufacture_Notify_All._icon_Resource[index]:ChangeTextureInfoNameAsync(_iconPath)
    ...
    if progressRate < _rate and _rate > 0 then
      progressRate = _rate
    end
  end
  if 0 < PaGlobal_Manufacture_Notify_All._successCount then
  ...
  for key, value in pairs(PaGlobal_Manufacture_Notify_All._data_Result) do
    ...
    PaGlobal_Manufacture_Notify_All._icon_Result[index]:ChangeTextureInfoNameAsync(_iconPath)
  ...
  if size > 0 then
    ...
    for key, value in pairs(PaGlobal_Manufacture_Notify_All._data_Resource) do
      ...
      if nil ~= itemWrapper then
        ...
        if nil ~= itemSS and value._key == itemSS:get()._key:getItemKey() then
