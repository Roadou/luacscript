...
function PaGlobal_MiniGame_YachtDice:initHandRankList()
  ...
  for ii = 0, #handRankKeyList do
    ...
    if nil ~= handRankWrapper then
      ...
      control:ChangeTextureInfoTextureIDAsync(iconTexture)
...
function PaGlobal_MiniGame_YachtDice:updateFieldCardXXX(index)
  ...
  fieldCard.stc_knowledgeImage:ChangeTextureInfoNameAsync(cardWrapper:getCardTextureName())
  fieldCard.stc_numberImage:ChangeTextureInfoNameAsync(numberPath)
  ...
  if nil ~= diceCard then
    diceCard.stc_knowledgeImage:ChangeTextureInfoNameAsync(cardWrapper:getCardTextureName())
    diceCard.stc_numberImage:ChangeTextureInfoNameAsync(numberPath)
    diceCard:SetShow(false)
  end
end
...
function PaGlobal_MiniGame_YachtDice:updateMyUserInfo()
  ...
  if nil ~= iconPath then
    turnBG:ChangeTextureInfoTextureIDAsync(iconPath)
  end
...
function PaGlobal_MiniGame_YachtDice:updateEnemyUserInfo()
  ...
  if nil ~= iconPath then
    ...
    elseif __eActorType_Npc == actorTypeRaw then
      turnBG:ChangeTextureInfoNameAsync(iconPath)
    end
  else
  end
...
function PaGlobal_MiniGame_YachtDice:initTurnBG(name, playerType)
  ...
  function turnBG:ChangeTextureInfoTextureIDAsync(textureID)
    turnBG.stc_rightIcon:ChangeTextureInfoTextureIDAsync(textureID)
    turnBG.stc_rightIcon:setRenderTexture(turnBG.stc_rightIcon:getBaseTexture())
    turnBG.stc_icon:ChangeTextureInfoTextureIDAsync(textureID)
    turnBG.stc_icon:setRenderTexture(turnBG.stc_icon:getBaseTexture())
  end
  function turnBG:ChangeTextureInfoNameAsync(texture)
    turnBG.stc_rightIcon:ChangeTextureInfoNameAsync(texture)
    ...
    turnBG.stc_rightIcon:setRenderTexture(turnBG.stc_rightIcon:getBaseTexture())
    turnBG.stc_icon:ChangeTextureInfoNameAsync(texture)
  ...
  function turnBG:Set_UserInfoControls(isBottomControls, userDataWrapper)
    ...
    local function setPlatformIcon(platformType)
      if platformType == __eGAME_PLATFORM_TYPE_PS then
        self.stc_platformIcon:ChangeTextureInfoNameAsync("combine/commonicon/crossplay_icon_ps.dds")
      else
        self.stc_platformIcon:ChangeTextureInfoNameAsync("combine/commonicon/crossplay_icon_other.dds")
      end
      if isBottomControls == true then
...
function PaGlobal_MiniGame_YachtDice:eventStartBeforeGameStart()
  ...
  _AudioPostEvent_SystemUiForXBOX(11, 63)
...
function PaGlobal_MiniGame_YachtDice:eventEndBeforeGameStart()
  ...
  self._isPvPFirstTurn = false
  local rollBtnBg = UI.getChildControl(self._ui._stc_rightBG, "Static_Roll_BG")
  local isPvPMode = self._currentGameDbType == __eMiniGameDBType_YachtDice_PVP_Normal or self._currentGameDbType == __eMiniGameDBType_YachtDice_PVP_Match
  if isPvPMode == true and 1 == ToClient_getCurrentTurnCount() then
    local isMyTurn = ToClient_isMyTurnMiniGameYachtDice()
    if true == isMyTurn then
      rollBtnBg:SetShow(true)
      self._ui._btn_roll:SetShow(true)
...
function PaGlobal_MiniGame_YachtDice:eventStartKeep(index, isKeep)
  ...
  if true == isKeep then
    ...
    _AudioPostEvent_SystemUiForXBOX(11, 119)
    fieldCard.txt_isKeep:SetAlphaChild(0)
    self._endTime = 0.3
  else
    audioPostEvent_SystemUi(11, 122)
    _AudioPostEvent_SystemUiForXBOX(11, 122)
...
function PaGlobal_MiniGame_YachtDice:updateTimer(deltaTime)
  ...
  elseif __eMiniGameDBType_YachtDice_PVP_Normal == self._currentGameDbType or __eMiniGameDBType_YachtDice_PVP_Match == self._currentGameDbType then
    currentTurnMaxTime = ToClient_getMiniGameYachtTurnMaxTime()
  ...
  if self._soundTime == remainTime then
    ...
    _AudioPostEvent_SystemUiForXBOX(11, 117)
  ...
  for ii = 0, __eYachtDiceHandRankMax - 1 do
    ...
    if nil ~= slot.keyRaw then
      if slot.startTime < self._eventHandRankTime and false == slot.isOpen then
        ...
        _AudioPostEvent_SystemUiForXBOX(11, 107)
      end
...
function PaGlobal_MiniGame_YachtDice:updateDrawSound(isDraw, drawCount)
  ...
  if nil ~= index then
    ...
    _AudioPostEvent_SystemUiForXBOX(11, index)
  end
end
function PaGlobal_MiniGame_YachtDice:updateCheckHandEffect(key, isSelf, point)
  ...
  _AudioPostEvent_SystemUiForXBOX(11, 118)
end
