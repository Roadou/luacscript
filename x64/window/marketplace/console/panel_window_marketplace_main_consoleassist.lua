...
function PaGlobalFunc_MarketPlace_ConsoleAssist_SetXYKeyGuide(idx, isShow)
  local self = MarketPlace_ConsoleAssist
  local base = PaGlobalFunc_ItemMarket_Get()
  self._ui._keyGuide_X:SetShow(isShow)
  self._ui._keyGuide_Y:SetShow(isShow)
  if base._itemListType.hotListByCategory == self._currentListType or base._itemListType.waitListByCategory == self._currentListType then
    self._ui._keyGuide_Y:SetShow(false)
  end
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._keyGuide, self._ui.stc_keyGuideArea, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT)
  self._currentListIdx = idx
...
function PaGlobalFunc_MarketPlace_ItemMarket_ViewDetailToolTip(isShow)
  ...
  elseif base._itemListType.waitListByCategory == self._currentListType then
...
function PaGlobalFunc_MarketPlace_ConsoleAssist_SetSubList(listType, idx)
  ...
  button:addInputEvent("Mouse_On", "PaGlobalFunc_MarketPlace_ConsoleAssist_SetXYKeyGuide(" .. idx .. ", true )")
  button:addInputEvent("Mouse_Out", "PaGlobalFunc_MarketPlace_ConsoleAssist_SetXYKeyGuide( -1,false )")
  if base._itemListType.hotListByCategory ~= self._currentListType and base._itemListType.waitListByCategory ~= self._currentListType then
    button:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "PaGlobalFunc_ItemMarket_BackPage()")
  end
end
...
function MarketPlace_ConsoleAssist:updateTab()
  ...
  elseif self._tabConfig._wallet == self._currentTab then
    ...
    self._ui.stc_kamaPackIcon:ChangeTextureInfoNameAsync("renewal/pcremaster/remaster_market_00.dds")
  ...
  elseif self._tabConfig._walletManage == self._currentTab then
    ...
    _panel:registerPadEvent(__eConsoleUIPadEvent_LT, "PaGlobalFunc_MarketPlace_Console_SelectTab(false)")
    _panel:registerPadEvent(__eConsoleUIPadEvent_RT, "PaGlobalFunc_MarketPlace_Console_SelectTab(true)")
    _panel:registerPadEvent(__eConsoleUIPadEvent_Up_LT, "")
  end
end
...
function MarketPlace_ConsoleAssist:initControl()
  ...
  self._ui._keyGuide_Y = UI.getChildControl(self._ui.stc_keyGuideArea, "StaticText_Y_ConsoleUI")
