...
function MarketPlace_MyInven:initControl()
  ...
  self._ui.stc_TitleIcon = UI.getChildControl(self._ui.stc_Title, "StaticText_TitleIcon")
...
function MarketPlace_MyInven:findSearchItem(isSearch)
  ...
  if CppEnums.ItemWhereType.eWarehouse == self._currentInvenType then
    warehouseWrapper = warehouse_get(PaGlobal_Warehouse_All_GetCurrentWaypoint())
...
function PaGlobalFunc_MarketPlace_SelectTab(tabIdx)
  ...
  self._ui.btn_WareHouseInven:SetCheck(false)
  if tabIdx == CppEnums.ItemWhereType.eInventory then
    self._ui.btn_NormalInven:SetCheck(true)
    self._ui.stc_TitleIcon:SetText(self._ui.btn_NormalInven:GetText())
  elseif tabIdx == CppEnums.ItemWhereType.eCashInventory then
    self._ui.btn_CashInven:SetCheck(true)
    self._ui.stc_TitleIcon:SetText(self._ui.btn_CashInven:GetText())
  elseif tabIdx == CppEnums.ItemWhereType.eWarehouse then
    self._ui.btn_WareHouseInven:SetCheck(true)
    self._ui.stc_TitleIcon:SetText(self._ui.btn_WareHouseInven:GetText())
  end
...
function PaGlobalFunc_MarketPlace_MyInven_Warehouse_GetItem(slotNo)
  local warehouseWrapper = warehouse_get(PaGlobal_Warehouse_All_GetCurrentWaypoint())
  if nil == warehouseWrapper then
    return nil
  end
...
function MarketPlace_MyInven:updateWareHouseInventory()
  ...
  local waypointKey = PaGlobal_Warehouse_All_GetCurrentWaypoint()
  local warehouseWrapper = warehouse_get(waypointKey)
  if warehouseWrapper == nil then
    return
  end
  local regionName = getRegionNameByWaypointKey(waypointKey)
  if regionName ~= nil then
    self._ui.stc_TitleIcon:SetText(self._ui.btn_WareHouseInven:GetText() .. " - " .. regionName)
  end
  local useStartSlot = 1
