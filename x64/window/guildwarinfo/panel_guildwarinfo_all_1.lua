...
function PaGlobal_GuildWarInfo_All:updateGuildList(content, index)
  ...
  if false == isSet then
    guildInfo.stc_GuildIcon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Default/Default_Buttons.dds")
...
function PaGlobal_GuildWarInfo_All:getIndexNextIndex(selectIndex, isRight)
  if isRight == true then
    for index = selectIndex + 1, self._MAXTERRITORYCOUNT - 1 do
      if ToClient_isStartMajorSiege(index) == true then
        return index
      end
    end
    ...
    for index = self._MAXTERRITORYCOUNT - 1, selectIndex + 1, -1 do
      if ToClient_isStartMajorSiege(index) == true then
        return index
      end
    end
...
function PaGlobal_GuildWarInfo_All:updateSecondRoundRankList(content, index)
  ...
  if 1 == index_32 then
    static2:ChangeTextureInfoNameAsync("Combine/Etc/Combine_Etc_TerritoryWar_01.dds")
