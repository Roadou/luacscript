...
function HandleEventLUp_SailorManagerAll_SelectSailor(index)
  ...
  if nil ~= sailorInfo._iconPath and "" ~= sailorInfo._iconPath then
    control.stc_workerImage:ChangeTextureInfoNameAsync(sailorInfo._iconPath)
  end
  local isRide = false
  ...
  if sailorInfo._isViceCaptain == true then
    control.txt_workerTitle:SetText(sailorInfo._name)
  else
  ...
  control.txt_expBarName:SetShow(sailorInfo._isViceCaptain == false)
  control.progress_expBG:SetShow(sailorInfo._isViceCaptain == false)
  control.progress_exp:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_costTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_speedTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_excelTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_turningTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_breakTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_patienceTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_powerTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_focusTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_sightTitle:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_costValue:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_speedValue:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_turningValue:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_excelValue:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_breakValue:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_cannonCoolTime:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_cannonAccuracy:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_cannonMaxLength:SetShow(sailorInfo._isViceCaptain == false)
  control.txt_cannonMaxAngle:SetShow(sailorInfo._isViceCaptain == false)
  ...
  if __eEmployeeState_Escape == sailorInfo._state then
    ...
    if true == PaGlobal_SailorManager_All._isConsole and false == PaGlobal_SailorManager_All._isOpenByESCMenu then
      Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandlerEventLUp_SailorManager_All_Ride()")
    end
  end
  local isViceCaptain = false
  if _ContentsGroup_EmployeePresetJob == true and sailorInfo._isViceCaptain == true then
    isViceCaptain = true
  end
  if PaGlobal_SailorManager_All._isConsole == true then
    PaGlobal_SailorManager_All:ConsoleKeySetting(isViceCaptain)
  elseif isViceCaptain == true then
    PaGlobal_SailorManager_All._ui.btn_WorkerFire_PC:SetShow(false)
  else
    PaGlobal_SailorManager_All._ui.btn_WorkerFire_PC:SetShow(true)
  end
end
...
function SailorManagerAll_FireSailor()
  ...
  elseif sailorWrapper:getJob() == __eEmployeeJob_ViceCaptain then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_CAPTAIN_DISMISS_MESSEAGE"))
  else
...
function HandleEventLUp_SailorManagementAll_SailorRestore(sailorIndex, loyalismType)
  ...
  for index = 1, recoveryItemSize do
    ...
    if nil ~= recoveryItem then
      ...
      restoreSlot.slotIcon:ChangeTextureInfoNameAsync("icon/" .. itemStatic:getIconPath())
      restoreSlot.slotIcon:SetShow(true)
      ...
      if true == PaGlobal_SailorManager_All._isConsole then
        restoreSlot.slotIcon:registerPadEvent(__eConsoleUIPadEvent_Up_A, "HandlerEventRUp_SailorManagementAll_requestRecoveryEmployee(" .. sailorIndex .. "," .. index - 1 .. ", " .. loyalismType .. ")")
        restoreSlot.slotIcon:registerPadEvent(__eConsoleUIPadEvent_LTPress_A, "HandlerEventRUp_SailorManagementAll_requestRecoveryEmployeeAll_Console(" .. sailorIndex .. "," .. index - 1 .. ", " .. loyalismType .. ")")
        restoreSlot.slotIcon:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandleEventLUp_SailorManagementAll_SailorRestoreClose()")
      end
...
function HandlerEventRUp_SailorManagementAll_requestRecoveryEmployeeAll_Console(sailorIndex, itemIndex, restoreValue)
  if nil == Panel_Window_SailorManager_All then
    return
  end
  local sailorInfo = PaGlobal_SailorManager_All._sailorInfoList[sailorIndex]
...
function PaGlobal_SailorManager_All_isShowRecoverToolTip()
  if false == PaGlobal_SailorManager_All._isConsole then
    return false
  end
  if nil == PaGlobal_SailorManager_All._ui.stc_restoreItemBG then
    return false
  end
  return PaGlobal_SailorManager_All._ui.stc_restoreItemBG:GetShow()
end
function PaGlobal_SailorManager_All_CloseRecoverToolTip()
  if false == PaGlobal_SailorManager_All._isConsole then
    return
  end
  if nil == PaGlobal_SailorManager_All._ui.stc_restoreItemBG then
  ...
  if false == PaGlobal_SailorManager_All._ui.stc_restoreItemBG:GetShow() then
    return
  end
  PaGlobal_SailorManager_All._ui.stc_restoreItemBG:SetShow(false)
end
  ...
  elseif nil == sailorInfo then
    ...
    if nil ~= servantInfo then
      if unsealServantInfo:getServantNo() ~= sailorInfo._onServantNo then
        ...
        ToClient_requestUnApplyEmployee(sailorInfo._sailorNo, 0)
      end
    else
...
function HandleEventLUp_SailorManagerAll_SailorTooltip(index, state, isShow)
