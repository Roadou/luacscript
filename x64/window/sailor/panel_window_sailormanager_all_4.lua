function PaGlobal_SailorRestore_All:initialize()
  ...
  self._ui.stc_bottomBG_Console = UI.getChildControl(Panel_Window_SailorRestore_All, "Static_Keyguide_ConsoleUI")
  self._ui.stc_keyGuide_Confirm = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_KeyGuide_Y")
  self._ui.stc_keyGuide_Select = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_KeyGuide_A")
  self._ui.stc_keyGuide_Cancle = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_KeyGuide_B")
  ...
  if true == self._isConsole then
    ...
    self._ui.stc_bottomBG_Console:SetShow(true)
    Panel_Window_SailorRestore_All:SetSize(Panel_Window_SailorRestore_All:GetSizeX(), Panel_Window_SailorRestore_All:GetSizeY() - self._ui.btn_complete:GetSizeY())
    local keyguides = {
      self._ui.stc_keyGuide_Confirm,
      self._ui.stc_keyGuide_Select,
      self._ui.stc_keyGuide_Cancle
    }
    ...
    self._ui.stc_bottomBG_Console:SetShow(false)
  end
...
function PaGlobal_SailorRestore_All:registerEventHandler()
  ...
  Panel_Window_SailorRestore_All:registerPadEvent(__eConsoleUIPadEvent_Y, "HandlerEventLUp_SailorRestoreAll_Confirm()")
  Panel_Window_SailorRestore_All:registerPadEvent(__eConsoleUIPadEvent_X, "PaGlobal_SailorRestore_All:prepareClose()")
end
...
function PaGlobal_SailorRestore_All:update()
  ...
  self.restoreItemExist = isExistItem
  ...
  for index = 1, recoveryItemSize do
    ...
    if nil ~= recoveryItem then
      ...
      restoreSlot.base.icon:ChangeTextureInfoNameAsync("icon/" .. itemStatic:getIconPath())
      restoreSlot.base.icon:SetShow(true)
      ...
      local gradeType = itemStatic:getGradeType()
      if gradeType > 0 and gradeType <= #UI.itemSlotConfig.borderTexture then
        restoreSlot.base.border:ChangeTextureInfoTextureIDAsync(UI.itemSlotConfig.borderTexture[gradeType].texture)
        restoreSlot.base.border:setRenderTexture(restoreSlot.base.border:getBaseTexture())
        restoreSlot.base.border:SetShow(true)
      end
...
function PaGlobal_SailorRestore_All:setPanelPosition()
  ...
  if _ContentsGroup_EmployeePresetJob == true then
    if Panel_Window_SailorRestore_All == nil or Panel_Window_SailorPresetManager_All == nil then
      return
    end
    pos.x = Panel_Window_SailorPresetManager_All:GetPosX() + Panel_Window_SailorPresetManager_All:GetSizeX() + 15
    pos.y = Panel_Window_SailorPresetManager_All:GetPosY()
  else
...
function PaGlobal_SailorRestore_All:prepareClose()
  ...
  Panel_Window_SailorRestore_All:ignorePadSnapMoveToOtherPanelUpdate(false)
...
function HandlerEventLUp_SailorRestoreAll_SelectItem(index)
  ...
  if _ContentsGroup_EmployeePresetJob == true then
    for index = 0, #PaGlobal_SailorPresetManager_All._employeeInfoList do
      local employeeInfo = PaGlobal_SailorPresetManager_All._employeeInfoList[index]
      if employeeInfo ~= nil and __eEmployeeState_Escape ~= employeeInfo._state and employeeInfo._loyalty < employeeInfo._maxLoyalty then
        sailorCount = sailorCount + 1
        local needLoyalty = employeeInfo._maxLoyalty - employeeInfo._loyalty
...
function HandlerEventLUp_SailorRestoreAll_Confirm()
  ...
  if _ContentsGroup_EmployeePresetJob == true then
    local employeeCount = ToClient_getEmployeeCount()
    for index = 0, employeeCount - 1 do
      ...
      if employeeWrapper ~= nil then
        ...
        if employeeKey:isDefined() == true then
          ToClient_requestRecoveryEmployee(employeeWrapper:getEmployeeNo(), PaGlobal_SailorRestore_All._selectItemIndex - 1, __eEmployeeLoyalismType_Loyalty, true)
        end
      end
    end
