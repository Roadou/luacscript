...
function PaGlobal_SailorOnBoard_All:update()
  ...
  if nil ~= temporaryWrapper then
    self._unsealServantInfo = temporaryWrapper:getUnsealVehicle(__eServantTypeShip)
    if nil ~= self._unsealServantInfo then
      self._unsealVehicleActorKeyRaw = self._unsealServantInfo:getActorKeyRaw()
      self._ui.stc_shipIcon:ChangeTextureInfoNameAsync(self._unsealServantInfo:getIconPath1())
