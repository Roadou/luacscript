function PaGlobal_SailorManager_All:initialize()
  ...
  self._ui.txt_expBarName = UI.getChildControl(self._ui.stc_sailorInfoBG, "StaticText_EXPbarName")
  self._ui.progress_expBG = UI.getChildControl(self._ui.stc_sailorInfoBG, "Static_EXPProgressBg")
  ...
  self._ui.txt_costTitle = UI.getChildControl(self._ui.stc_sailorInfoBG, "StaticText_CostTitle")
  ...
  self._ui.stc_keyGuide_AllRecovery = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_AllRestoreBtnLT_ConsoleUI")
  self._ui.stc_keyGuide_AllRepeat = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_AllRepeatBtnLT_ConsoleUI")
  self._ui.stc_keyGuide_Select = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_Select_ConsoleUI")
  self._ui.stc_keyGuide_Recovery = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_Recover_ConsoleUI")
  self._ui.stc_keyGuide_Onboard = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_Repeat_ConsoleUI")
  self._ui.stc_keyGuide_Close = UI.getChildControl(self._ui.stc_bottomBG_Console, "StaticText_Close_ConsoleUI")
  ...
  self._ui.btn_closeResotreItem:SetShow(not self._isConsole)
...
function PaGlobal_SailorManager_All:ConsoleKeySetting(isViceCaptain)
  self._ui.stc_bottomBG_Console:SetShow(self._isConsole)
  if false == self._isConsole then
    return
  end
  if true == self._isOpenByESCMenu then
    Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "")
    Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "")
    Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "")
  else
    Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandlerEventLUp_SailorManager_All_Ride()")
    Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "PaGlobal_SailorRestore_All:prepareOpen()")
    if isViceCaptain == nil or isViceCaptain == false then
      Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "HandleEventLUp_SailorManagerAll_FireSailor()")
    else
      Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "")
    end
  end
  self._ui.stc_keyGuide_AllRecovery:SetShow(false)
  self._ui.stc_keyGuide_AllRepeat:SetShow(false)
  self._ui.stc_keyGuide_Select:SetShow(false)
  self._ui.stc_keyGuide_Recovery:SetShow(false)
  self._ui.stc_keyGuide_Onboard:SetShow(false)
  self._ui.stc_keyGuide_Close:SetShow(false)
  local curIndex = 1
  local keyGuides = {}
  if false == self._isOpenByESCMenu then
    keyGuides[curIndex] = self._ui.stc_keyGuide_AllRecovery
    keyGuides[curIndex]:SetShow(true)
    curIndex = curIndex + 1
    if isViceCaptain == nil or isViceCaptain == false then
      keyGuides[curIndex] = self._ui.stc_keyGuide_AllRepeat
      keyGuides[curIndex]:SetShow(true)
      curIndex = curIndex + 1
    end
  end
  keyGuides[curIndex] = self._ui.stc_keyGuide_Select
  keyGuides[curIndex]:SetShow(true)
  curIndex = curIndex + 1
  if false == self._isOpenByESCMenu then
    keyGuides[curIndex] = self._ui.stc_keyGuide_Recovery
    keyGuides[curIndex]:SetShow(true)
    curIndex = curIndex + 1
    keyGuides[curIndex] = self._ui.stc_keyGuide_Onboard
    keyGuides[curIndex]:SetShow(true)
    curIndex = curIndex + 1
  end
  keyGuides[curIndex] = self._ui.stc_keyGuide_Close
  keyGuides[curIndex]:SetShow(true)
  curIndex = curIndex + 1
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuides, self._ui.stc_bottomBG_Console, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
end
...
function PaGlobal_SailorManager_All:listCreate(control, key)
      ...
      if nil ~= servantInfo then
        if sailorInfo._isApply then
          ...
          if false == self._isConsole then
            ...
            txt_state1:SetTextMode(__eTextMode_LimitText)
            txt_state1:SetText(txt_state1:GetText())
          end
          txt_state1:SetShow(true)
        else
  ...
  if nil ~= sailorInfo._iconPath and "" ~= sailorInfo._iconPath then
    stc_sailorImg:ChangeTextureInfoNameAsync(sailorInfo._iconPath)
  end
  progress_exp:SetProgressRate(sailorInfo._expRate)
  progress_royalty:SetProgressRate(sailorInfo._royalRate)
  if sailorInfo._isViceCaptain == true then
    txt_name:SetText(sailorInfo._name)
  else
  ...
  btn_buttonBG:addInputEvent("Mouse_On", "HandleEventLUp_SailorManagerAll_SailorTooltip(" .. index .. "," .. sailorInfo._state .. ",true)")
  btn_buttonBG:addInputEvent("Mouse_Out", "HandleEventLUp_SailorManagerAll_SailorTooltip(" .. index .. "," .. sailorInfo._state .. ",false)")
  local isOpenByESCMenu = PaGlobal_SailorManager_All._isOpenByESCMenu
  btn_restore:SetShow(not isOpenByESCMenu)
  ...
  if true == self._isConsole then
    btn_buttonBG:registerPadEvent(__eConsoleUIPadEvent_Up_A, "HandleEventLUp_SailorManagerAll_SelectSailor(" .. index .. ")")
    if true == isOpenByESCMenu then
      btn_buttonBG:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "")
      Panel_Window_SailorManager_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "")
    elseif true == isEscape then
      btn_buttonBG:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandleEventLUp_SailorManagementAll_SailorRestore(" .. index .. "," .. __eEmployeeLoyalismType_resurrection .. ")")
    else
      btn_buttonBG:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandleEventLUp_SailorManagementAll_SailorRestore(" .. index .. "," .. __eEmployeeLoyalismType_Loyalty .. ")")
    end
  end
  txt_displayRoyalty:SetText(tostring(sailorInfo._loyalty) .. "/" .. tostring(sailorInfo._maxLoyalty))
  self._sailorBtnList[index] = btn_buttonBG
end
    ...
    if nil ~= sailorWrapper then
      ...
      if true == sailorKey:isDefined() then
        ...
        local sailorInfoTemplate = {
          ...
          _isViceCaptain = sailorWrapper:getJob() == __eEmployeeJob_ViceCaptain
        }
...
function PaGlobal_SailorManager_All:prepareOpen()
  ...
  Panel_Window_SailorManager_All:ignorePadSnapMoveToOtherPanelUpdate(true)
  self:ConsoleKeySetting()
  local isOpenByESCMenu = not PaGlobal_SailorManager_All._isOpenByESCMenu
  self._ui.btn_restoreAll_PC:SetShow(isOpenByESCMenu and false == self._isConsole)
  self._ui.btn_ride_PC:SetShow(isOpenByESCMenu and false == self._isConsole)
  self._ui.btn_WorkerFire_PC:SetShow(isOpenByESCMenu and false == self._isConsole)
  PaGlobal_SailorManager_All:resize()
  if false == isOpenByESCMenu then
    PaGlobal_SailorOnBoard_All:prepareClose()
    Panel_Window_SailorManager_All:SetSpanSize(0, 0)
  else
    PaGlobal_SailorOnBoard_All:prepareOpen()
    Panel_Window_SailorManager_All:SetSpanSize(self._originSpanX, 0)
  end
  ...
  if nil ~= temporaryWrapper then
    local unsealServantInfo = temporaryWrapper:getUnsealVehicle(__eServantTypeShip)
...
function PaGlobal_SailorManager_All:prepareClose()
  ...
  Panel_Window_SailorManager_All:ignorePadSnapMoveToOtherPanelUpdate(false)
  ...
  elseif true == Panel_Window_Sailor_Onboard_All:GetShow() then
    PaGlobal_SailorOnBoard_All:prepareClose()
