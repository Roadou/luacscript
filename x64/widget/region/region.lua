...
function Region_OnChangeRegion(isSafeZone, isArenaZone, isDesert, isAtoraxion, regionType, regionKey, areaName, territoryName)
  if Panel_RoseWar_Region ~= nil and Panel_RoseWar_Region:GetShow() == true then
    return
  end
  if isAtoraxion == true or regionKey == 1103 then
    if PaGlobalFunc_Atoraxion_RegionInfo_Open ~= nil then
      PaGlobalFunc_Atoraxion_RegionInfo_Open(areaName, regionKey)
    end
    if FromClient_Atoraxion_RegionChange_Open ~= nil then
      FromClient_Atoraxion_RegionChange_Open(areaName)
  ...
  regionName:SetText(areaName)
  regigonPlace:SetText(territoryName)
  ...
  local isExistSiege = ToClient_IsExistSiegeByRegionKey(regionKey)
  if isExistSiege == true then
    local isSiegeBeing = ToClient_IsSiegeBeingByRegionKey(regionKey)
