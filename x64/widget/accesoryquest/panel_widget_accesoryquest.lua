...
function AccesoryQuest:update()
  ...
  local questWrapper = questList:getSuddenQuestWrapperOnCurrentRegion()
...
function Input_AccesoryQuest_ApplyReward()
  if PaGlobalFunc_AccesoryQuest_IsSatisfied() == true then
    ToClient_RequestCompleteQuest(AccesoryQuest._nowQuestNo._group, AccesoryQuest._nowQuestNo._quest)
  end
end
...
function FromClient_AccesoryQuest_selfPlayer_regionChanged(isSafeZone, isArenaZone, isDesert, isAtoraxion, regionType, regionKey, areaName, territoryName)
  local self = AccesoryQuest
  self._nowRegionKey = Int64toInt32(regionKey)
  self:update()
