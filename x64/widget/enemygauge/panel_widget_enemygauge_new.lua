...
function targetHpInfo_Update_Monster(actorKey, nowHP)
  ...
  elseif checkActiveCondition(targetActor:getCharacterKey()) == true then
    ...
    lua_TargetInfo_BossHpRate:ChangeTextureInfoNameAsync("Renewal/Progress/Console_Progressbar_04.dds")
...
function panel_Update_Monster_Info(actorKey)
  ...
  if targetActor:get():isPlayer() then
    ...
    if playerActorProxyWrapper:get():isVolunteer() then
      ...
      if ToClient_namelessPVPServer() == true then
        ...
        if _ContentsGroup_HardCoreChannel == true and ToClient_isHardCoreChannel() and actor ~= nil then
  ...
  elseif true == isKnowledgeGradeShow then
    ...
    if instanceMapKey == __eInstanceRedBattleField_PapuCrio and targetActor:get():isVehicle() then
...
function _recentTargetInfo_TendencyOnDead_MSG(targetActor, actorKey)
  ...
  if ToClient_DontShowTendencyOnDeadMessageWhenOceanSiegeBeging(targetActor:get()) == true then
    return
  end
...
function FGlobal_Panel_Monster_Bar_RePos()
  if _ContentsGroup_HardCoreChannel == true and ToClient_isHardCoreChannel() then
    Panel_Monster_Bar:SetPosY(Panel_Widget_Compass_Hardcore:GetSizeY())
  elseif Panel_LocalWar ~= nil and Panel_LocalWar:GetShow() then
