...
function selfPlayerStatusBar:resourceTypeCheck(selfPlayerWrapper)
  ...
  if mpType == CppEnums.MpBarColor.DARKELF then
    resourceType = 4
  end
  ...
  if 0 == isColorBlindMode then
    ...
    self._staticGage_HP:ChangeTextureInfoNameAsync("Renewal/Progress/Console_Progressbar_03.dds")
...
local function checkHpAlert(hp, maxHp, isLevelUp)
  ...
  if wrapper ~= nil and wrapper:getAlertHp() == false and strongMonsterAlert == false then
...
function PaGlobalFunc_UserBar_CharacterInfoWindowUpdate()
  if PaGlobalFunc_IsRemasterUIOption() == true then
    return
  end
  ...
  PaGlobalFunc_UserBar_CharacterInfoWindowUpdateHp()
  PaGlobalFunc_UserBar_CharacterInfoWindowUpdateMp(mp, maxMp, mp / maxMp * 100)
end
function PaGlobalFunc_UserBar_CharacterInfoWindowUpdateHp()
  if PaGlobalFunc_IsRemasterUIOption() == true then
    return
  end
  local self = selfPlayerStatusBar
...
function FromClient_UserBar_CharacterInfoWindowUpdateHp(hp, maxHp, minorSiegeMaxHp, percentHp, percentProgress, isArenaZone, classType, combatResourceType)
  if PaGlobalFunc_IsRemasterUIOption() == true then
    return
  end
  local self = selfPlayerStatusBar
  local resourceType = combatResourceType
  local mpType = NewClassData.newClass_CategoryType[classType]._mpType
  if mpType == CppEnums.MpBarColor.DARKELF then
    resourceType = 4
  end
  ...
  if 0 ~= maxHp and (hp ~= prevHP or maxHp ~= prevMaxHP) then
    self._staticShowHp:SetText(tostring(hp) .. "/" .. tostring(maxHp))
    self._staticGage_HP_Later:SetProgressRate(percentProgress)
    self._staticGage_HP:SetProgressRate(percentProgress)
    if hp - prevHP > 5 then
      self._staticGage_HP:AddEffect("fUI_Gauge_Red", false, 0, 0)
    end
    FGlobal_MainStatus_FadeIn(5)
...
function PaGlobalFunc_UserBar_CharacterInfoWindowUpdateMp(mp, maxMp, percent)
  if PaGlobalFunc_IsRemasterUIOption() == true then
    return
  end
  local self = selfPlayerStatusBar
  ...
  local classType = playerWrapper:getClassType()
  self:resourceTypeCheck(playerWrapper)
  local isEP_Character = __eClassType_ElfRanger == classType or __eClassType_RangerMan == classType
  local isFP_Character = __eClassType_Warrior == classType or __eClassType_Giant == classType or __eClassType_BladeMaster == classType or __eClassType_BladeMasterWoman == classType or __eClassType_Kunoichi == classType or __eClassType_Combattant == classType or __eClassType_Lhan == classType or __eClassType_Guardian == classType
  local isBP_Character = __eClassType_Valkyrie == classType
  ...
  _prevMPSize = mp
  if 0 ~= maxMp then
    self._staticGage_CombatResource:SetProgressRate(percent)
    self._staticGage_CombatResource:SetShow(true)
    if mp ~= prevMP or maxMp ~= prevMaxMP then
      self._staticShowMp:SetText(tostring(mp) .. "/" .. tostring(maxMp))
      self._combatResources_Later:SetProgressRate(percent)
      self._combatResources_Later:SetShow(true)
...
function selfPlayerStatusBar:registMessageHandler()
  ...
  registerEvent("FromClient_SelfPlayerHpChanged", "FromClient_UserBar_CharacterInfoWindowUpdateHp")
  registerEvent("FromClient_SelfPlayerMpChanged", "PaGlobalFunc_UserBar_CharacterInfoWindowUpdateMp")
...
function MainStatus_ChangeTexture_On()
  Panel_MainStatus_User_Bar:ChangeTextureInfoNameAsync("new_ui_common_forlua/default/window_sample_drag.dds")
