...
local function registEventHandler()
  if _ContentsGroup_ShowOtherClassSkill == true then
    _btn_NewSkill:addInputEvent("Mouse_LUp", "PaGlobalFunc_SkillGroup_All_Open()")
...
function PackageIconPosition()
  ...
  local premiumValueBuff2025 = player:getUserChargeTime(UI_BUFFTYPE.eUserChargeType_ChallengeBuff, 1)
  ...
  local applyPremiumValueBuff2025 = player:isApplyChargeSkill(UI_BUFFTYPE.eUserChargeType_ChallengeBuff, 1)
  ...
  if (applyPremiumValueBuff == true or applyPremiumValueBuff2025 == true) and (premiumValueBuff > 0 or premiumValueBuff2025 > 0) then
...
function FromClient_PackageIconUpdate()
  ...
  if true == isPremiumPcRoom and not isRussiaArea and not isGameTypeNA() and not isGameTypeSA() and not isGameTypeTR() then
