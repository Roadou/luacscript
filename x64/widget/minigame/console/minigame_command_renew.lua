...
local function CommandTextureChange(contorl, index)
  contorl:ChangeTextureInfoNameAsync("renewal/etc/minigame/console_etc_minigame_00.dds")
...
function MiniGame_Command_keyPress(keyType)
  ...
  if commands[currentCommandIndex] == keyType then
    ...
    local checkControl = checkIconList[currentCommandIndex]
    if nil ~= checkControl then
      checkControl:SetPosX(textControl:GetPosX() + textControl:GetSizeX() / 2 - checkControl:GetSizeX() / 2)
      checkControl:SetPosY(textControl:GetPosY() + (textControl:GetSizeY() + 30) - checkControl:GetSizeY() / 2)
