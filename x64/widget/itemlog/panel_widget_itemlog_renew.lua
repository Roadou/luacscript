...
function ItemLog:show(itemEnchantKey, slotNo, itemCount, posY)
  ...
  self.iconControl:ChangeTextureInfoNameAsync("icon/" .. item:getIconPath())
  self.titleControl:SetText(item:getName() .. " x " .. tostring(itemCount))
  local grade = item:getGradeType()
  self.bgControl:ChangeTextureInfoNameAsync("renewal/frame/console_frame_00.dds")
  if 4 == grade then
...
function PaGlobalFunc_ItemLog_ClearReadyToShowData()
  local self = ItemLogManager
  if self == nil then
    return
  end
  for i in pairs(self._logInfos) do
    self._logInfos[i] = nil
  end
  for j in pairs(self._readyLogs) do
    self._readyLogs[j] = nil
  end
end
