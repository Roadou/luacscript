...
function PaGlobal_RecommendManager:ChangeRecommendIconTexture(type)
  ...
  self._recommendedBefore = true
  self._savedDelay = 0
  PaGlobalFunc_CashShopAlert_ChangeTexture(type)
end
function PaGlobal_RecommendManager:getInventoryisFull()
  local selfPlayerInventory = getSelfPlayer():get():getInventory()
  local size = selfPlayerInventory:size()
  if size >= self.MAXSLOT then
    return false
  end
  local cnt = selfPlayerInventory:getFreeCount()
  return 0 == cnt
end
...
function PaGlobal_RecommendManager:getHavePetNow()
  ...
  local nowexp = selfPlayer:getExp_s64()
  if 0 == self._expforCheckInCombat then
...
function PaGlobalFunc_RecommendManager_ProcessRecommend()
  if false == Panel_CashShopAlert:GetShow() then
    return
  end
  local isRecommend = true
  if Recommend_TYPE.TYPE_INVENTORY == PaGlobal_RecommendManager._nowRecommendedType then
    isRecommend = PaGlobal_RecommendManager:getInventoryisFull()
  elseif Recommend_TYPE.TYPE_WEIGHT == PaGlobal_RecommendManager._nowRecommendedType then
    isRecommend = PaGlobal_RecommendManager:getWeightisFull()
  elseif Recommend_TYPE.TYPE_PET == PaGlobal_RecommendManager._nowRecommendedType then
    isRecommend = PaGlobal_RecommendManager:getHavePetNow()
  end
