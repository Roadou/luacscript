...
function cashShopAlert:registEventHandler()
  ...
  registerEvent("selfPlayer_regionChangedOnlyUsedSafeZone", "FromClient_cashShopAlert_UpdateRegion")
...
function OpenCashShop()
  ...
  if Recommend_TYPE.TYPE_INVENTORY == recommendType then
    PaGlobalFunc_EasyBuy_Open(5, nil, false)
  elseif Recommend_TYPE.TYPE_WEIGHT == recommendType then
    PaGlobalFunc_EasyBuy_Open(4, nil, false)
  else
    PearlShop_Open()
...
function PaGlobalFunc_CashShopAlert_ChangeTexture(cashShopType)
  ...
  self._btn_CashShop:ChangeTextureInfoNameAsync(cashShopAlert.iconPath)
  local x1, y1, x2, y2
...
function FromClient_cashShopAlert_UpdateRegion(isSafeZone)
