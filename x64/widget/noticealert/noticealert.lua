...
local function createNoticeMsg()
  ...
  for index = 1, _maxNoticeCount do
    ...
    noticePanel:ChangeTextureInfoNameAsync("")
...
function Proc_NoticeAlert_Ack(message, noticeType, noticeValue)
  ...
  if CppEnums.EChatNoticeType.GoldOrcShow == noticeType then
    if nil ~= message then
      FGlobal_WorldBossShow(message, noticeType, noticeValue)
    end
    return
  ...
  if CppEnums.EChatNoticeType.Ocean == noticeType or CppEnums.EChatNoticeType.Invasion == noticeType or CppEnums.EChatNoticeType.Noti == noticeType then
    if message ~= nil then
      if CppEnums.EChatNoticeType.Ocean == noticeType then
        ToClient_RequestNakMessageRenewelWithString(__eNakMessage_Ocean, message)
      elseif CppEnums.EChatNoticeType.Invasion == noticeType then
        ToClient_RequestNakMessageRenewelWithString(__eNakMessage_Invasion, message)
      elseif CppEnums.EChatNoticeType.Noti == noticeType then
        ToClient_RequestNakMessageRenewelWithString(__eNakMessage_Noti, message)
      end
    end
    return
  end
