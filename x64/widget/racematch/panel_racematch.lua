...
function FromClient_MatchRaceResult()
  ...
  for idx = 0, resultCount - 1 do
    ...
    if false == raceResultInfo:isEmpty() and 0 ~= raceResultInfo:getRank() then
      ...
      self.resultList[idx].bg:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Widget/Party/Matchresult_00.dds")
      ...
      if _rank > 3 then
        ...
        self.resultList[idx].rankIcon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Widget/Party/MatchResult_00.dds")
        local x1, y1, x2, y2 = 0, 0, 0, 0
        if 1 == _rank then
