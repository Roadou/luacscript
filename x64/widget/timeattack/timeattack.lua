...
function FromClient_endTimeAttack(isSuccess, clearTime)
  ...
  if true == isSuccess then
    ...
    local mainStr = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_TIMAATTACK_SUCCESSMSG", "time", timeString)
    ToClient_RequestNakMessageRenewelWithString(__eNakMessage_TimeAttackSuccessm, mainStr)
  elseif false == isSuccess then
    ToClient_RequestNakMessageRenewel(__eNakMessage_TimeAttackFail)
  end
