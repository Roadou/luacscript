...
function IngameCustomize_ShowActual(isOpenMaidActorMode)
  PaGlobalFunc_FullScreenFade_FadeOut(0.5, 0.5)
  if not IngameCustomize_isClearToShow() then
    return
  end
  local openMaidMode = false
  if _ContentsGroup_MaidActorSystem == true and isOpenMaidActorMode ~= nil then
    openMaidMode = isOpenMaidActorMode
  end
  if openMaidMode == true then
    if getCustomizingManager():showMaidActorMode() == false then
      return
    end
  elseif getCustomizingManager():show() == false then
    return
  end
