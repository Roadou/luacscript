...
function FieldViewMode_ChangeTexture_On()
  var_UI.panel:ChangeTextureInfoNameAsync("new_ui_common_forlua/default/window_sample_drag.dds")
...
function FieldViewMode_ChangeTexture_Off()
  if Panel_UIControl:IsShow() then
    var_UI.panel:ChangeTextureInfoNameAsync("new_ui_common_forlua/default/window_sample_isWidget.dds")
    fieldViewText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_FIELDVIEWMODE_UI"))
  else
    var_UI.panel:ChangeTextureInfoNameAsync("new_ui_common_forlua/default/window_sample_empty.dds")
  end
end
local function initialize()
  ...
  if not isGameServiceTypeConsole() then
    local savedCameraDist = ToClient_getGameUIManagerWrapper():getLuaCacheDataListNumber(__ePadCameraDistanceFromCharacter)
    local savedCameraPosX = ToClient_getGameUIManagerWrapper():getLuaCacheDataListNumber(__ePadCameraFromCharacterPosX)
    local savedCameraPosY = ToClient_getGameUIManagerWrapper():getLuaCacheDataListNumber(__ePadCameraFromCharacterPosY)
    if savedCameraDist ~= 0 or savedCameraPosX ~= 0 or savedCameraPosY ~= 0 then
      characterCameraSetPos(float2(savedCameraPosX, savedCameraPosY), savedCameraDist)
    else
      characterCameraReset()
    end
  end
