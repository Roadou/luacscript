...
function PaGlobalFunc_NPCShop_ALL_SellNonStackItem(index)
  if nil == Panel_Dialog_NPCShop_All or false == Panel_Dialog_NPCShop_All:GetShow() then
    return
  end
  PaGlobal_NPCShop_ALL._value._selectedSlotIndex = index
  local itemCount = ToClient_getNonStackItemCount(CppEnums.ItemWhereType.eInventory, PaGlobal_NPCShop_ALL._value._selectedSlotIndex, false, 0)
  itemCount = toInt64(0, itemCount)
  Panel_NumberPad_Show(true, itemCount, PaGlobal_NPCShop_ALL._value._selectedSlotIndex, PaGlobalFunc_NPCShop_ALL_SellNonStackItemToShop)
end
function PaGlobalFunc_NPCShop_ALL_SellNonStackItemToShop(itemCount, slotNo)
  ...
  local function sellDoit()
    getSelfPlayer():get():requestSellItem(slotNo, itemCount, fromWhereType, toWhereType, PaGlobal_NPCShop_ALL._value._isCamping)
  end
  local itemWrapper = getInventoryItem(slotNo)
  if itemWrapper == nil then
    return
  end
  if itemWrapper:isDisplayWorldMarket() ~= false then
