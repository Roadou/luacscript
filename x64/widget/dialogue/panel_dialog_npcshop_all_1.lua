function PaGlobal_NPCShop_ALL:initialize()
  ...
  self._ui._stc_HardCoreMoney_Icon = UI.getChildControl(self._ui._stc_myMoney_Area, "Static_HardCoreCoin")
  ...
  self._ui._stc_HardCoreMoney_Icon:SetShow(false)
  self._keyGuideList = {
...
function PaGlobal_NPCShop_ALL:pushKeyToList2()
  ...
  if self._isPadSnap == false then
    ...
    if self._value._currentTabIndex ~= self._ENUMTABINDEXREBUY then
    ...
    if dataIndex < self._value._itemListSize then
      ...
      if nil ~= shopItemWrapper then
        ...
        if self._ENUMTABINDEXBUY == self._value._currentTabIndex then
          ...
          local needDarkMoonBuff = shopItem:needDarkMoonBuff()
          PaGlobal_NPCShop_ALL:setItem(dataIndex, shopItemWrapper:getStaticStatus(), shopItem.leftCount_s64, itemPrice_s64, s64_inventoryItemCount, shopItem:getItemUsablePeriod(), shopItem:getNeedIntimacy(), nil, isLimitedBuyableCount, remainingCount, resetType, isLimitedByUser, buyableMaxCount, needDarkMoonBuff)
        elseif self._ENUMTABINDEXSELL == self._value._currentTabIndex then
...
function PaGlobal_NPCShop_ALL:updateMoney()
  ...
  if true == isGuildFlag then
    ...
    PaGlobalFunc_ShopTypeTradePriceIconChange(self._isSavedTradePrice, self._isSavedTradePriceType)
...
function PaGlobalFunc_ShopTypeTradePriceIconChange(isTradePrice, priceType)
  ...
  if true == isTradePrice then
    ...
    elseif priceType == __ePriceType_HardCore then
      self._ui._stc_Money_Icon:SetShow(false)
      self._ui._stc_OceanMoney_Icon:SetShow(false)
      self._ui._stc_HardCoreMoney_Icon:SetShow(true)
      self._ui._txt_myMoney:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_HARDCORENMONEY"))
      local mileageValue = Defines.s64_const.s64_0
...
function PaGlobal_NPCShop_ALL:buySome()
  ...
  if true == _ContentsGroup_Barter and true == self._isSavedTradePrice and self._isSavedTradePriceType == __ePriceType_Shell then
    local priceItemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eMoneyInventory, __eInventoryMoneyType_Shell)
  ...
  if true == _ContentsGroup_HardCoreChannel and true == self._isSavedTradePrice and self._isSavedTradePriceType == __ePriceType_HardCore then
    local priceItemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eMoneyInventory, __eInventoryMoneyType_HardCore)
    money_s64 = toInt64(0, 0)
    if nil ~= priceItemWrapper then
      money_s64 = priceItemWrapper:get():getCount_s64()
  ...
  if s64_maxMoneyNumber < s64_maxWeightNumber or s64_maxWeightNumber < toInt64(0, 0) then
  ...
  if 0 < shopItem:getNeedIntimacy() then
    ...
    if 0 == reduceIntimacyValue then
      ...
      s64_maxWeightNumber = toInt64(0, 0)
    else
...
function PaGlobal_NPCShop_ALL:buyOrSellItem(inputNumber)
  ...
  if self._ENUMTABINDEXBUY == self._value._currentTabIndex then
    ...
    if Panel_NumberPad_IsPopUp() and Panel_NumberPad_IsType("NpcShop_BuySome") then
      local isTradePrice = true == shopItemWrapper:getStaticStatus():isTradePrice()
      local tradePriceType = shopItemWrapper:getStaticStatus():getTradePriceType()
      ...
      if isTradePrice == true then
        if _ContentsGroup_Barter == true and tradePriceType == __ePriceType_Shell then
        ...
        elseif _ContentsGroup_HardCoreChannel == true and tradePriceType == __ePriceType_HardCore then
          contentString = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_NPCSHOP_BUY_ALERT_SEACOIN", "item", selectItemName, "number", tostring(buyCount), "price", makeDotMoney(totalPrice))
        end
      end
      local messageboxData = {
      ...
      if isTradePrice == true and tradePriceType == __ePriceType_Shell and buyCount > PaGlobal_NPCShop_ALL.POPUP_MESSAGEBOX_SEACOIN then
        MessageBox.showMessageBox(messageboxData, nil, nil, isUsePadSnapping)
      elseif isTradePrice == true and tradePriceType == __ePriceType_HardCore and buyCount > PaGlobal_NPCShop_ALL.POPUP_MESSAGEBOX_MONEY then
        MessageBox.showMessageBox(messageboxData, nil, nil, isUsePadSnapping)
...
function PaGlobal_NPCShop_ALL:ShowToolTipByPad(selectedIndex, show)
  ...
  if true == show then
    ...
    elseif self._ENUMTABINDEXSELL == self._value._currentTabIndex then
      ...
      PaGlobalFunc_TooltipInfo_Open(Defines.TooltipDataType.ItemSSWrapper, itemSSW, Defines.TooltipTargetType.Item, 0, getScreenSizeX())
    elseif self._ENUMTABINDEXREBUY == self._value._currentTabIndex then
      local itemWrapper = npcShop_getItemRepurchaseForToolTip(selectedIndex)
...
function PaGlobal_NPCShop_ALL:InvenItemRClick(idx)
  ...
  if nil ~= itemWrapper then
    ...
    if _ContentsGroup_MoveNonStackItem == true and itemStaticStatus:isStackable() == false and itemWrapper:get():getEndurance() == 32767 then
      PaGlobalFunc_NPCShop_ALL_SellNonStackItem(idx)
    elseif Defines.s64_const.s64_1 == itemCount then
...
function PaGlobal_NPCShop_ALL:setItem(dataIndex, itemStaticWrapper, s64_stackCount, s64_price, s64_invenCount, rentTime, Intimacy, disable, isLimitedBuyableCount, remainingCount, resetType, isLimitedByUser, buyableMaxCount, needDarkMoonBuff)
  ...
  if _ContentsGroup_DarkMoonAdditionalBuff == true and needDarkMoonBuff ~= nil and needDarkMoonBuff == true then
    createdSlot.desc:SetText(PAGetString(Defines.StringSheet_GAME, "NPCSHOP_NEED_DARKMOONBUFF"))
  end
  ...
  if true == _ContentsGroup_Barter then
    ...
    if true == itemStatic:isTradePrice() then
      local priceType = itemStatic:getTradePriceType()
      createdSlot.coinIcon:SetColor(4294967295)
      createdSlot.coinIcon:SetSize(19, 19)
      createdSlot.coinIcon:SetSpanSize(420, 37)
      if priceType == __ePriceType_Shell then
        createdSlot.coinIcon:ChangeTextureInfoNameAsync("Combine/Icon/Combine_Cashshop_Icon_00.dds")
      ...
      elseif priceType == __ePriceType_HardCore then
        myInvenMoney_s64 = toInt64(0, 0)
        createdSlot.coinIcon:ChangeTextureInfoTextureIDAsync("Combine_EdanaCoin_Icon")
      ...
      createdSlot.coinIcon:ChangeTextureInfoNameAsync("combine/icon/combine_basic_icon_00.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(createdSlot.coinIcon, 359, 103, 409, 153)
      createdSlot.coinIcon:getBaseTexture():setUV(x1, y1, x2, y2)
      createdSlot.coinIcon:setRenderTexture(createdSlot.coinIcon:getBaseTexture())
      createdSlot.coinIcon:SetSpanSize(415, 30)
    end
    if itemStatic:isTradePrice() == true then
      self._isSavedTradePrice = itemStatic:isTradePrice()
    end
    self._isSavedTradePriceType = itemStatic:getTradePriceType()
    PaGlobalFunc_ShopTypeTradePriceIconChange(itemStatic:isTradePrice(), itemStatic:getTradePriceType())
  ...
  elseif self._ENUMTABINDEXBUY == self._value._currentTabIndex then
    local isBuy = true
    if itemStatic:isTradePrice() then
