...
function SkillSpecialize:initValue()
  ...
  if __eSkillTypeParam_Inherit == skillTypeParam then
    self._ui.staticText_Skill_Awaken_Special_Title:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILLREINFORCE_SUCCESSION"))
  else
    local selfPlayer = getSelfPlayer()
    if nil ~= selfPlayer and __eClassType_ShyWaman == selfPlayer:getClassType() then
      ...
      self._ui.staticText_Skill_Awaken_Special_Title:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILLREINFORCE_AWAKEN"))
    end
  end
  self:initValuePage2()
end
...
function SkillSpecialize:setContentPage1()
  ...
  self._successionReinforceCount = 0
  for index = self._config.reinforceSkillCount, self._config.reinforceSkillViewCount - 1 do
    ...
    if __eSkillTypeParam_Normal == skillTypeParam or __eSkillTypeParam_Awaken == skillTypeParam then
      reinforceCount = awakenSkill_ReinforceCount
    else
      reinforceCount = successionSkill_ReinforceCount
    end
    if index < reinforceCount + 3 then
      control = self._ui.rbtn_List_ChangeSkill[index]
      if nil ~= control then
        control.state = self._enum.eButtonStateAlready
        self:setSkillButtonData(index, control, normalSkill_ReinforceCount)
      end
      currnetReinforceCount = currnetReinforceCount + 1
      totalReinforceCount = totalReinforceCount + 1
    else
      ...
      if nil ~= control then
        ...
        local selfPlayer = getSelfPlayer()
        if totalReinforceCount == reinforcableSkillCount or nil == selfPlayer or __eClassType_ShyWaman == selfPlayer:getClassType() then
          control.state = self._enum.eButtonStateLock
          control.staticText_EnableSpecialize:SetShow(false)
          self:changeTexturePage1Button(control.radiobutton, self._enum.eButtonStateLock)
          control.radiobutton:SetEnable(false)
          ...
          local skillTypeParam = ToClient_GetSelfPlayerSkillType()
          if __eSkillTypeParam_Inherit == skillTypeParam then
            control.staticText_EnableSpecialize:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_SKILLREINFORCE_SUCCESSION_1"))
          else
...
function SkillSpecialize:setSkillButtonData(buttonIndex, control, normalSkill_ReinforceCount)
  local _type = self:getSkillAwakeningType(buttonIndex)
  if nil == control then
    return
  end
  local thisIndex = buttonIndex
  if __eSkillTypeParam_Awaken == _type then
  ...
  for index = 0, reinforcedSkillcount - 1 do
    ...
    if nil ~= skillSSW then
      local awakeningType = skillSSW:getSkillAwakenBranchType()
      if 0 == _type and 0 == awakeningType then
        if thisIndex == normalSkill_ReinforceIndex then
          reinforceIndex = index
          break
        end
        normalSkill_ReinforceIndex = normalSkill_ReinforceIndex + 1
      elseif 1 == _type and 1 == awakeningType then
        if thisIndex == awakenSkill_ReinforceIndex then
          reinforceIndex = index
          break
        end
      ...
      elseif 2 == _type then
        if __eSkillTypeParam_Normal == awakeningType then
          if normalSkill_ReinforceCount == dupCounter then
            if index > self._successionReinforceCount then
              self._successionReinforceCount = index
              reinforceIndex = index
              break
            end
          else
        ...
        elseif __eSkillTypeParam_Inherit == awakeningType and index > self._successionReinforceCount then
          self._successionReinforceCount = index
          reinforceIndex = index
          break
        end
  ...
  control.static_Icon:ChangeTextureInfoNameAsync("Icon/" .. skillTypeSSW:getIconPath())
...
function SkillSpecialize:setPage2_0Step(skillNo)
  ...
  if __eSkillTypeParam_Inherit == _type then
    count = count + PaGlobalFunc_Dialog_SkillSpecialize_Reinforcable_SkillCount(__eSkillTypeParam_Normal)
  end
  if 0 == count then
...
function SkillSpecialize:setPage2SelectedSkill(skillNo)
  if nil == skillNo then
    ...
    self._ui.static_SkilllIcon:ChangeTextureInfoNameAsync("Icon/" .. skillTypeSSW:getIconPath())
    ...
    self._ui.static_SelectedSkillIcon:ChangeTextureInfoNameAsync("Icon/" .. skillTypeSSW:getIconPath())
...
function SkillSpecialize:changeTexturePage1Button(button, _type)
  ...
  button:ChangeTextureInfoNameAsync(self._texture.page1ButtonTexture)
    ...
    if nil ~= skillSSW then
      ...
      if _type == awakeningType or __eSkillTypeParam_Inherit == _type and __eSkillTypeParam_Normal == awakeningType then
        if count == _key then
          ...
          static_SkilllIcon:ChangeTextureInfoNameAsync("Icon/" .. skillTypeSSW:getIconPath())
...
function PaGlobalFunc_Dialog_SkillSpecialize_Doit()
  ...
  if nil == self._value.currnetReinforceIndex then
    ...
    local function doActual()
      ToClient_RequestSkillAwakening(__eSkillReinforceType_NonPlayer, self._value.currentSelectSkillIndex, self._value.currentEffectIndex, self._value.currentEffectIndex2)
    end
  ...
  elseif nil == self._value.currentSelectSkillIndex then
    ...
    ToClient_RequestChangeAwakeningBitFlag(__eSkillReinforceType_NonPlayer, self._value.currnetReinforceIndex, self._value.currentEffectIndex, self._value.currentEffectIndex2)
  else
    if nil == self._value.currentEffectIndex then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_SKILLREINFORCE_SELECTFIRSTOPTION"))
      return
    ...
    local function doActual()
      ToClient_RequestChangeAwakeningSkill(__eSkillReinforceType_NonPlayer, self._value.currnetReinforceIndex, self._value.currentSelectSkillIndex, self._value.currentEffectIndex, self._value.currentEffectIndex2)
    end
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILLAWAKEN_AWAKENTITLE"),
      content = getEffectText(self._value.currentEffectIndex) .. "\n" .. getEffectText(self._value.currentEffectIndex2),
