...
function SkillCooltime_Add_FromClient(TSkillKey, TSkillNo, isPlayingReplay, reuseCycle, iconPath)
  if true == isPlayingReplay then
    Panel_SkillCooltime:SetShow(true)
  end
  local slot = skillCooltime.slots[TSkillKey]
  if nil == slot then
    slot = skillCooltime:getSlot()
    skillCooltime.slots[TSkillKey] = slot
  end
  slot.iconBg:SetShow(false)
  local slotDesc = skillCooltime.slots_description[TSkillKey]
  if nil == slotDesc then
    slotDesc = {
      remainTime = skillCooltime.textRemainTime,
      skillNo = TSkillNo
    }
    skillCooltime.slots_description[TSkillKey] = slotDesc
  end
  skillReuseTime = reuseCycle
  slot:clearSkill()
  if slot.icon ~= nil then
    slot.icon:ChangeTextureInfoNameAsync("Icon/" .. iconPath)
    slot.icon:SetAlpha(1)
  end
  slotDesc.remainTime = skillCooltime.textRemainTime
...
function skillCooltime:registMessageHandler()
  registerEvent("EventSkillCooltime", "SkillCooltime_Add_FromClient")
