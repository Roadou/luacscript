function Panel_SkillTooltip_Show(slotNo, isShowNextLevel, SlotType, isReserveSkill, isResetable)
  ...
  local groupNo, groupLevel
  if PaGlobalFunc_SkillGroup_All_GetClearSkillGroupInfo ~= nil then
    groupNo, groupLevel = PaGlobalFunc_SkillGroup_All_GetClearSkillGroupInfo()
  elseif PaGlobalFunc_SkillGroup_GetMouseOnSkillGroupNo ~= nil then
    groupNo, groupLevel = PaGlobalFunc_SkillGroup_GetMouseOnSkillGroupNo()
  end
  if isShowNextLevel == true and groupNo ~= nil and groupNo ~= -1 and groupLevel ~= nil then
    local nextSkillkey = ToClient_getNextLevelSkill(groupNo, groupLevel)
    if nextSkillkey ~= nil and nextSkillkey ~= 0 then
      local nextSkillNo = nextSkillkey:getSkillNo()
      local nextSkillSS = getSkillTypeStaticStatus(nextSkillNo)
      if nextSkillSS ~= nil and nextSkillSS:isValidLocalizing() == true then
...
function HandleMOver_SkillWindow_ToolTipShow(skillNo, isShowNextLevel, SlotType, isFusion)
  if true == _ContentsGroup_RenewUI or true == _ContentsGroup_UsePadSnapping then
    if _ContentsGroup_ShowOtherClassSkill == true then
      if PaGlobal_SkillGroup_All ~= nil and PaGlobal_SkillGroup_All._ui._chk_playShow ~= nil and PaGlobal_SkillGroup_All._ui._chk_playShow:IsCheck() == true then
        return
      end
    elseif PaGlobal_SkillGroup ~= nil and nil ~= PaGlobal_SkillGroup._ui._stc_playShow and true == PaGlobal_SkillGroup._ui._stc_playShow:IsCheck() then
      return
    end
  end
...
function SkillEvent_SkillGroupWindow_ClearSkill(skillPointType)
  if PaGlobalFunc_SkillGroup_All_IsSearchState ~= nil and PaGlobalFunc_SkillGroup_All_IsSearchState() == true then
    return
  elseif PaGlobalFunc_SkillGroup_IsSearchState ~= nil and PaGlobalFunc_SkillGroup_IsSearchState() == true then
    return
  end
  ...
  if _ContentsGroup_ShowOtherClassSkill == true then
    PaGlobal_SkillGroup_All._targetSkillType = __eSkillTypeParam_Normal
  elseif PaGlobal_SkillGroup ~= nil then
...
function SkillEvent_SkillGroupWindow_ClearSkillMessage()
  if _ContentsGroup_ShowOtherClassSkill == true then
    PaGlobal_SkillGroup_All:update()
  else
