...
function PaGlobalFunc_FloatingTooltip_GetEnchantLevelStr(itemSSW)
  ...
  if enchantLevel > 0 then
    if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and (itemSSW:isSpecialEnchantItem() == false or itemSSW:isSpecialAccessory() == true) then
      enchantStr = HighEnchantLevel_ReplaceString(enchantLevel + 15)
    elseif 1 == itemSSW:getItemType() and enchantLevel > 15 then
      enchantStr = HighEnchantLevel_ReplaceString(enchantLevel)
    end
  end
  if enchantStr == "" then
    return nil
...
function PaGlobalFunc_FloatingTooltip_GetPanel()
  return _panel
end
function PaGlobalFunc_FloatingTooltip_GetDefaultGap()
  return self._defaultXGap
end
