...
local _targetData = {
  [Defines.TooltipTargetType.Item] = {
    ...
    COMPONENT_TYPE.ITEMSKILLOPTION,
    ...
    COMPONENT_TYPE.ENDURANCE_RECOVERY,
...
local _config = {createIcon = true, createEnchant = true}
...
function TooltipInfo:initialize()
  _componentOption = {
    ...
    [COMPONENT_TYPE.ITEMSKILLOPTION] = {fillDataFunc = TooltipInfo_updateITEMSKILLOPTION, isInsideClippingArea = true},
    [COMPONENT_TYPE.ENDURANCE_RECOVERY] = {fillDataFunc = TooltipInfo_updateENDURANCE_RECOVERY, isInsideClippingArea = true}
  }
  ...
  self._ui.stc_components = {
    [1] = {
      ...
      [COMPONENT_TYPE.ITEMSKILLOPTION] = UI.getChildControl(self._ui.stc_scrollContent[1], "ITEMSKILLOPTION_INFO"),
      ...
      [COMPONENT_TYPE.ENDURANCE_RECOVERY] = UI.getChildControl(self._ui.stc_scrollContent[1], "ENDURANCE_RECOVERY")
    }
  }
  ...
  for ii = 2, self._poolCount do
    ...
    self._ui.stc_components[ii][COMPONENT_TYPE.ITEMSKILLOPTION] = UI.getChildControl(self._ui.stc_scrollContent[ii], "ITEMSKILLOPTION_INFO")
    ...
    self._ui.stc_components[ii][COMPONENT_TYPE.ENDURANCE_RECOVERY] = UI.getChildControl(self._ui.stc_scrollContent[ii], "ENDURANCE_RECOVERY")
  end
  self._ui.stc_BG[2]:ChangeTextureInfoNameAsync("renewal/frame/console_comboguidebg_02.dds")
...
function PaGlobalFunc_TooltipInfo_SolareEquipPresetOpen(tooltipDataType, data, tooltipTargetType, targetX, compareX, slotType, enchantKey)
  TooltipInfo:open(tooltipDataType, data, tooltipTargetType, targetX, compareX, nil, enchantKey, slotType, nil)
end
function PaGlobalFunc_TooltipInfo_AbyssOnePresetOpen(tooltipDataType, data, tooltipTargetType, targetX, compareX, slotType, enchantKey)
  TooltipInfo:open(tooltipDataType, data, tooltipTargetType, targetX, compareX, nil, enchantKey, slotType, nil)
end
...
function TooltipInfo:open(tooltipDataType, data, tooltipTargetType, targetX, compareX, isMyWallet, enchantKey, slotType, isEquippedItem)
  ...
  local itemWrapper
  local itemSSW = data
  local equipPresetCompareKey
  if slotType == "SolarePreset" then
    equipPresetCompareKey = enchantKey
    enchantKey = nil
  end
  ...
  elseif slotType == "SolarePreset" then
    local presetIndex = highFromUint64(equipPresetCompareKey)
    local equipSlotNo = lowFromUint64(equipPresetCompareKey)
    local compareEquipedItemSSW = ToClient_GetSolareEquippedItemWrapper(presetIndex, equipSlotNo)
    if compareEquipedItemSSW ~= nil then
      local equipItemSSW = compareEquipedItemSSW:getStaticStatus()
      self._compareItemWrapper = compareEquipedItemSSW
      ...
      self:compose(compareEquipedItemSSW, equipItemSSW, tooltipTargetType, xPos, isMyWallet, enchantKey, slotType, true)
...
function TooltipInfo:close()
  ...
  if true == _ContentsGroup_RenewUI then
    ...
    if PaGlobal_TutorialManager ~= nil and false == PaGlobal_TutorialManager:isDoingTutorial() then
...
function TooltipInfo_updateHEADER(itemWrapper, itemSSW, tooltipTargetType)
  ...
  slotBG:ChangeTextureInfoNameAsync("renewal/frame/console_frame_00.dds")
  ...
  if nil ~= itemWrapper then
    ...
    if isSoulCollector then
      self._ui.slot_itemIcon[_currentPool].icon:ChangeTextureInfoNameAsync("icon/" .. itemWrapper:getStaticStatus():getIconPath())
      if 0 == isCurrentSoulCount then
        self._ui.slot_itemIcon[_currentPool].icon:ChangeTextureInfoNameAsync("icon/" .. itemWrapper:getStaticStatus():getIconPath())
      elseif isCurrentSoulCount > 0 and isCurrentSoulCount < isMaxSoulCount then
        self._ui.slot_itemIcon[_currentPool].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/ExceptionIcon/00040351_1.dds")
      else
        self._ui.slot_itemIcon[_currentPool].icon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/ExceptionIcon/00040351_2.dds")
      end
    end
  end
  return true
    ...
    if ii <= slotCount then
      ...
      if nil ~= itemWrapper then
        ...
        if nil ~= crystalESSW then
          ...
          self._ui.stc_socketIcon[_currentPool][ii]:ChangeTextureInfoNameAsync("icon/" .. crystalESSW:getIconPath())
...
function TooltipInfo_updateARTIFACTS(itemWrapper, itemSSW, tooltipTargetType, isMyWallet, enchantKey, slotType)
  ...
  for jewelIdx = 0, soketCount - 1 do
    ...
    if item ~= nil then
      ...
      elseif slotType ~= nil and slotType == "ArtifactPreset" and isArtifact == true and _ContentsGroup_LightStoneBag == true and PaGlobalFunc_ArtifactPreset_All_CurrentMouseOverPresetNo ~= nil then
...
function TooltipInfo_updateDESCRIPTION(itemWrapper, itemSSW, tooltipTargetType, isMyWallet, enchantKey, slotType)
    ...
    for index = 0, bagSize - 1 do
      ...
      if bagItemWrapper ~= nil then
        ...
        elseif isSeedBag == true then
          ...
          local addString = tostring(bagItemSS:getName())
  ...
  if itemSSW:get():isCash() and nil ~= itemWrapper and itemWrapper:get():isSealed() and requestIsRegisterItemForItemMarket(itemWrapper:get():getKey()) then
    if desc ~= "" then
      desc = desc .. [[
    ...
    desc = desc .. PAGetString(Defines.StringSheet_GAME, "LUA_ITEMTOOLTIP_TAXFREE") .. [[
...
- ]] .. itemSSW:getEnchantDescription()
  ...
  content2:SetPosY(content:GetTextSizeY() + content1:GetTextSizeY())
...
function TooltipInfo_updateITEMSKILLOPTION(itemWrapper, itemSSW, tooltipDataType)
  ...
  if __eContentsType_ItemSkillOption ~= itemSSW:getContentsEventType() then
    return false
  end
  local appliedSkillCount = itemWrapper:getAppliedItemSkillOptionCount()
  if appliedSkillCount <= 0 then
    return false
  end
  local buffText = PaGlobalFunc_ItemSkillOption_GetSkillDesc(itemWrapper, "\n")
  local component = self._ui.stc_components[_currentPool][COMPONENT_TYPE.ITEMSKILLOPTION]
  local header = UI.getChildControl(component, "StaticText_Content")
  local content = UI.getChildControl(component, "StaticText_Content1")
  header:SetShow(true)
  content:SetShow(true)
  content:SetPosY(header:GetSizeY() + _defaultGapBetweenComponent)
  content:SetTextMode(__eTextMode_AutoWrap)
  content:SetText("<PAColor0xff96d4fc>" .. buffText .. "<PAOldColor>")
  content:SetSize(_defaultSizeX, content:GetTextSizeY())
  component:SetSize(content:GetSizeX(), header:GetTextSizeY() + content:GetTextSizeY() + _defaultGapBetweenComponent)
  return true
end
...
function TooltipInfo_updateENDURANCE_RECOVERY(itemWrapper, itemSSW, tooltipTargetType)
  if nil == itemWrapper then
    return false
  end
  if nil == itemSSW then
  ...
  local component = self._ui.stc_components[_currentPool][COMPONENT_TYPE.ENDURANCE_RECOVERY]
  local recoveryStaticText = UI.getChildControl(component, "StaticText_ItemRecoveryDesc")
  recoveryStaticText:SetTextMode(__eTextMode_AutoWrap)
  recoveryStaticText:SetAutoResize(true)
  local dynamicMaxEndurance = itemWrapper:get():getMaxEndurance()
  if itemSSW:isEquipable() and dynamicMaxEndurance <= 50 and 0 < itemSSW:getRecoveryMaxEndurance() then
    if itemSSW:getGradeType() < __eItemGradeEpic then
      recoveryStaticText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_ITEMTOOLTIP_RECOVOERYDESC"))
    else
      recoveryStaticText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_ITEMTOOLTIP_RECOVOERYDESC2"))
    end
  else
    return false
  end
  component:SetSize(_defaultSizeX, recoveryStaticText:GetTextSizeY())
  return true
end
