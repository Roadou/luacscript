...
function PaGlobal_TooltipSkill_All:registEventHandler()
  ...
  if _ContentsGroup_ShowOtherClassSkill == true then
    registerEvent("EventSkillWindowShowForLearn", "PaGlobalFunc_SkillGroup_All_Open")
  else
...
function PaGlobal_TooltipSkill_All:showTooltip(target, skillNo, skillTypeSS, isShowNextLevel, isReserveSkillOn, isResetable, isForceResetable, isItemSkillTooltip)
  ...
  local level = 1
  if getLearnedSkillLevel(skillTypeSS) > 0 then
    level = getLearnedSkillLevel(skillTypeSS)
  end
  ...
  target.skillIcon:ChangeTextureInfoNameAsync("icon/" .. skillTypeSSW:getIconPath())
  ...
  if isLearnRequireInfo then
    ...
    if "RoseWarSkill" == self.TooltipSkillGroup._callTooltipType then
      isLevelShow = false
    end
