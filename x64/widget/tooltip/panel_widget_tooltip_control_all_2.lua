function PaGlobal_Tooltip_Control_All:setTooltipInfo(target, inputValue, isSSW, isItemWrapper, chattingLinkedItem, index, isNextEnchantInfo, invenSlotNo, itemNamingStr)
  ...
  target._ui.stc_itemIcon:ChangeTextureInfoNameAsync("icon/" .. itemSSW:getIconPath())
  if nil ~= itemWrapper then
    ...
    if isSoulCollector then
      target._ui.stc_itemIcon:ChangeTextureInfoNameAsync("icon/" .. itemWrapper:getStaticStatus():getIconPath())
      if 0 == isCurrentSoulCount then
        target._ui.stc_itemIcon:ChangeTextureInfoNameAsync("icon/" .. itemWrapper:getStaticStatus():getIconPath())
      elseif isCurrentSoulCount > 0 and isCurrentSoulCount < isMaxSoulCount then
        target._ui.stc_itemIcon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/ExceptionIcon/00040351_1.dds")
      else
        target._ui.stc_itemIcon:ChangeTextureInfoNameAsync("New_UI_Common_forLua/ExceptionIcon/00040351_2.dds")
      end
    end
  end
  local enchantLevel = itemSSW:get()._key:getEnchantLevel()
  ...
  for jewelIdx = 0, 5 do
    if jewelIdx < socketMaxCount - 1 then
      ...
      elseif nil ~= itemEnchantSSW then
        ...
        target._ui.stc_soketSlot[jewelIdx + 1]:ChangeTextureInfoNameAsync("icon/" .. itemEnchantSSW:getIconPath())
        ...
        target._ui.stc_soketSlot[jewelIdx + 1]:ChangeTextureInfoNameAsync(" ")
      end
    else
      target._ui.txt_soketName[jewelIdx + 1]:SetShow(false)
      target._ui.txt_soketEffect[jewelIdx + 1]:SetShow(false)
  ...
  if isTradeItem then
    ...
    if s64_sellPrice > _const.s64_0 and 0 == enchantLevel then
      if true == itemSSW:isTradePrice() then
        ...
        target._ui.txt_itemPrice_storeSell_value:SetText(priceText)
      else
...
function PaGlobal_Tooltip_Control_All:setEnchantLevel(target, itemSSW)
  ...
  if enchantLevel > 0 and enchantLevel < 26 then
    target._ui.txt_enchantLevel:SetText(HighRomaEnchantLevel_ReplaceString(enchantLevel))
    if _ContentsGroup_KoreanEnchantGrade == true then
      target._ui.txt_enchantLevel:SetText(HighKoreanEnchantLevel_ReplaceString(enchantLevel))
    end
  ...
  if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and (itemSSW:isSpecialEnchantItem() == false or itemSSW:isSpecialAccessory() == true) then
    target._ui.txt_enchantLevel:SetText(HighRomaEnchantLevel_ReplaceString(enchantLevel + 15))
    if _ContentsGroup_KoreanEnchantGrade == true then
      target._ui.txt_enchantLevel:SetText(HighKoreanEnchantLevel_ReplaceString(enchantLevel + 15))
    end
    target._ui.txt_enchantLevel:SetShow(true)
  end
end
function PaGlobal_Tooltip_Control_All:setTooltipPos(target, item, itemSSW, itemWrapper, bottomTarget, changeItemNamePos, iconMovePos, elementgap, dynamicMaxEndurance, chattingLinkedItem, useLimitShow, soketCount)
  ...
  if self._NORMALTOOLTIP == target and true == self._dataObject.inventory then
    if (isGameTypeKorea() or isGameTypeJapan() or isGameTypeRussia() or isGameTypeNA() or isGameTypeTaiwan()) and getContentsServiceType() ~= CppEnums.ContentsServiceType.eContentsServiceType_CBT then
  ...
  if not itemSSW:get():isCash() then
    if itemSSW:isEquipable() and 0 < itemSSW:getRecoveryMaxEndurance() and dynamicMaxEndurance <= 50 and 16145 ~= itemSSW:get()._key:getItemKey() and not _ContentsGroup_RenewUI then
      ...
      if itemSSW:getGradeType() < __eItemGradeEpic then
        ...
        target._ui.txt_recoveryDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_ITEMTOOLTIP_RECOVOERYDESC2"))
      end
    else
      target._ui.txt_recoveryDesc:SetShow(false)
    end
