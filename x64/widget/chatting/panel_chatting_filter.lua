...
function ChattingFilter:Close()
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    self.ui.btn_Filter:SetIgnore(false)
...
function ChattingFilter_InsertFilterString()
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    local result = ToClient_CheckChinaProhibitedWord(filterString, __eSceneIdChattingFilter)
    if result == true then
      self.ui.btn_Filter:SetIgnore(true)
      self.ui.edit_Filter:RegistReturnKeyEvent("")
    end
    return
  end
...
function FromClient_CheckChinaProhibitedWord_ChattingFilter(isPass, filteredText)
