...
local chatInput = {
  config = {
    ...
    btnPosYGap = 21,
    sizeX = 352,
    sizeXOffsetForChatLanguageIcon = 20,
    toggleType = {
      chattingMacro = 0,
      socialAction = 1,
      emoticon = 2
    },
    toggleButtonsPaddingX = 5
  },
  control = {
    ...
    buttons = Array.new(),
    chatInputLanguage = UI.getChildControl(Panel_Chatting_Input, "StaticText_ChatLanguage"),
...
function ChatInput_GetRecentChatType()
  ...
  if ToClient_HardCoreChannelWithContensOption() == true then
    lastChatType = UI_CT.World
  end
  return lastChatType
end
local lastMemoryChatType = ChatInput_GetRecentChatType()
function chatInput:init()
  ...
  if isGameTypeKorea() then
    ...
    ChatInput_Resize()
    chatInput.control.chatInputLanguage:SetShow(true)
    chatInput.control.chatInputLanguage:SetPosX(self.control.edit:GetPosX() + self.control.edit:GetSizeX() + 9)
    chatInput.control.whisperEdit:SetPosX(Panel_Chatting_Input:GetSizeX() - 4)
...
function ChatInput_UpdateChannelChatBtn(isShow)
  ...
  if true == _ContentsGroup_ChannelChat then
    if true == isShow then
      local lastChatType = ChatInput_GetRecentChatType()
...
function chatInput:clearLinkedQuest()
  self.linkedQuestCount = 0
  for i = 0, self.maxLinkedQuestCount - 1 do
    self.linkedQuestData[i] = nil
  end
  self.control.edit:SetCursorMove(true)
end
  ...
  if self.isChatTypeChangedMode then
    ...
    if permission then
      ...
      if self.lastChatType == UI_CT.World or self.lastChatType == UI_CT.Guild or self.lastChatType == UI_CT.Public or self.lastChatType == UI_CT.Party or self.lastChatType == UI_CT.WorldWithItem then
        ...
        ToClient_ClearLinkedQuestInfo()
        chatInput:clearLinkedQuest()
      end
      ...
      SetToggleButtonsPosition(false)
...
function ChatInput_UpdatePermission()
  ...
  if nil ~= selfPlayerWrapper then
    ...
    if true == isLocalWar or true == isArsha then
    ...
    if ToClient_HardCoreChannelWithContensOption() == true then
      self.permissions[UI_CT.Private] = false
      self.permissions[UI_CT.Public] = false
      self.permissions[UI_CT.Team] = false
      self.permissions[UI_CT.WorldWithItem] = false
      self.permissions[UI_CT.RolePlay] = false
      self.permissions[UI_CT.Arsha] = false
      self.permissions[UI_CT.Guild] = false
      self.permissions[UI_CT.Alliance] = false
      self.permissions[UI_CT.GuildManager] = false
      self.permissions[UI_CT.Channel] = false
    end
  end
...
function ChatInput_SendMessage()
  local self = chatInput
  local memoryChatType = ChatInput_GetRecentChatType()
...
function ChatInput_Show()
  ...
  if false == ToClient_isCommunicationAllowed() then
    ...
    ToClient_showPrivilegeError(false, false, true)
    return
  end
  ...
  local self = chatInput
  SetFocusEdit(chatInput.control.edit)
  local isLastChatType = ChatInput_GetRecentChatType()
  ...
  if true == isGameTypeKorea() then
    local isNative = ToClient_isImeNativeConversionMode()
    SetChatInputLanaugeText(isNative)
  end
end
...
function HandleClicked_clickNameType()
  ...
  if preNameType == 1 or ToClient_HardCoreChannelWithContensOption() == true then
...
function FGlobal_ChattingInput_LinkedItemByInventory(slotNo, inventoryType)
  ...
  if chatInput.linkedQuestCount ~= 0 then
    return
  end
...
function isChatInputLinkedQuest(groupId, questId)
  local questStaticStatus = questList_getQuestStatic(groupId, questId)
  if questStaticStatus == nil then
    return
  end
  questName = questStaticStatus:getTitle()
  local linkedQuestPos = {startPos = 0, endPos = 0}
  local oldStr = chatInput.control.edit:GetEditText()
  linkedQuestPos.startPos = chatting_linkedItemRealPos(oldStr)
  local newStr = oldStr .. "[" .. questName .. "]"
  linkedQuestPos.endPos = chatting_linkedItemRealPos(newStr)
  ReleaseImeComposition()
  chatInput.control.edit:SetEditText(newStr, true)
  if chatInput.control.edit:GetEditTextSize() <= chatInput.maxEditInput then
    chatInput.linkedQuestData[chatInput.linkedQuestCount] = linkedQuestPos
    return true
  else
    chatInput.control.edit:SetEditText(oldStr, true)
    return false
...
function FGlobal_ChattingInput_LinkedQuest(groupId, questId)
  if UI_CT.World ~= chatInput.lastChatType and UI_CT.Guild ~= chatInput.lastChatType and UI_CT.Public ~= chatInput.lastChatType and UI_CT.Party ~= chatInput.lastChatType and UI_CT.WorldWithItem ~= chatInput.lastChatType and UI_CT.Private ~= chatInput.lastChatType and UI_CT.RolePlay ~= chatInput.lastChatType and UI_CT.Channel ~= chatInput.lastChatType and UI_CT.GuildManager ~= chatInput.lastChatType then
    return
  end
  if chatInput.maxLinkedQuestCount <= chatInput.linkedQuestCount then
    return
  end
  if chatInput.linkedItemCount ~= 0 then
    return
  end
  if _ContentsGroup_RenewUI_Chatting == true then
    if PaGlobalFunc_ChattingInfo_GetShow() == false then
      PaGlobalFunc_ChattingInfo_Open()
    end
  elseif Panel_Chatting_Input:IsShow() == false then
    ChatInput_Show()
  end
  chatInput.linkedQuestCount = chatInput.linkedQuestCount + 1
  if isChatInputLinkedQuest(groupId, questId) then
    chatInput.control.edit:SetCursorMove(false)
    chatInput.control.edit:SetCursorPosIndex(0)
    ToClient_SetLinkedQuestInfo(groupId, questId, chatInput.linkedQuestData[chatInput.linkedQuestCount].startPos, chatInput.linkedQuestData[chatInput.linkedQuestCount].endPos)
  end
end
...
function ChatInput_CheckRemoveLinkedQuest()
  if UI_CT.World ~= chatInput.lastChatType and UI_CT.Guild ~= chatInput.lastChatType and UI_CT.Public ~= chatInput.lastChatType and UI_CT.Party ~= chatInput.lastChatType and UI_CT.WorldWithItem ~= chatInput.lastChatType and UI_CT.Private ~= chatInput.lastChatType and UI_CT.RolePlay ~= chatInput.lastChatType and UI_CT.Channel ~= chatInput.lastChatType then
    return
  end
  if chatInput.linkedQuestCount <= 0 then
  ...
  local str = chatInput.control.edit:GetEditText()
  local editStrLen = string.len(str)
  if editStrLen < chatInput.linkedQuestData[chatInput.linkedQuestCount].endPos then
    if 0 == chatInput.linkedQuestData[chatInput.linkedQuestCount].startPos then
      chatInput.control.edit:SetEditText("", true)
    else
      chatInput.control.edit:SetEditText(string.sub(str, 1, chatInput.linkedQuestData[chatInput.linkedQuestCount].startPos), true)
    end
    ToClient_ClearLinkedQuestInfo()
    chatInput:clearLinkedQuest()
  end
...
function ChatInput_Resize()
  if true == isGameTypeKorea() then
    Panel_Chatting_Input:SetSize(chatInput.config.sizeX + chatInput.config.sizeXOffsetForChatLanguageIcon, 30)
  else
    Panel_Chatting_Input:SetSize(chatInput.config.sizeX, 30)
  end
...
function FromClient_ChatInput_ChangeConversionStatus(isNative)
  SetChatInputLanaugeText(isNative)
end
function SetChatInputLanaugeText(isNative)
  if true == isGameTypeKorea() then
    if true == isNative then
      chatInput.control.chatInputLanguage:SetText("\234\176\128")
    else
      chatInput.control.chatInputLanguage:SetText("A")
    end
  end
end
function SetToggleButtonsPosition(isWhisper)
  local controlStartPosX
  if true == isWhisper then
    controlStartPosX = chatInput.control.whisperEdit:GetPosX() + chatInput.control.whisperEdit:GetSizeX() + chatInput.config.toggleButtonsPaddingX
  else
    controlStartPosX = Panel_Chatting_Input:GetSizeX() + chatInput.config.toggleButtonsPaddingX
  end
  chatInput.control.macroButton:SetPosX(controlStartPosX)
  controlStartPosX = controlStartPosX + chatInput.control.macroButton:GetSizeX() + chatInput.config.toggleButtonsPaddingX
  chatInput.control.socialButton:SetPosX(controlStartPosX)
  controlStartPosX = controlStartPosX + chatInput.control.socialButton:GetSizeX() + chatInput.config.toggleButtonsPaddingX
  chatInput.control.radioButton_Emoticon:SetPosX(controlStartPosX)
  if chatInput.control.radioButton_Emoticon:GetShow() then
    controlStartPosX = controlStartPosX + chatInput.control.radioButton_Emoticon:GetSizeX() + chatInput.config.toggleButtonsPaddingX
  end
  chatInput.control.button_Guide:SetPosX(controlStartPosX)
end
registerEvent("FromClient_ChangeConversionStatus", "FromClient_ChatInput_ChangeConversionStatus")
