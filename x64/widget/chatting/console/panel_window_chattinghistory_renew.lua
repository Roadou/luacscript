...
function ChattingHistory:initialize()
  ...
  for ii = 1, self._tabCount do
    ...
    radioButton:ChangeTextureInfoNameAsync(_tabButtonTextrue[ii])
...
function ChattingHistory:createPool(poolIndex)
  local ChatUIPool = {
    ...
    _maxcount_ChattingLinkedQuest = ChattingHistory._cloneCount,
  ...
  function ChatUIPool:initialize(poolIndex)
    ...
    for ii = 1, ch._cloneCount do
      ...
      ChatUIPool._list_ChattingLinkedQuest[ii] = UI.createAndCopyBasePropertyControl(ch._ui.stc_sampleBG, "StaticText_ChatLinkedQuest", ch._listPanel[poolIndex], "StaticText_ChatLinkedQuest" .. ii)
  ...
  function ChatUIPool:newChattingLinkedQuest(messageIndex)
    self._count_ChattingLinkedQuest = self._count_ChattingLinkedQuest + 1
    self._list_LinkedQuestMessageIndex[self._count_ChattingLinkedQuest - 1] = messageIndex
    return self._list_ChattingLinkedQuest[self._count_ChattingLinkedQuest - 1]
  end
  ...
  function ChatUIPool:clearChattingLinkedQuest(messageIndex)
    self._count_ChattingLinkedQuest = 1
  end
  ...
  function ChatUIPool:clear()
    ...
    self._list_LinkedQuestMessageIndex = {}
  ...
  function ChatUIPool:hideNotUse()
    ...
    for index = self._count_ChattingLinkedQuest, self._maxcount_ChattingLinkedQuest do
      self._list_ChattingLinkedQuest[index]:SetShow(false)
    end
...
function PaGlobalFunc_ChattingInfo_InviteParty_InParty()
  ...
  if isEnterPersonalField == false and isEnterPersonalRelayField == false then
...
function ChattingHistory:updateSelectionBox()
  ...
  self._ui.stc_selectedLine:SetMonoTone(false == _snappedOnThisPanel and PaGlobalFunc_PartySetting_GetShow ~= nil and PaGlobalFunc_PartySetting_GetShow() == true)
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chatCount = chatPanel:getMessageCount()
  if chatCount < 1 then
  ...
  if PaGlobalFunc_PartySetting_GetShow ~= nil and PaGlobalFunc_PartySetting_GetShow() == true then
...
function PaGlobalFunc_ChattingHistory_GetSelectedSenderForPartyInvite(nameType)
  ...
  if _ContentsGroup_PartyMatching_Limit_OtherServerInvite == true then
    if chattingMessage ~= nil and chattingMessage:getSender(nameType) ~= nil and chattingMessage.chatType ~= CHAT_TYPE.System and chattingMessage.chatType ~= CHAT_TYPE.Notice and chattingMessage.isMe == false then
      return chattingMessage:getSender(nameType)
    else
      return nil
    end
  elseif nil ~= chattingMessage and nil ~= chattingMessage:getSender(nameType) and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and true == chattingMessage.isSameChannel and false == chattingMessage.isMe then
    return chattingMessage:getSender(nameType)
  else
    return nil
  end
