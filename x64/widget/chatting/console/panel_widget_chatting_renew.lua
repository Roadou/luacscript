...
function ChattingInfo:open()
  ...
  if Panel_Window_Knowledge_Renew ~= nil and Panel_Window_Knowledge_Renew:GetShow() then
    PaGlobalFunc_Window_Knowledge_Exit()
  end
  if PaGlobal_PetList_Close_All ~= nil then
    PaGlobal_PetList_Close_All()
  end
  if PaGlobalFunc_WorkerManager_All_Close ~= nil then
    PaGlobalFunc_WorkerManager_All_Close()
  end
  if PaGlobalFunc_AlchemyKnowledgeCheckShow ~= nil and PaGlobalFunc_AlchemyKnowledgeCheckShow() then
    PaGlobalFunc_AlchemyKnowledgeClose()
  end
  if Panel_Window_Manufacture_All ~= nil and Panel_Window_Manufacture_All:GetShow() then
    PaGlobalFunc_Manufacture_All_Close()
  end
  if Panel_Window_Achievement_BookShelf_All ~= nil and Panel_Window_Achievement_BookShelf_All:GetShow() then
...
function ChattingInfo:clearLinkedQuest()
  self.linkedQuestCount = 0
  for ii = 0, self.maxLinkedQuestCount - 1 do
    self.linkedQuestData[ii] = nil
  end
  if nil ~= self._ui.edit_chatContent then
    self._ui.edit_chatContent:SetCursorMove(true)
  end
...
function PaGlobalFunc_ChattingInfo_CheckRemoveLinkedQuest()
  local self = ChattingInfo
  if CHAT_TYPE.World ~= self._lastChatType and CHAT_TYPE.Guild ~= self._lastChatType and CHAT_TYPE.Public ~= self._lastChatType and CHAT_TYPE.Party ~= self._lastChatType and CHAT_TYPE.WorldWithItem ~= self._lastChatType and CHAT_TYPE.Private ~= self._lastChatType then
    return
  end
  if self.linkedQuestCount <= 0 then
    return
  end
  local str = self._ui.edit_chatContent:GetEditText()
  local editStrLen = string.len(str)
  if editStrLen < self.linkedQuestData[self.linkedQuestCount].endPos then
    if 0 == self.linkedQuestData[self.linkedQuestCount].startPos then
      self._ui.edit_chatContent:SetEditText("", true)
    else
      self._ui.edit_chatContent:SetEditText(string.sub(str, 1, self.linkedQuestData[self.linkedQuestCount].startPos), true)
    end
    ToClient_ClearLinkedQuestInfo()
    self:clearLinkedQuest()
  end
