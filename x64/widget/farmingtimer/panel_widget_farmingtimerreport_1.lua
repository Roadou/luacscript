...
function PaGlobal_FarmingTimerReport:itemEnchantLevel(itemSSW)
  ...
  if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and (itemSSW:isSpecialEnchantItem() == false or itemSSW:isSpecialAccessory() == true) then
    printenchantLevel = HighRomaEnchantLevel_ReplaceString(enchantLevel + 15)
  else
    ...
    if enchantLevel > 0 and enchantLevel > 0 and enchantLevel < 26 then
...
function PaGlobal_FarmingTimerReport:experienceIconSetting(control, experienceType)
  ...
  local stc_Icon = UI.getChildControl(control, "Static_ItemIcon")
  stc_Icon:SetShow(true)
  stc_Icon:ChangeTextureInfoNameAsync("Combine/Icon/Combine_Tab_Icon_00.dds")
