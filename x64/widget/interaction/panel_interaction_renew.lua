...
local _button_ID = {
  ...
  [__eInteractionType_WildHorserRegisterCarriage] = "Button_WildHorserRegisterCarriage"
}
local _button_TextureUV = {
  ...
  [__eInteractionType_WildHorserRegisterCarriage] = {
    x1 = 126,
    y1 = 168,
    x2 = 156,
    y2 = 198
...
function PanelInteraction:init()
  ...
  _desc_Text[__eInteractionType_WildHorserRegisterCarriage] = PAGetString(Defines.StringSheet_GAME, "LUA_INTERACTION_WILDHORSE_REGISTER_CARRIAGE_TITLE")
  for ii = 0, eInteractionTypeMax do
  ...
  _interaction_Button_Text[__eInteractionType_WildHorserRegisterCarriage] = PAGetString(Defines.StringSheet_GAME, "LUA_INTERACTION_WILDHORSE_REGISTER_CARRIAGE_TITLE")
...
function PanelInteraction:show(actor)
  ...
  for ii, value in pairs(_interactionTargetUIList) do
    ...
    if CppEnums.InteractionType.InteractionType_Collect == ii and isShow == true then
      isShow = false
      ...
      for ii, value in pairs(_collectInteractionTargetUIList) do
        local isAble = collectWrapper:getCharacterStaticStatusWrapper():isCollectableToolType(_collect_Button_ID[ii])
        if actor:get():isHoeCharacter() == true then
          if _collect_Button_ID[ii] == __eCollectToolType_Hoe then
            isAble = true
          else
            isAble = false
          end
...
function PanelInteraction:getButtonIcon(control, index)
  ...
  if index == __eInteractionType_ClearDeadCountInElephantBarn then
    control:ChangeTextureInfoNameAsync("combine/console/Console_Function_Interction_01.dds")
  end
...
function PaGlobalFunc_PanelInteraction_ButtonPosUpdate(deltaTime)
  ...
  if ANIMATION_TIME < self._buttonTimeAcc then
    ...
    local actor = interaction_getInteractable()
    local collect_index2 = 0
    for collect_index1, value in pairs(_collectInteractionTargetUIList) do
      local isShow = _collectInteractionTargetUIList[collect_index1]:GetShow()
      if true == isShow then
        local div = collect_index2 / self._SHOW_BUTTON_COUNT
        ...
        _collectInteractionTargetUIList[collect_index1]:SetPosX(buttonPosX)
        _collectInteractionTargetUIList[collect_index1]:SetPosY(buttonPosY)
        _collectInteractionTargetUIList[collect_index1]:SetAlpha(aniRate)
        self._ui.needCollectTool:SetPosX(buttonPosX + 2)
        self._ui.needCollectTool:SetPosY(buttonPosY + 2)
        collect_index2 = collect_index2 + 1
      end
    end
    local jj = 0
    for ii, value in pairs(_interactionTargetUIList) do
...
function PaGlobalFunc_PanelInteraction_ButtonPushed(interactionType)
  ...
  elseif __eInteractionType_WildHorserRegisterCarriage == interactionType then
...
function FGlobal_Interaction_CheckAndGetPressedKeyCode_Xbox(deltaTime)
  ...
  if _ContentsGroup_NewUI_Fishing_All == false and PaGlobalFunc_Fishing_GetFishingMiniGame_3 ~= nil and PaGlobalFunc_Fishing_GetFishingMiniGame_3() == false then
    return
  end
...
function PanelInteraction:RollingAnimation(deltaTime)
  ...
  for collect_index, value in pairs(_collectInteractionTargetUIList) do
    local isShow = _collectInteractionTargetUIList[collect_index]:GetShow()
    if isShow then
      ...
      _collectInteractionTargetUIList[collect_index]:SetPosX(buttonPosX)
      _collectInteractionTargetUIList[collect_index]:SetPosY(buttonPosY)
      self._ui.needCollectTool:SetPosX(buttonPosX + 2)
      self._ui.needCollectTool:SetPosY(buttonPosY + 2)
...
function PanelInteraction:SetHarvestTexture(actor, index)
  ...
  if true == self:IsChangeLivestockHarvestTexture(actor) then
    ...
    _interactionTargetUIList[index]:ChangeTextureInfoNameAsync("renewal/function/console_function_interction_00.dds")
...
function PanelInteraction:GetLivestockHarvestIcon(control, index)
  ...
  control:ChangeTextureInfoNameAsync("combine/console/console_function_interction_00.dds")
