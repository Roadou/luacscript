...
local currentPosition = float3(0, 0, 0)
local currentRate = 0
local currentSelfPosX = 0
local currentSelfPosY = 0
...
function FromClient_EnterToInstanceField_UpdateMiniMap()
  currentSelfPosX = 9999
  currentSelfPosY = 9999
  local constSize = const.size
  local loopCount = constSize * constSize - 1
  for index = 0, loopCount do
    Panel_RadarRealLine.mapValue[index] = int2(9999, 9999)
...
local function updateMapImage(position, deltaTime)
  if position.x == currentPosition.x and position.z == currentPosition.z and currentRate == RadarMap_GetDistanceToPixelRate() and currentSelfPosX == RadarMap_GetSelfPosInRadar().x and currentSelfPosY == RadarMap_GetSelfPosInRadar().y and Panel_RadarRealLine.maxAlphaDesert == prevMaxAlpha then
    return
  end
  local isResistDesert = ToClient_getSelfPlayerIsResistDesert()
  local isDesert = ToClient_getCurrentRegionIsDesert()
  ...
  currentSelfPosX = radarMap.pcPosBaseControl.x
  currentSelfPosY = radarMap.pcPosBaseControl.y
  ...
  local currentSectorX = convertPosToSectorX(currentPosition)
  local currentSectorY = convertPosToSectorY(currentPosition)
  local currentSectorZ = convertPosToSectorZ(currentPosition)
  local inSectorPosX = convertPosToInSectorPosX(currentPosition)
  local inSectorPosY = convertPosToInSectorPosY(currentPosition)
  local inSectorPosZ = convertPosToInSectorPosZ(currentPosition)
  local startX = floor(-(inSectorPosX * 0.01 * currentRate) * 2 - constImageSize * 2 + currentSelfPosX)
  local startY = floor(inSectorPosZ * 0.01 * currentRate * 2 - constImageSize * 3 + currentSelfPosY)
  ...
  for index = 0, loopCount do
    ...
    local xSector = currentSectorX - halfConstSize + indexModConstSize
    local zSector = currentSectorZ + halfConstSize - floorIndexDivConstSize
    ...
    local xValue = (currentSectorX + index) % constSize
    local zValue = constSize - (currentSectorZ - floorIndexDivConstSize) % constSize - 1
...
function RadarBackground_updatePerFrame(position, deltaTime)
  updatePanel()
  updateMapImage(position, deltaTime)
end
