...
local superWhistleTextureByType = {
  [CppEnums.VehicleType.Type_Horse] = {
    path = "Renewal/Button/Console_Btn_Main_01.dds",
    x1 = 1,
    y1 = 1,
    x2 = 52,
    y2 = 52
...
function Panel_Widget_TopIcon_Servant_info:updateContent()
  ...
  if nil ~= landVehicleWrapper then
    ...
    if self._value.currentType ~= vehicleType or true == self._haveSuperWhistle and false == doHaveContentsItem(__eContentsType_SuperWhistle, CppEnums.VehicleType.Type_Horse) then
      self._value.currentType = vehicleType
      self._ui.static_ServantIcon_Template:ChangeTextureInfoNameAsync(textureByType[vehicleType].path)
      ...
      self._haveSuperWhistle = false
    end
    self:updateLandVehicleIcon(vehicleType)
    PaGlobalFunc_TopIcon_Show(TopWidgetIconType.LandVehicle)
  else
    PaGlobalFunc_TopIcon_Exit(TopWidgetIconType.LandVehicle)
  end
end
function Panel_Widget_TopIcon_Servant_info:updateLandVehicleIcon(vehicleType)
  if nil == self._ui.static_ServantIcon_Template then
    return
  end
  if CppEnums.VehicleType.Type_Horse == vehicleType or CppEnums.VehicleType.Type_Camel == vehicleType or CppEnums.VehicleType.Type_Donkey == vehicleType or CppEnums.VehicleType.Type_RidableBabyElephant == vehicleType then
    if true == doHaveContentsItem(__eContentsType_SuperWhistle, CppEnums.VehicleType.Type_Horse) then
      if false == self._haveSuperWhistle then
        self._ui.static_ServantIcon_Template:ChangeTextureInfoNameAsync(superWhistleTextureByType[vehicleType].path)
        local x1, y1, x2, y2 = setTextureUV_Func(self._ui.static_ServantIcon_Template, superWhistleTextureByType[vehicleType].x1, superWhistleTextureByType[vehicleType].y1, superWhistleTextureByType[vehicleType].x2, superWhistleTextureByType[vehicleType].y2)
        self._ui.static_ServantIcon_Template:getBaseTexture():setUV(x1, y1, x2, y2)
        self._ui.static_ServantIcon_Template:setRenderTexture(self._ui.static_ServantIcon_Template:getBaseTexture())
        self._ui.static_ServantIcon_Template:SetColor(Defines.Color.C_FFEEEEEE)
        self._haveSuperWhistle = true
      end
    elseif true == doHaveContentsItem(__eContentsType_OrderToServant, CppEnums.VehicleType.Type_Horse) then
      self._ui.static_ServantIcon_Template:SetColor(Defines.Color.C_FFFFE069)
      self._haveSuperWhistle = false
    else
      self._ui.static_ServantIcon_Template:SetColor(Defines.Color.C_FFEEEEEE)
      self._haveSuperWhistle = false
