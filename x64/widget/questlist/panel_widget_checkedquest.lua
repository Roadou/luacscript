  ...
  for ii = #self._timeAttackQuestList, 1, -1 do
    ...
    if nil ~= questUI then
      ...
      if remainTime > 0 then
        local isCheck = ToClient_isTimeAttackQuestConditionState(questUI._questNo._groupId, questUI._questNo._questId)
        if isCheck == true then
          questUI._uiEndTime:SetText("")
        else
...
function checkedQuestWidget:setGuildQuestUi(uiIndex)
  ...
  local startPosition = questUi._uiTitleBG:GetSizeY()
  questUi._uiGroupTitle:SetPosY(startPosition)
  local textSize = questUi._uiGroupTitle:GetPosY()
  if isDone == false then
    textSize = textSize + startPosition
  else
    textSize = textSize + 5
  end
  local index = 0
...
function checkedQuestWidget:setGuildQuest()
  ...
  if true == questData.isProgressing then
    ...
    local isSatisfied = ToClient_isSatisfyCurrentGuildQuest()
    if isSatisfied == false and questData.remainTime <= 0 then
      ...
      if questConditionCount <= completeCount then
        questData.demand = {}
        questData.demand[1] = " " .. PAGetString(Defines.StringSheet_GAME, "GUILDQUEST_COMPLETE_WIDGET")
        questData.state = self._config._questState._complete
...
function checkedQuestWidget:setQuestBG(control, BGtype)
  control:ChangeTextureInfoNameAsync("renewal/frame/console_frame_01.dds")
...
function checkedQuestWidget:findTarget(questGroupId, questId, condition, isAuto)
  if ToClient_isFindWayPossibleQuest(questGroupId, questId) == false then
    return
  end
...
function checkedQuestWidget:mouseOn_GuildQuset(isShow)
  ...
  if true == isShow then
    ...
    control:ChangeTextureInfoNameAsync("New_UI_Common_forLua/Default/BlackPanel_Series.dds")
...
function EventRadingOnQuest(questStaticWrapper, index)
  ...
  if nil ~= npcData then
    checkedQuestWidget._questGroupId = 0
    checkedQuestWidget._questId = 0
