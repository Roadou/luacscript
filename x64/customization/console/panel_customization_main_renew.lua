...
local CustomizationMain = {
  ...
  _keyGuidesText = {
    ...
    _strChangeMode = PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZE_UI_CHANGE_MODE_BTN_NAME"),
...
function PaGlobalFunc_Customization_ClickedZodiac(index)
  ...
  if zodiacImagePath ~= nil then
    self._ui._static_ZodiacImage:ChangeTextureInfoNameAsync(zodiacImagePath)
...
function CustomizationMain:MakeKeyGuide(state)
  ...
  if self._config._common == state then
    ...
    if _ContentsGroup_MaidActorSystem == true then
      self:addKeyGuide(keyGuideTableTemplete, {
        self._keyGuideTextureConfig._RT,
        self._keyGuideTextureConfig._Plus,
        self._keyGuideTextureConfig._Y
      }, keyText._strChangeMode)
    end
...
function PaGlobalFunc_Customization_ChangeCharacterMode()
  if _ContentsGroup_MaidActorSystem == false then
    return
  end
  local prevMaidMode = getCustomizingManager():isShowMaidActorMode()
  ToClient_DeleteCustomizationTempData()
  HandleClicked_CloseIngameCustomizationActual()
  if prevMaidMode == true then
    IngameCustomize_ShowActual(false)
  else
    IngameCustomize_ShowActual(true)
  end
end
function CustomizationMain:InitEvent()
  ...
  if _ContentsGroup_MaidActorSystem == true then
    Panel_Customizing:registerPadEvent(__eConsoleUIPadEvent_RTPress_Y, "PaGlobalFunc_Customization_ChangeCharacterMode()")
  end
  if ToClient_IsDevelopment() == true then
    registerEvent("PlayGoClickMainMenu", "FromClient_PlayGoClickMainMenu")
    registerEvent("PlayGoClickSubMenu", "FromClient_PlayGoClickSubMenu")
    registerEvent("PlayGoBackToSelectClass", "FromClient_PlayGoBackToSelectClass")
    registerEvent("PlayGoMoveBack", "FromClient_PlayGoMoveBack")
  end
end
...
function FromClient_PlayGoClickMainMenu(mainMenuIdx)
  PaGlobalFunc_Customization_ClickedMainMenu(mainMenuIdx)
end
function FromClient_PlayGoClickSubMenu(mainIdx, subIdx)
  local self = CustomizationMain
  if mainIdx == 0 and subIdx < self._config._CustomizingMaxSubTree then
    PaGlobalFunc_Customization_ClickedCustomizing(subIdx)
  elseif mainIdx == 1 and subIdx < getZodiacCount() then
    PaGlobalFunc_Customization_ClickedZodiac(subIdx)
  elseif mainIdx == 2 and subIdx < self._config._ActionMaxSubTree then
    PaGlobalFunc_Customization_ClickedAction(subIdx)
  else
    return
  end
end
function FromClient_PlayGoBackToSelectClass()
  PaGlobalFunc_Customization_SelectClass()
end
function FromClient_PlayGoMoveBack()
