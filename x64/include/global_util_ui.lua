...
function UI.setLimitTextToReferenceAndAddTooltip(hoverTargetControl, title, desc, textReferenceControl)
  if hoverTargetControl == nil or textReferenceControl == nil then
    return
  end
  if nil == textReferenceControl.IsLimitText or nil == textReferenceControl.SetTextMode then
    UI.ASSERT(nil ~= textReferenceControl.IsLimitText, "textReferenceControl does not contain key <IsLimitText>")
    return
  end
  textReferenceControl:SetTextMode(__eTextMode_LimitText)
  textReferenceControl:SetText(textReferenceControl:GetText())
  if textReferenceControl:IsLimitText() then
    local key = #_tooltipControlRefTable + 1
    hoverTargetControl:SetIgnore(false)
    _tooltipControlRefTable[key] = {}
    _tooltipControlRefTable[key].control = textReferenceControl
    _tooltipControlRefTable[key].title = title
    _tooltipControlRefTable[key].desc = desc
    hoverTargetControl:addInputEvent("Mouse_On", "PaGlobalFunc_GlobalUtil_limitTextTooltip(" .. key .. ")")
    hoverTargetControl:addInputEvent("Mouse_Out", "TooltipSimple_Hide()")
  end
end
function UI.setTextAndAddTooltip(control, title, desc)
  if nil == control then
    return
  end
  if nil == control.SetTextMode then
    return
  end
  control:SetText(control:GetText())
  local key = #_tooltipControlRefTable + 1
  ...
  _tooltipControlRefTable[key].title = title
  _tooltipControlRefTable[key].desc = desc
  control:addInputEvent("Mouse_On", "PaGlobalFunc_GlobalUtil_TextTooltip(" .. key .. ")")
  control:addInputEvent("Mouse_Out", "TooltipSimple_Hide()")
end
...
function PaGlobalFunc_GlobalUtil_TextTooltip(key)
  ...
  if data.control:IsLimitText() then
    TooltipSimple_Hide()
    return
  end
...
function FromClient_ShowLimitTextToolTip(isShow, control)
  if (isShow == false or isShow == nil or control == nil) and TooltipSimple_Hide ~= nil then
    TooltipSimple_Hide()
    return
  end
...
function UI.changeTextureUV(control, url, uv)
  control:ChangeTextureInfoNameAsync(url)
...
function PaGlobalFunc_ConsoleKeyGuide_SetAlign(listKeyGuid, parentControl, alignType, keySize, keySpace)
  ...
  if CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP == alignType then
    ...
    local backgroundTexturePadding = 40
    ...
    for index, value in pairs(reversedTable) do
      if value:GetShow() == true then
        ...
        if childCount == 0 then
          ...
          posX = posX - length
          if backgroundTexturePadding > posX then
...
function PaGlobalFunc_GlobalUtil_RoundRateToSecondPoint(rate)
  if rate <= 0 then
    rate = 0
  end
  local roundRate = (rate + 50) / 100
  roundRate = math.floor(roundRate) / 100
  return string.format("%.2f", roundRate)
end
function PaGlobalFunc_GlobalUtil_RoundRateToThridPoint(rate)
  if rate <= 0 then
    rate = 0
  end
  local roundRate = (rate + 5) / 10
  roundRate = math.floor(roundRate) / 1000
  return string.format("%.3f", roundRate)
end
registerEvent("FromClient_luaLoadCompleteLateUdpate", "FromClient_UpdateDynamicScalePanelList")
registerEvent("FromClient_ShowLimitTextToolTip", "FromClient_ShowLimitTextToolTip")
