...
function convertSecondsToClockTime(seconds)
  ...
  if clockSeconds < 10 then
    clockTime = clockTime .. "0" .. tostring(math.floor(clockSeconds))
  else
    clockTime = clockTime .. tostring(math.floor(clockSeconds))
  end
  return clockTime
end
...
function onCollectGarbage()
  local time = os.clock()
  local rv = collectgarbage("collect")
  _PA_SVC_LOG("collect LUA garbage rv = " .. tostring(rv))
  _PA_SVC_LOG("collect LUA garbage time = " .. tostring(os.clock() - time))
end
function onStopGarbage()
  local rv = collectgarbage("stop")
  _PA_SVC_LOG("stop LUA garbage rv = " .. tostring(rv))
end
function onReStartGarbage()
  local rv = collectgarbage("restart")
  _PA_SVC_LOG("restart LUA garbage rv = " .. tostring(rv))
end
...
function isItemMarketSecureCode()
  ...
  if (isGameTypeKorea() or isGameTypeJapan() or isGameTypeRussia() or isGameTypeNA() or isGameTypeTaiwan() or isGameTypeSA()) and true == isSecureCode then
    return true
  else
    return false
  end
...
function PaGlobalFunc_Util_SetGuildMemberGradeIcon(control, guildMemberGrade)
  ...
  control:ChangeTextureInfoNameAsync("Renewal/PcRemaster/Remaster_ETC_Guild.dds")
  if __eGuildMemberGradeMaster == guildMemberGrade then
    control:ChangeTextureInfoNameAsync("Combine/Etc/Combine_Etc_Guild_00.dds")
  ...
  elseif __eGuildMemberGradeMissionOfficer == guildMemberGrade then
    ...
    control:getBaseTexture():setUV(x1, y1, x2, y2)
  elseif __eGuildMemberGradeVolunteer == guildMemberGrade then
    control:ChangeTextureInfoNameAsync("Combine/Etc/Combine_Etc_Guild_00.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(control, 227, 17, 270, 43)
    control:getBaseTexture():setUV(x1, y1, x2, y2)
  end
...
function PaGlobalFunc_Util_CraftLevelColorStrReplace(lev, str)
  if lev >= 1 and lev <= 10 then
    lev = "<PAColor0xFFC4C4C4>" .. str .. "<PAOldColor>"
  elseif lev >= 11 and lev <= 20 then
    lev = "<PAColor0xFF76B24D>" .. str .. "<PAOldColor>"
  elseif lev >= 21 and lev <= 30 then
    lev = "<PAColor0xFF3B8BBE>" .. str .. "<PAOldColor>"
  elseif lev >= 31 and lev <= 40 then
    lev = "<PAColor0xFFEBC467>" .. str .. "<PAOldColor>"
  elseif lev >= 41 and lev <= 50 then
    if true == _ContentsGroup_ChinaFontColor then
      lev = "<PAColor0xffffad38>" .. str .. "<PAOldColor>"
    else
      lev = "<PAColor0xFFD04D47>" .. str .. "<PAOldColor>"
    end
  elseif lev >= 51 and lev <= 80 then
    lev = "<PAColor0xFFB23BC7>" .. str .. "<PAOldColor>"
  elseif lev >= 81 and lev <= 180 then
    lev = "<PAColor0xFFC78045>" .. str .. "<PAOldColor>"
  end
  return lev
end
...
function PaGlobalFunc_Util_IsDragon(vehicleType)
  if __eVehicleType_GuildDragon == vehicleType then
    return true
  end
  return false
end
...
Util.VEHICLE_EQUIP_TYPE_TEXTURE_PATH = {
  ...
  [Util.ENUM_VEHICLE_EQUIP_TYPE_GROUP._DRAGON] = ""
}
...
function PaGlobalFunc_Util_getVehicleTypeGroup(vehicleType)
  ...
  elseif true == PaGlobalFunc_Util_IsDragon(vehicleType) then
    return Util.ENUM_VEHICLE_EQUIP_TYPE_GROUP._DRAGON
  end
  return Util.ENUM_VEHICLE_EQUIP_TYPE_GROUP._COUNT
end
...
Util.Help = {
  ...
  desc = {
    ...
    OlviaAcademyPass = PAGetString(Defines.StringSheet_GAME, "LUA_OLVIA_ACADEMY_TITLE_TOOLTIP_DESC")
  },
  controls = {}
}
...
function PaGlobalFunc_Util_GetItemGradeColorName(enchantItemKey, isUseRoma)
  ...
  if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() then
    if itemSSW:isSpecialEnchantItem() == false or itemSSW:isSpecialAccessory() == true then
      name = HighAccessoryEnchantLevel_ReplaceString(enchantLevel) .. " " .. name
    end
  elseif enchantLevel > 0 and enchantLevel < 26 then
    name = HighEnchantLevel_ReplaceString(enchantLevel) .. " " .. name
  end
...
function PaGlobalFunc_InGameBannerSolare()
  ...
  local isPS5 = ToClient_isPS5()
  local isXBox = ToClient_isXBox()
  local isPC = not isPS5 and not isXBox
  local url
  if isPC == true and isGameTypeCH() == false then
    if isGameTypeASIA() == true then
      url = "https://blackdesert.pearlabyss.com/Asia/en-us/Game/InGame/Wiki?_masterWikiNo=163"
    else
      Panel_WebHelper_ShowToggle("Solare")
      return
    end
  elseif isGameTypeKorea() == true then
    url = "https://www.kr.playblackdesert.com/ko-KR/News/Detail?groupContentNo=9518"
  ...
  elseif true == isGameTypeASIA() then
    url = ToClient_getIngameBoardURL() .. "/WikiSearch/solare"
  elseif isGameTypeCH() == true then
    url = "https://bd.qq.com/cp/a20240715gwyqyy/adventurer.html?newsid=18448981&newid=18551238"
  elseif isPS5 == true or isXBox == true then
    url = "https://www.console.playblackdesert.com/Wiki/Default?wikiNo=303"
  end
  if url == nil then
    return
  end
  ...
  elseif isXBox == true then
    ToClient_OpenNativeWebBrowserXB1(url, PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SOLARE_TITLE"))
  elseif isGameTypeCH() == true then
